...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_DILL_DUMMY_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_DILL_DUMMY_H_
...
class CDIL_LIN_DUMMY : public CBaseDIL_LIN_Controller {
 public:
  ...
  HRESULT LIN_GetControllerParams(LONG& lParam, UINT nChannel,
  ...
  HRESULT LIN_GetErrorCount(SERROR_CNT& sErrorCnt, UINT nChannel,
                            ECONTR_PARAM eContrParam);
  HRESULT LIN_SetAppParams(HWND hWndOwner);
  HRESULT LIN_ManageMsgBuf(BYTE bAction, DWORD ClientID,
                           CBaseLINBufFSE* pBufObj);
  HRESULT LIN_RegisterClient(BOOL bRegister, DWORD& ClientID,
                             char* pacClientName);
  ...
  // Schedule table
  HRESULT LIN_RegisterLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                       CSheduleTable ouTable, int& nHandle);
  HRESULT LIN_DeRegisterLinScheduleTabel(DWORD& dwClientId, int& nChannel,
                                         int& nTableHandle);
  HRESULT LIN_StartLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                    int& nTableHandle);
  HRESULT LIN_EnableLinScheuleCommand(DWORD& dwClientId, int& nChannel,
                                      int nTableHandle, int nIndex,
                                      bool bEnable);
  HRESULT LIN_UpdateLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                     int& nHandle, CSheduleTable& ouTable);

  // Individual Header commands.
  HRESULT LIN_RegisterLinHeader(DWORD& dwClientId, int& nChannel, int nId,
                                int nCycleTimer);
  HRESULT LIN_DeRegisterLinHeader(DWORD& dwClientId, int& nChannel, int nId);
};

#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_DILL_DUMMY_H_