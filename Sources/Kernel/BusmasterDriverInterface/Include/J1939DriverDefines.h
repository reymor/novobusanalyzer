...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_J1939DRIVERDEFINES_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_J1939DRIVERDEFINES_H_
...
#define COPY_DATA_INC_DEST(TgtStream, SrcStream, TotBytes) \
  {                                                        \
    memcpy(TgtStream, SrcStream, TotBytes);                \
    TgtStream += TotBytes;                                 \
  }
#define COPY_DATA_INC_SOURCE(TgtStream, SrcStream, TotBytes) \
  {                                                          \
    memcpy(TgtStream, SrcStream, TotBytes);                  \
    SrcStream += TotBytes;                                   \
  }
...
typedef void (*PCLBCK_FN_LDATA_IND)(DWORD dwClient, UINT32 unPGN, BYTE bySrc,
                                    BYTE byDest, BOOL bSuccess);
typedef void (*PCLBCK_FN_BC_LDATA_CONF)(DWORD dwClient, UINT32 unPGN,
...
enum ETYPE_ACK { ACK_POS = 0, ACK_NEG };
...
typedef struct tagSTRUCT_PGN {
  BYTE m_byPDU_Specific : 8; /* PDU Specific (PS), Destination address if 0 <=
                                PF <= 239. Group Extension (GE)if 240 <= PF <=
                                255. */
  BYTE m_byPDU_Format : 8;   // PDU Format (PF), Indicates Peer_2_peer or
                             // Broadcast.
...
typedef struct tagSTRUCT_29_BIT_ID {
  ...
  UNION_PGN m_uPGN;  // Parameter group number(PGN). SAE lists various PGNS and
                     // its SPNS.
...
typedef struct tagSTJ1939_MSG {
  ...
  STJ1939_MSG_PROPERTIES
  m_sMsgProperties; /* J1939 message properties. (29 bit Extended ID,
                       Timestamp, Channel, Type, Direction) */
  ...
  UINT unGetSize(void) const {
    return (sizeof(STJ1939_MSG_PROPERTIES) + sizeof(UINT) +
            (sizeof(BYTE) * m_unDLC));
  }

  /* Renders the BYTE stream of the total message. */
  void vGetDataStream(BYTE* pbyData) const {
    COPY_DATA_INC_DEST(pbyData, &m_sMsgProperties,
                       sizeof(STJ1939_MSG_PROPERTIES));
  ...
  void vSetDataStream(BYTE* pbyData) {
    COPY_DATA_INC_SOURCE(&m_sMsgProperties, pbyData,
                         sizeof(STJ1939_MSG_PROPERTIES));
    UINT unTempDLC = 0;
...
const int MAX_MSG_LEN_J1939 =
    sizeof(STJ1939_MSG_PROPERTIES) + sizeof(UINT) + MAX_DATA_LEN_J1939;

#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_J1939DRIVERDEFINES_H_