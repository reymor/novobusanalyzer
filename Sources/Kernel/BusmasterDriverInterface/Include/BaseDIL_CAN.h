...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_BASEDIL_CAN_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_BASEDIL_CAN_H_
...
class CBaseDIL_CAN : public IBusService {
 public:
  ...
  virtual HRESULT DILC_ListHwInterfaces(INTERFACE_HW_LIST& asSelHwInterface,
                                        ...
                                        bool bLoadFromXML = 0) = 0;

  /**
   * Call this function to select a particular hardware interface.
   */
  virtual HRESULT DILC_SelectHwInterfaces(
   ...
   * Send messages
   */
  virtual HRESULT DILC_SendMsg(DWORD dwClientID,
                               const STCAN_MSG& sCanTxMsg) = 0;
   ...
   */
  virtual HRESULT DILC_GetControllerParams(LONG& lParam, UINT nChannel,
  ...
  virtual HRESULT DILC_GetErrorCount(SERROR_CNT& sErrorCnt, UINT nChannel,
                                     ECONTR_PARAM eContrParam) = 0;
  virtual HRESULT DILC_SetHardwareChannel(
      PSCONTROLLER_DETAILS m_asControllerDetails, DWORD dwDriverId,
      bool bHardwareListed = true,
      unsigned int unChannelCount = defCHANNEL_CAN_MAX) = 0;
};

#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_BASEDIL_CAN_H_