...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_BASEDIL_FLEXRAY_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_BASEDIL_FLEXRAY_H_
...
class CBaseDIL_FLEXRAY : public IBusService {
 public:
   ...
   */
  // clang-format on
  virtual HRESULT DILF_RegisterClient(BOOL bRegister, DWORD& ClientID,
   ...
   */
  virtual HRESULT DILF_ManageMsgBuf(BYTE bAction, DWORD ClientID,
                                    CBaseFLEXBufFSE* pBufObj) = 0;
   ...
   * it*/
  virtual HRESULT DILF_GetTimeModeMapping(SYSTEMTIME& CurrSysTime,
                                          UINT64& TimeStamp) = 0;
  ...
  virtual HRESULT DILF_SelectHwInterfaces(
      const INTERFACE_HW_LIST& sSelHwInterface) = 0;
  ...
  release the memory. */
  // virtual HRESULT DILF_SetConfigData(PCHAR pInitData,  ABS_CLUSTER_INFO
  // asClusterinfo, int Length, ABS_DEVICE_CONFIG  asDeviceConfig) = 0;
  virtual HRESULT DILF_SetConfigData(
      IBMNetWorkGetService* ouAbsSFibexContainer) = 0;
  ...
  /* Send Flx messages*/
  virtual HRESULT DILF_SendFlxMsg(DWORD dwClientID,
                                  s_FLXDATAMSG* pouFlxTxMsg) = 0;

  /* Send PDU */
  virtual HRESULT DILF_SendPdu(DWORD dwClientID, s_FLXPDUDATAMSG* pouFlxTxMsg,
                               bool bSetUpdateBit) = 0;

  /* Send Flx messages*/
  virtual HRESULT DILF_DeleteFlxMsg(DWORD dwClientID,
                                    s_FLXDATAMSG* pouFlxTxMsg) = 0;
  ...
  virtual HRESULT DILF_GetLastErrorString(std::string acErrorStr,
                                          HRESULT& nError) = 0;

  virtual HRESULT DILF_GetControllerCapabilities(
      s_FLXControllerCapabilities& ouFlexRayCapabilities) = 0;

  virtual HRESULT DILF_GetControllerParams(
      std::list<FlexRayControllerParams>&) = 0;
};

#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_BASEDIL_FLEXRAY_H_