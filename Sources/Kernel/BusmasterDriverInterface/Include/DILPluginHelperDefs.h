#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_DILPLUGINHELPERDEFS_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_DILPLUGINHELPERDEFS_H_

#define LOG_ERR_MSG()
#define VALIDATE_POINTER_RETURN_VAL(Ptr, RetVal) \
  if (Ptr == nullptr) {                          \
    return RetVal;                               \
  }
#define VALIDATE_VALUE_RETURN_VAL(Val1, Val2, RetVal) \
  if (Val1 != Val2) {                                 \
    return RetVal;                                    \
  }
...
#define VALIDATE_POINTER_NO_RETURN_LOG(Ptr) \
  if (Ptr == nullptr) {                     \
    LOG_ERR_MSG();                          \
  }
#define VALIDATE_POINTER_RETURN_VOID_LOG(Ptr) \
  if (Ptr == nullptr) {                       \
    LOG_ERR_MSG();                            \
    return;                                   \
  }
...
class SELECTED_CHANNELS {
 public:
  SELECTED_CHANNELS() { m_nChannelCount = 0; }
...
#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_DILPLUGINHELPERDEFS_H_