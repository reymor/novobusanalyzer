...
CBaseDIL_FLEXRAY* getFlexRayDil() {
  ...
  if (nullptr != dllHandle) {
    auto pGetFlexrayDil =
        (GETFLEXRAYDIL)GetProcAddress(dllHandle, "GetFlexRayDIL");
...
USAGEMODE HRESULT GetDilInterface(ETYPE_BUS eBusType,
  ...
  switch (eBusType) {
    case CAN: {
      ...
      *ppvInterface =
          sg_pouDIL_CAN; /* Doesn't matter even if sg_pouFP_CAN is null */
    } break;
    case LIN: {
      ...
      // Else the object has been existing already
      *ppvInterface =
          sg_pouDIL_LIN; /* Doesn't matter even if sg_pouFP_CAN is null */
    } break;
    case MCNET:
      break;
    case J1939: {
      ...
      // Else the object has been existing already
      *ppvInterface =
          sg_pouDILI_J1939; /* Doesn't matter even if sg_pouFP_CAN is null */
    } break;
    case FLEXRAY: {
      ...
      // Else the object has been existing already
      *ppvInterface =
          sg_pouDIL_FLEXRAY; /* Doesn't matter even if sg_pouFP_CAN is null */
    } break;
    default: {
      hResult = S_FALSE;
    } break;
