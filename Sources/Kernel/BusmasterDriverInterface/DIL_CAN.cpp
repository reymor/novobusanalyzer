...
#include "../../BUSMASTER/Utility/MultiLanguageSupport.h"  //@TODO: find best way to include this
#include "Error.h"
#include "stdafx.h"
...
int CDIL_CAN::ExitInstance() { return 0; }
/* ROUTER CODE STARTS */
 ...
 */
BOOL CDIL_CAN::InitInstance() { return TRUE; }

void CDIL_CAN::vInitialize(SCONTROLLER_DETAILS& controller,
                           BOOL bUpdateHWDesc) {
  ...
  // OCI_CANFD_TX_DELAY_COMPENSATION_OFF
  controller.m_bTxDelayCompensationControl = 0;
...
HMODULE CDIL_CAN::vLoadEtasBoaLibrary() {
  ...
  // Check for BOA 2
  lError = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ETAS\\BOA\\2", 0,
                        KEY_READ, &sKey);
  ...
  // Check for BOA 1.5
  lError = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ETAS\\BOA\\1.5", 0,
                        KEY_READ, &sKey);
  if (lError == ERROR_SUCCESS) {
    RegCloseKey(sKey);
  ...
  // Check for BOA 1.4
  lError = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ETAS\\BOA\\1.4", 0,
                        KEY_READ, &sKey);
  if (lError == ERROR_SUCCESS) {
    RegCloseKey(sKey);
...
HRESULT CDIL_CAN::DILC_SelectDriver(DWORD dwDriverID, HWND hWndOwner) {
  ...
  if (DILC_GetSelectedDriver() == dwDriverID) {
    ...
    if (m_hDll == nullptr) {
      ...
      pfGetIDILCAN_Controller = (GETIDIL_CAN_CONTROLLER)GetProcAddress(
          m_hDll, "GetIDIL_CAN_Controller");
 ...
 */
HRESULT CDIL_CAN::DILC_ManageMsgBuf(BYTE bAction, DWORD ClientID,
 ...
 */
HRESULT CDIL_CAN::DILC_RegisterClient(BOOL bRegister, DWORD& ClientID,
                                      char* pacClientName) {
  return m_pBaseDILCAN_Controller->CAN_RegisterClient(bRegister, ClientID,
                                                      pacClientName);
}
 ...
 */
DWORD CDIL_CAN::DILC_GetSelectedDriver(void) { return m_dwDriverID; }
 ...
 */
HRESULT CDIL_CAN::DILC_GetTimeModeMapping(SYSTEMTIME& CurrSysTime,
                                          UINT64& TimeStamp,
                                          LARGE_INTEGER& QueryTickCount) {
  return m_pBaseDILCAN_Controller->CAN_GetTimeModeMapping(
...
// Flag is used to prevent initialization when loaded from XMl Config
  ...
  HRESULT hr = m_pBaseDILCAN_Controller->CAN_ListHwInterfaces(sSelHwInterface,
                                                              nCount, InitData);
 ...
 */
HRESULT CDIL_CAN::DILC_SelectHwInterfaces(
    const INTERFACE_HW_LIST& sSelHwInterface, INT nCount) {
  return m_pBaseDILCAN_Controller->CAN_SelectHwInterface(sSelHwInterface,
                                                         nCount);
}
 ...
 * release the memory.
 */
HRESULT CDIL_CAN::DILC_SetConfigData(PSCONTROLLER_DETAILS pInitData,
                                     int Length) {
 ...
 */
HRESULT CDIL_CAN::DILC_GetCntrlStatus(const HANDLE& hEvent,
                                      UINT& unCntrlStatus) {
 ...
 */
HRESULT CDIL_CAN::DILC_GetControllerParams(LONG& lParam, UINT nChannel,
                                           ECONTR_PARAM eContrParam) {
  if (m_pBaseDILCAN_Controller) {
    return m_pBaseDILCAN_Controller->CAN_GetControllerParams(lParam, nChannel,
                                                             eContrParam);
  } else {
    return S_FALSE;
  }
 ...
 */
HRESULT CDIL_CAN::DILC_SetControllerParams(int nValue,
                                           ECONTR_PARAM eContrparam) {
  if (m_pBaseDILCAN_Controller) {
    return m_pBaseDILCAN_Controller->CAN_SetControllerParams(nValue,
                                                             eContrparam);
  } else {
    return S_FALSE;
  }
 ...
 */
HRESULT CDIL_CAN::DILC_GetErrorCount(SERROR_CNT& sErrorCnt, UINT nChannel,
                                     ECONTR_PARAM eContrParam) {
  return m_pBaseDILCAN_Controller->CAN_GetErrorCount(sErrorCnt, nChannel,
                                                     eContrParam);
}
HRESULT CDIL_CAN::DILC_SetHardwareChannel(
    PSCONTROLLER_DETAILS m_asControllerDetails, DWORD dwDriverId,
    bool bHardwareListed, unsigned int unChannelCount) {
  return m_pBaseDILCAN_Controller->CAN_SetHardwareChannel(
      m_asControllerDetails, dwDriverId, bHardwareListed, unChannelCount);
}