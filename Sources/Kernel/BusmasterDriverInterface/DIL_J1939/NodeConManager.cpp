    ...
    switch (pThreadParam->m_unActionCode) {
      case INVOKE_FUNCTION: {
        while (psNodeConMgr->m_ouMsgBufVSE.GetMsgCount() > 0) {
          ...
          int nResult = psNodeConMgr->m_ouMsgBufVSE.ReadFromBuffer(
              nType, psNodeConMgr->m_abyData, nSize);
...
Modifications  :
******************************************************************************/
BOOL CNodeConManager::bIsActive(void) { return (m_bIsActive == TRUE); }
...
Modifications  :
******************************************************************************/
CNodeConManager::CNodeConManager(int nNodeNo, char* pacNodeName,
                                 UINT64 un64ECUName, BYTE byPrefAddress,
                                 BOOL bIsMonNode) {
  ...
  m_pConDet = m_pNodeConDetArr[0] =
      new CConnectionDet(ADDRESS_NULL, ADDRESS_NULL);
...
Modifications  :
******************************************************************************/
BYTE CNodeConManager::byGetNodeAddress() { return m_byNodeAddress; }
...
Modifications  :
******************************************************************************/
BYTE CNodeConManager::byGetConCount() { return m_byConCount; }
...
BOOL CNodeConManager::bIsMsgForThisNode(UINT32 unExtId) {
  ...
  return (
      (uExtId.m_s29BitId.m_uPGN.m_sPGN.m_byPDU_Specific == m_byNodeAddress) ||
  ...
  if (uExtId.m_s29BitId.m_bySrcAddress == m_byNodeAddress) {
    if (m_u64ECUName.m_un64ECU_NAME < unECUName) {
      ...
      if (m_u64ECUName.m_sECU_NAME.m_byARB_ADRS_CPL == 0x1) {
        m_byNodeAddress =
            CNetworkMgmt::ouGetNWManagementObj().byGetUnclaimedAddress();
    ...
    CNetworkMgmt::ouGetNWManagementObj().vUpdateAddressMap(
        unECUName, uExtId.m_s29BitId.m_bySrcAddress);
  }
}
...
BOOL CNodeConManager::bProcessNodeLevelMsg(const STCANDATA& sCanData) {
  ...
  BOOL bResult = TRUE;
  STJ1939_MSG sJ1939Msg;
  EJ1939_MSG_TYPE eType =
      eGetMsgType(sCanData.m_uDataInfo.m_sCANMsg.m_unMsgID,
  ...
  vCreateTempJ1939Msg(sJ1939Msg, sCanData,
...
void CNodeConManager::vStartTimerFunction(UINT /*unTimePeriod*/) {
  ...
  CreateTimerQueueTimer(&m_hTimer, nullptr, WaitOrTimerCallback, this, 0, nTWD,
...
void CNodeConManager::WriteIntoClientsBuffer(STJ1939_MSG& sJ1939Msg) {
  TRACE("%x, %x, %x, %x, %x\n",
        ...
        sJ1939Msg.m_sMsgProperties.m_uExtendedID.m_s29BitId.m_uPGN.m_sPGN
            .m_byPDU_Specific,
        sJ1939Msg.m_sMsgProperties.m_eDirection, m_byNodeAddress);
  ...
  for (int i = 0; i < m_OutBufArr.GetSize(); i++) {
    ...
    podCurrBuf->WriteIntoBuffer(ETYPE_BUS::J1939, abyData,
                                sJ1939Msg.unGetSize());
  }
}
...
void CNodeConManager::vProcBAMMsg(const STCAN_MSG& sCanMsg) {
  ...
  m_pConDet->m_BCLastFrameLen =
      (UINT)byGetLastFrameLen(m_pConDet->m_BCRXLongDataLen);
  UINT32 unPGN = 0;
...
void CNodeConManager::vProcConRequestMsg(const STCAN_MSG& sCanMsg) {
  ...
  m_pConDet->m_unRxLastFrameLen =
      (UINT)byGetLastFrameLen(m_pConDet->m_unRXLongDataLen);
  ...
  vSendConAckMsg(m_pConDet->m_byDestAddress, sCanMsg.m_ucData[4],
                 m_pConDet->m_unPGN, sCanMsg.m_ucChannel);
  ...
  switch (m_pConDet->eGetConStatus()) {
    ...
    case T_STARTUP: {
      CTransferLayer::ouGetTransLayerObj().vTConnectCon(0, T_CONNECTED, 0,
                                                        CM_STANDARD);
    } break;
    case T_CONNECTED: {
...
Modifications  :
******************************************************************************/
void CNodeConManager::vReInitConDet() { m_pConDet->vInitializeMemberVar(); }
...
Modifications  :
******************************************************************************/
void CNodeConManager::vProcessCmdAdresMsg(const STCAN_MSG& sCANMsg,
                                          UINT64 un64ECU_NAME) {
  if (un64ECU_NAME == m_u64ECUName.m_un64ECU_NAME) {
    if (m_u64ECUName.m_sECU_NAME.m_byARB_ADRS_CPL ==
        0x1)  // dynamic adres capable
    {
...
void CNodeConManager::vProcessBroadCastData(const STCANDATA& sCanData) {
  ...
  if (byLastReceived == (BYTE)(sCanMsg.m_ucData[0] - 1)) {
    ...
    if (m_pConDet->m_BCRxSeqVar == m_pConDet->m_BCTotalPackets) {
      ...
      sJ1939Msg.m_sMsgProperties.m_uExtendedID.m_s29BitId.vSetPGN(
          m_pConDet->m_BCPGN);
      ...
      memcpy(&(m_pConDet->m_BCRXLongData[unDataIndex]), &(sCanMsg.m_ucData[1]),
             MAX_TPDU_DATA_SIZE);
    }
  }
}
...
void CNodeConManager::vProcessLongData(const STCANDATA& sCanData) {
  ...
  if (byLastReceived == (sCanMsg.m_ucData[0] - 1)) {
    ...
    if (m_pConDet->m_byCurrPacket == (m_pConDet->m_byMaxPacketWOC2S - 1)) {
      ...
      // prepare id
      UINT unExtId =
          Prepare_P2P_Id(PDU_FORMAT_TPCM, m_byNodeAddress,
    ...
    m_pConDet->m_byCurrPacket =
        (m_pConDet->m_byCurrPacket + 1) % m_pConDet->m_byMaxPacketWOC2S;
    ...
    if (m_pConDet->m_byRxSeqNo == m_pConDet->m_unRxTotalPackets) {
      ...
      EXECUTE_CLBCK_FN(CLBCK_FN_LDATA_IND, m_pConDet->m_unPGN,
                       m_pConDet->m_bySrcAddress, m_pConDet->m_byDestAddress,
                       TRUE);
...
Modifications  :
******************************************************************************/
void CNodeConManager::vSendConAbortMsg(UINT32 unExtId, BYTE byReason,
                                       UINT unChannel) {
  UNION_29_BIT_ID uExtId = {0};
  ...
  UINT unId =
      Prepare_P2P_Id(PDU_FORMAT_TPCM, m_byNodeAddress,
                     uExtId.m_s29BitId.m_bySrcAddress, DEFAULT_PRIORITY - 1);
...
Input(s)       :
Output         :
Functionality  :  Process connection level msg like TPCM, TPDT, BAM, CON ABORT
ETC., Member of      :  CNodeConManager Friend of      :  - Author(s)      :
Pradeep Kadoor Date Created   :  23/11/2010 Modifications  :
******************************************************************************/
BOOL CNodeConManager::bProcessConLevelMsg(const STCANDATA& CurrMsgCAN) {
  ...
  if (bIsEOM_ACK(sCanMsg.m_unMsgID, sCanMsg.m_ucData[0])) {
    ...
    if (m_pConDet != nullptr) {
      if (m_pConDet->eGetConStatus() == T_DISCONNECTED) {
        ...
        vSendConAbortMsg(sCanMsg.m_unMsgID, REASON_NODE_ENGAGED,
                         sCanMsg.m_ucChannel);
      }
    } else {
  ...
  } else if (bIsBAM(sCanMsg.m_unMsgID,
                    sCanMsg.m_ucData[0])) /* BROADCAST Announce*/
  {
    if (m_pConDet != nullptr) {
      if (m_pConDet->eGetConStatus() == T_DISCONNECTED) {
        vReInitConDet();
...
Modifications  :
******************************************************************************/
void CNodeConManager::vSendACLMsg(BYTE byDestAddress, UINT unChannel,
                                  BOOL bNewEvent) {
  UINT32 unExtId = Prepare_P2P_Id(PDU_FORMAT_ACL, m_byNodeAddress,
                                  byDestAddress, DEFAULT_PRIORITY - 1);
  vSendFrame(MAX_FRAME_DATA_SIZE, (BYTE*)&(m_u64ECUName.m_un64ECU_NAME),
             unExtId, unChannel);
  // Update address map
  CNetworkMgmt::ouGetNWManagementObj().vUpdateAddressMap(
      m_u64ECUName.m_un64ECU_NAME, m_byNodeAddress);
  if (bNewEvent == TRUE) {
...
void CNodeConManager::vTransmitMessage(STJ1939_MSG* psMsg) {
  switch (psMsg->m_sMsgProperties.m_eType) {
    case MSG_TYPE_BROADCAST: {
      ...
      BYTE byDestAddress = psMsg->m_sMsgProperties.m_uExtendedID.m_s29BitId
                               .m_uPGN.m_sPGN.m_byPDU_Specific;

      if (psMsg->m_unDLC > MAX_FRAME_DATA_SIZE) {
        BYTE byPriority = psMsg->m_sMsgProperties.m_uExtendedID.m_s29BitId
                              .m_uPGN.m_sPGN.m_byPriority;
        pCreateAndGetConDet(byDestAddress);
        m_pConDet->m_eTxConMode = CM_BROADCAST;
        vSendBAM(psMsg->m_unDLC, unPGN, byDestAddress, byPriority,
                 psMsg->m_sMsgProperties.m_byChannel);
        vSendBroadCastData(psMsg->m_pbyData, psMsg->m_unDLC, byDestAddress,
                           byPriority, psMsg->m_sMsgProperties.m_byChannel);
        ...
        EXECUTE_CLBCK_FN(CLBCK_FN_BC_LDATA_CONF, unPGN, m_byNodeAddress,
                         byDestAddress, TRUE);
      } else {
        vSendFrame((UCHAR)psMsg->m_unDLC, psMsg->m_pbyData,
                   psMsg->m_sMsgProperties.m_uExtendedID.m_unExtID,
                   psMsg->m_sMsgProperties.m_byChannel);
        EXECUTE_CLBCK_FN(CLBCK_FN_BC_LDATA_CONF, unPGN, m_byNodeAddress,
                         byDestAddress, TRUE);
      }
    ...
    case MSG_TYPE_DATA: {
      BYTE byDestAddress = psMsg->m_sMsgProperties.m_uExtendedID.m_s29BitId
                               .m_uPGN.m_sPGN.m_byPDU_Specific;
      BYTE byPriorirty = psMsg->m_sMsgProperties.m_uExtendedID.m_s29BitId.m_uPGN
                             .m_sPGN.m_byPriority;
      UINT unPGN = psMsg->m_sMsgProperties.m_uExtendedID.m_s29BitId.unGetPGN();
      if (psMsg->m_unDLC > MAX_FRAME_DATA_SIZE) {
        HRESULT hResult =
            TransmitRequestToSend(byDestAddress, byPriorirty, psMsg->m_unDLC,
                                  unPGN, psMsg->m_sMsgProperties.m_byChannel);
        ...
        if ((hResult == DATA_CLEAR_2_SEND) || (hResult == DATA_DELAY_2_SEND)) {
          ...
          if (SendLongMsg(psMsg, m_pConDet, hResult) == DATA_EOM) {
            ...
            WriteIntoClientsBuffer(*psMsg);
            EXECUTE_CLBCK_FN(CLBCK_FN_LDATA_CONF, unPGN, m_byNodeAddress,
                             byDestAddress, TRUE);
          } else {
            EXECUTE_CLBCK_FN(CLBCK_FN_LDATA_CONF, unPGN, m_byNodeAddress,
                             byDestAddress, FALSE);
...
Modifications  :
******************************************************************************/
HRESULT CNodeConManager::SendLongMsg(STJ1939_MSG* psMsg,
                                     CConnectionDet* pConDet,
                                     HRESULT hPreviousResult) {
  ...
  if (hPreviousResult == DATA_CLEAR_2_SEND) {
    ...
    // Send all the frame except the last frame,(nCurrFrameNo < (nLastFrameNo -
    // 1))
    UINT32 unExtId =
        Prepare_P2P_Id(PDU_FORMAT_TPDT, m_byNodeAddress,
                       psMsg->m_sMsgProperties.m_uExtendedID.m_s29BitId.m_uPGN
                           .m_sPGN.m_byPDU_Specific,
                       psMsg->m_sMsgProperties.m_uExtendedID.m_s29BitId.m_uPGN
                           .m_sPGN.m_byPriority);
    hResult =
        SendPackets(unExtId, psMsg->m_pbyData, nLastFrameNo, nLastFrameLen,
                    pConDet, psMsg->m_sMsgProperties.m_byChannel);
  }
  return hResult;
}
...
Input(s)       :
Output         :
Functionality  :  Sends the required number of packets and waits for
acknowledgement Member of      :  CNodeConManager Friend of      :  - Author(s)
:  Pradeep Kadoor Date Created   :  23/11/2010 Modifications  :
******************************************************************************/
HRESULT CNodeConManager::SendPackets(UINT32 unExtId, BYTE* pbyData,
                                     UINT unActualPackets, UINT unLastFrameLen,
                                     CConnectionDet* pConDet, UINT unChannel) {
  HRESULT hResult = S_FALSE;

  UINT unPacksToBeSent = min(pConDet->m_byMaxPacketWOC2S,
                             (unActualPackets - pConDet->m_unNextPacket));
  // Just for the safety
  ...
  hResult = nIsMsgSentSuccess(unExtId, pbyData, unActualPackets, unLastFrameLen,
                              pConDet, unChannel);
  return hResult;
}
...
HRESULT CNodeConManager::DelaySendProcess(CConnectionDet* pConDet) {
  ...
  dwWaitRet =
      WaitForSingleObject(pConDet->m_hDataDelayWait, CNetworkMgmt::sg_unTO_T4);
...
Modifications  :
******************************************************************************/
INT CNodeConManager::nIsMsgSentSuccess(UINT32 unExtId, BYTE* pbyData,
                                       UINT unActualPackets,
                                       UINT unLastFrameLen,
                                       CConnectionDet* pConDet,
                                       UINT unChannel) {
    ...
    if (pConDet->m_byResult == DATA_CLEAR_2_SEND) {
      if (pConDet->m_byTxAckSeqNo == (pConDet->m_unNextPacket % MAX_SEQ_NO)) {
        if ((pConDet->m_unNextPacket + 1) == unActualPackets)  // EOM
        {
          ...
          nDataAck = SendPackets(unExtId, pbyData, unActualPackets,
                                 unLastFrameLen, pConDet, unChannel);
        }
      } else {
...
void CNodeConManager::vSendMsgWithoutAckReq(UCHAR ucFrameLen, BYTE* pFrameData,
  ...
  // Allow other threads to perform wait for delay time
  Sleep(0);
...
Modifications  :
******************************************************************************/
void CNodeConManager::vSendConAckMsg(BYTE byDestAdres, BYTE byTotalPackets,
                                     UINT32 unPGN, UINT unChannel) {
  // Create CA message
  BYTE byCANMsg[MAX_FRAME_DATA_SIZE] = {0xFF};
  PrepareClear_2_Send(byCANMsg, byTotalPackets, 1, unPGN);
  UINT32 unID = Prepare_P2P_Id(PDU_FORMAT_TPCM, m_byNodeAddress, byDestAdres,
                               DEFAULT_PRIORITY);
  // Send clear to send
...
HRESULT CNodeConManager::WaitAfterClear2Send(void) {
  ...
  DWORD dwWaitRet = WaitForSingleObject(m_pConDet->m_hClear2SendWait,
                                        CNetworkMgmt::sg_unTO_T2);
  if (dwWaitRet == WAIT_OBJECT_0) {
...
HRESULT CNodeConManager::WaitForMorePackets(void) {
  ...
  DWORD dwWaitRet =
      WaitForSingleObject(m_pConDet->m_hDataRxWait, CNetworkMgmt::sg_unTO_T1);
  if (dwWaitRet == WAIT_OBJECT_0) {
...
HRESULT CNodeConManager::WaitandProcessBroadcastData(void) {
  HRESULT hResult = S_FALSE;
  DWORD dwWaitRet =
      WaitForSingleObject(m_pConDet->m_hBCDataRXWait, CNetworkMgmt::sg_unTO_T1);
  if (dwWaitRet == WAIT_OBJECT_0) {
...
Modifications  :
******************************************************************************/
void CNodeConManager::vSendFrame(UCHAR ucFrameLen, BYTE* pFrameData, UINT unID,
                                 UINT unChannel) {
  CTransferLayer::ouGetTransLayerObj().vTransmitCANMsg(
      m_dwClientID, unID, ucFrameLen, pFrameData, unChannel);
}
...
Modifications  :
******************************************************************************/
void CNodeConManager::vFormJ1939MsgForSending(UINT unChannel, STJ1939_MSG& sMsg,
                                              EJ1939_MSG_TYPE eMsgType,
                                              UINT32 unPGN, BYTE* pbyData,
                                              UINT unDLC, BYTE byPriority,
                                              BYTE byDestAddress) {
  ...
  switch (eMsgType) {
    case MSG_TYPE_COMMAND:  // Normal Data
    {
      ...
      sMsg.m_sMsgProperties.m_uExtendedID.m_unExtID = Prepare_P2P_Id(
          (BYTE)(unPGN >> 8), m_byNodeAddress, byDestAddress, byPriority);
      sMsg.m_unDLC = unDLC;
    ...
    case MSG_TYPE_REQUEST: {
      sMsg.m_sMsgProperties.m_eType = MSG_TYPE_DATA;
      sMsg.m_sMsgProperties.m_uExtendedID.m_unExtID = Prepare_P2P_Id(
          PDU_FORMAT_REQUEST4, m_byNodeAddress, byDestAddress, byPriority);
    ...
    case MSG_TYPE_ACKNOWLEDGEMENT: {
      sMsg.m_sMsgProperties.m_eType = MSG_TYPE_DATA;
      sMsg.m_sMsgProperties.m_uExtendedID.m_unExtID = Prepare_P2P_Id(
          PDU_FORMAT_ACK, m_byNodeAddress, byDestAddress, byPriority);
      sMsg.m_unDLC = MAX_FRAME_DATA_SIZE;
      memcpy(sMsg.m_pbyData, pbyData,
             MAX_FRAME_DATA_SIZE);  // ACKNOWLEDGEMENT_TYPE
    } break;
    case MSG_TYPE_GROUP_FUNCTIONS: {
      // TBD
    } break;
    case MSG_TYPE_NM_ACL: {
      ...
      sMsg.m_sMsgProperties.m_uExtendedID.m_unExtID = Prepare_P2P_Id(
          PDU_FORMAT_ACL, m_byNodeAddress, byDestAddress, byPriority);
      sMsg.m_unDLC = MAX_FRAME_DATA_SIZE;
      memcpy(sMsg.m_pbyData, (BYTE*)&(m_u64ECUName.m_un64ECU_NAME),
             sMsg.m_unDLC);
      /* update network management entry */
      CNetworkMgmt::ouGetNWManagementObj().vUpdateAddressMap(
          m_u64ECUName.m_un64ECU_NAME, (BYTE)m_byNodeAddress);
    } break;
...
void CNodeConManager::vSendMessage(UINT unChannel, EJ1939_MSG_TYPE eMsgType,
  ...
  vFormJ1939MsgForSending(unChannel, m_sTxMsg, eMsgType, unPGN, pbyData, unDLC,
                          byPriority, byDestAdres);
  static BYTE abyData[MAX_MSG_LEN_J1939] = {0xFF};
  m_sTxMsg.vGetDataStream(abyData);
  UINT unSize = m_sTxMsg.unGetSize();
  m_ouMsgBufVSE.WriteIntoBuffer(ETYPE_BUS::J1939, abyData, unSize);
...
Modifications  :
******************************************************************************/
void CNodeConManager::vSendBAM(UINT unMsgSize, UINT32 unPGN, BYTE byDestAddress,
  ...
  UINT unExtId = Prepare_P2P_Id(PDU_FORMAT_TPCM, m_byNodeAddress, byDestAddress,
                                byPriority);
  /* Data preparation */
...
Modifications  :
******************************************************************************/
void CNodeConManager::vSendBroadCastData(BYTE* pbyData, UINT unMsgSize,
                                         BYTE byDestAddress, BYTE byPriority,
                                         UINT unChannel) {
  ...
  UINT unDataIndex = 0;
  // Send all the frame except the last frame,(nCurrFrameNo < (nLastFrameNo -
  // 1)) UCHAR ucVS_SendSeqStateVar = 0;
  UINT32 unExtId;
  unExtId = Prepare_P2P_Id(PDU_FORMAT_TPDT, m_byNodeAddress, byDestAddress,
                           byPriority);
  for (int nCurrFrameNo = 0; nCurrFrameNo < nLastFrameNo; nCurrFrameNo++) {
    if (nCurrFrameNo == nLastFrameNo - 1)  // last frame
    {
      ...
      vSendMsgWithoutAckReq(MAX_FRAME_DATA_SIZE, TxFrame, unExtId, unChannel);
    } else {
      TxFrame[0] = (BYTE)(nCurrFrameNo + 1);  // W4 Removal
      memcpy(&TxFrame[1], pbyData + unDataIndex, MAX_TPDU_DATA_SIZE);
      vSendMsgWithoutAckReq(MAX_FRAME_DATA_SIZE, TxFrame, unExtId, unChannel);
      unDataIndex +=
          MAX_TPDU_DATA_SIZE;  // Max byte data of J1939 in one CAN frame
    }
  }
}
...
CConnectionDet* CNodeConManager::pCreateAndGetConDet(BYTE byDestAdres) {
  if (m_pConDet == nullptr) {
    m_pConDet = m_pNodeConDetArr[0] =
        new CConnectionDet(m_byNodeAddress, byDestAdres);
    m_byConCount = 1;
  } else {
    vReInitConDet();
    m_pConDet->m_bySrcAddress = m_byNodeAddress;
...
HRESULT CNodeConManager::TransmitRequestToSend(BYTE byDestAddress,
                                               BYTE byPriority, UINT unDLC,
                                               UINT32 unPGN, UINT unChannel) {
  HRESULT hResult = S_FALSE;
  pCreateAndGetConDet(byDestAddress);
  if ((m_pConDet != nullptr) &&
      (m_pConDet->eGetConStatus() == T_DISCONNECTED)) {
    m_pConDet->m_unPGN = unPGN;
    ...
    DWORD dwWaitRet = WaitForSingleObject(
        m_pConDet->m_hDataAckWait,
        CNetworkMgmt::sg_unTO_RESPONSE);  // pConDet->m_cTAC_AckConnectionTO);
    if (dwWaitRet == WAIT_OBJECT_0) {
...
HRESULT CNodeConManager::StartAdresClaimProc(BYTE byAddress) {
  ...
  if (CNetworkMgmt::ouGetNWManagementObj()
    ...
    for (UINT i = 0; i < (UINT)lParam; i++) {
      vSendMessage(i + 1, MSG_TYPE_NM_ACL, m_byNodeAddress,
                   (BYTE*)&m_u64ECUName.m_un64ECU_NAME, MAX_FRAME_DATA_SIZE,
                   DEFAULT_PRIORITY - 1, ADDRESS_ALL);
    }
    hResult = S_OK;
  }
  return hResult;
...
Modifications  :
******************************************************************************/
BOOL CNodeConManager::bIsMonitorNode(void) { return m_bIsMonNode; }
