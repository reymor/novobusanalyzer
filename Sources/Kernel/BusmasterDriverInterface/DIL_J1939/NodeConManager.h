...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_DIL_J1939_NODECONMANAGER_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_DIL_J1939_NODECONMANAGER_H_
...
class CNodeConManager {
 ...
 protected:
  ...
  CMsgBufFSE<STCANDATA> m_ouCANBuff;
  BYTE m_byConCount;
  BOOL m_bIsActive;
  HANDLE m_hTimer;

 private:
  void vTransmitLongData(short sDataLength, BYTE* pbData,
                         CConnectionDet* pConDet);
  INT nIsMsgSentSuccess(UINT32 unExtId, BYTE* pbyData, UINT unNoOfPackets,
                        UINT unLastFrameLen, CConnectionDet* pConDet,
                        UINT unChannel);
  void vSendMsgWithoutAckReq(UCHAR ucFrameLen, BYTE* pFrameData, UINT unID,
                             UINT unChannel);
  HRESULT SendPackets(UINT32 unExtId, BYTE* pbyData, UINT unActualPackets,
                      UINT nLastFrameLen, CConnectionDet* pConDet,
                      UINT unChannel);
 ...
 protected:
  void vSendFrame(UCHAR ucFrameLen, BYTE* pFrameData, UINT unID,
                  UINT unChannel);
 ...
 public:
  ...
  HRESULT TransmitRequestToSend(BYTE byDestAddress, BYTE byPriority, UINT unDLC,
  ...
  void vSendBroadCastData(BYTE* pbyData, UINT unMsgSize, BYTE byDestAddress,
                          BYTE byPriority, UINT unChannel);
  HRESULT SendLongMsg(STJ1939_MSG* psMsg, CConnectionDet* pConDet,
 ...
 public:
  ...
  void vSendConAckMsg(BYTE byDestAdres, BYTE byTotalPackets, UINT32 unPGN,
                      UINT unChannel);
  ...
  void vExecuteClbckFuncPtrs(ETYPE_CLBCK_FN eClbckType, UINT32 unPGN,
                             BYTE bySrc, BYTE byDest, BOOL bSuccess);
};

#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_DIL_J1939_NODECONMANAGER_H_