...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_DIL_J1939_J1939_UTILITYFUNCS_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_DIL_J1939_J1939_UTILITYFUNCS_H_
...
typedef enum { T_DISCONNECTED = 0, T_CONNECTED, T_STARTUP } eCON_STATUS;

/* TYPES OF CONNECTION MODE */
typedef enum { CM_STANDARD = 0, CM_BROADCAST } eCON_MODE;

/* TYPES OF REASON FOR DISCONNECTION */
typedef enum { T_FAULT = 0, T_SHUTDOWN } eREASON;
...
static void PrepareEOM_ACK(BYTE* byCANData, UINT unDLC, BYTE byNoOfPackets,
                           UINT32 unPGN) {
...
static void PrepareClear_2_Send(BYTE* byCANData, BYTE byNoOfPackets,
                                BYTE byNextPacket, UINT32 unPGN) {
...
static void vPrepareData(BYTE* byFrameData, BYTE byControlByte, UINT unDLC,
                         UINT32 unPGN) {
...
static BOOL bIsConAckMsg(STCAN_MSG& sCanMsg) {
  ...
  return ((uExtId.m_s29BitId.m_uPGN.m_sPGN.m_byPDU_Format == PDU_FORMAT_TPCM) &&
          (sCanMsg.m_ucData[0] == CB_CLEAR_TO_SEND) &&
          (0 == sCanMsg.m_ucData[2]));
}
...
static void vCreateTempJ1939Msg(STJ1939_MSG& sMsg, const STCANDATA& sCanData,
  ...
  sMsg.m_sMsgProperties.m_eDirection =
      (sCanData.m_ucDataType == RX_FLAG) ? DIR_RX : DIR_TX;
...
#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_DIL_J1939_J1939_UTILITYFUNCS_H_