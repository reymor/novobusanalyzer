...
CMonitorNode::CMonitorNode(int nNodeNo, char* pacNodeName, UINT64 un64ECUName,
                           BYTE byPrefAdres)
    : CNodeConManager(nNodeNo, pacNodeName, un64ECUName, byPrefAdres, TRUE) {
  for (int i = 0; i < MAX_NODE_TO_MONITOR; i++) {
    m_pMonNodeConDetArr[i] = nullptr;
  }
}

CMonitorNode::~CMonitorNode(void) { vRemoveAllConnections(); }
  ...
  switch (psMsg->m_sMsgProperties.m_eType) {
    case MSG_TYPE_BROADCAST: {
      ...
      if (psMsg->m_unDLC > MAX_FRAME_DATA_SIZE) {
        ...
        EXECUTE_CLBCK_FN(CLBCK_FN_BC_LDATA_CONF, unPGN, byGetNodeAddress(),
                         byDestAddress, TRUE);
      } else {
      ...
      if (psMsg->m_unDLC > MAX_FRAME_DATA_SIZE) {
        ...
        if ((hResult == DATA_CLEAR_2_SEND) || (hResult == DATA_DELAY_2_SEND)) {
          ...
          if (SendLongMsg(psMsg, m_pConDet, hResult) == DATA_EOM) {
            ...
            EXECUTE_CLBCK_FN(CLBCK_FN_LDATA_CONF, unPGN, byGetNodeAddress(),
                             byDestAddress, TRUE);
          } else {
            EXECUTE_CLBCK_FN(CLBCK_FN_LDATA_CONF, unPGN, byGetNodeAddress(),
                             byDestAddress, FALSE);
...
void CMonitorNode::vProcessLongDataByMonNode(const STCANDATA& CurrMsgCAN,
  ...
  if (byLastReceived == (sCanMsg.m_ucData[0] - 1)) {
    ...
    pConDet->m_byCurrPacket =
        (pConDet->m_byCurrPacket + 1) % pConDet->m_byMaxPacketWOC2S;
  ...
  if (bIsEOM_ACK(sCanMsg.m_unMsgID, sCanMsg.m_ucData[0])) {
    ...
    if (pConDet != nullptr) {
      if (pConDet->eGetConStatus() == T_DISCONNECTED) {
        ...
        pConDet->m_unRxLastFrameLen =
            (UINT)byGetLastFrameLen(pConDet->m_unRXLongDataLen);
                    ...
                    sCanMsg.m_ucData[0])) /* BROADCAST Announce*/
  {
    ...
    if (pConDet != nullptr) {
      if (pConDet->eGetConStatus() == T_DISCONNECTED) {
        ...
        pConDet->m_BCLastFrameLen =
            (UINT)byGetLastFrameLen(pConDet->m_BCRXLongDataLen);
