...
void CNetworkMgmt::vDoInit(void) { m_ouReadCANMsg.vDoInit(); }

void CNetworkMgmt::vDoExit(void) { m_ouReadCANMsg.vDoExit(); }
...
HRESULT CNetworkMgmt::GetTimeModeMapping(SYSTEMTIME& CurrSysTime,
                                         UINT64& unAbsTime) {
...
void CNetworkMgmt::vGetTimeOut(ETYPE_TIMEOUT eTimeOutType,
                               UINT& unMiliSeconds) {
  switch (eTimeOutType) {
...
void CNetworkMgmt::vConfigureTimeOut(ETYPE_TIMEOUT eTimeOutType,
                                     UINT unMiliSeconds) {
  switch (eTimeOutType) {
    case TYPE_TO_BROADCAST: {
...
BOOL CNetworkMgmt::bIsOnline(void) { return m_bOnline; }
...
/**************************************************************
 ************************************************************** */
void CNetworkMgmt::vMDisconnectInd(short /*sLocalLc*/, short /*sRemoteLc*/,
                                   eREASON /*eReason*/) {}
/**************************************************************
 ************************************************************** */
...
void CNetworkMgmt::vMErrorInd(short /*sLocalLc*/, short /*sRemoteLc*/,
                              char /*cError*/) {
...
/**************************************************************
************************************************************** */
LONG CNetworkMgmt::lCreateNodeConManager(char* pacNodeName, UINT64 un64ECUName,
  ...
  if (lResult == S_OK) {
    ...
    if (_tcscmp(pacNodeName, J1939_MONITOR_NODE) == 0) {
      pNodeConMgr =
          new CMonitorNode(nEmptyPos, pacNodeName, un64ECUName, byPrefAdres);
    } else {
      // Provide its position in array and its PGN
      pNodeConMgr =
          new CNodeConManager(nEmptyPos, pacNodeName, un64ECUName, byPrefAdres);
    }
    if (pNodeConMgr != nullptr) {
      ...
      if (_tcscmp(pacNodeName, J1939_MONITOR_NODE) == 0) {
        ...
        hResult =
            m_pIDIL_CAN->DILC_RegisterClient(TRUE, dwClientId, pacNodeName);
      }
      // ASSERT(hResult == S_OK);
      if ((hResult == S_OK) || (hResult == ERR_CLIENT_EXISTS)) {
        CBaseCANBufFSE* pouBuffer = pNodeConMgr->pouGetBuf();
        hResult =
            m_pIDIL_CAN->DILC_ManageMsgBuf(MSGBUF_ADD, dwClientId, pouBuffer);
        ASSERT(hResult == S_OK);
        if (hResult == S_OK) {
          pNodeConMgr->m_dwClientID = dwClientId;
          m_ouReadCANMsg.AddEventHandle(pouBuffer->hGetNotifyingEvent(),
                                        (BYTE)nEmptyPos);
        }
        ...
        if (m_bOnline == TRUE) {
          ...
          if ((byPrefAddress >= ADDRESS_NULL) ||
              (bIsAddressClaimed(byPrefAddress) == TRUE)) {
  ...
  for (int i = 0; i < DEF_MAX_SIMULATED_NODE; i++) {
    if (nullptr != m_ConMgrArr[i]) {
      if (dwClientId == m_ConMgrArr[i]->m_dwClientID) {
        ...
        if (m_ConMgrArr[i]->m_dwClientID !=
            m_dwCANMonitorNodeClientId)  // Do not remove client from CAN if
                                         // monitor
        {
          m_pIDIL_CAN->DILC_RegisterClient(FALSE, m_ConMgrArr[i]->m_dwClientID,
                                           omClientName.GetBuffer(0));
        } else  // Do not remove client from CAN if monitor, remove only the
                // buffer
        {
          CBaseCANBufFSE* pouBuffer = m_ConMgrArr[i]->pouGetBuf();
          m_pIDIL_CAN->DILC_ManageMsgBuf(
              MSGBUF_CLEAR, m_ConMgrArr[i]->m_dwClientID, pouBuffer);
        }
...
CBaseDIL_CAN* CNetworkMgmt::GetICANDIL(void) { return m_pIDIL_CAN; }
  ...
  for (int i = 0; i < DEF_MAX_SIMULATED_NODE; i++) {
    if (nullptr != m_ConMgrArr[i]) {
      ...
      if (m_ConMgrArr[i]->m_dwClientID != m_dwCANMonitorNodeClientId) {
        m_pIDIL_CAN->DILC_RegisterClient(FALSE, m_ConMgrArr[i]->m_dwClientID,
                                         omClientName.GetBuffer());
      } else {
        // Do not remove client from CAN if monitor, remove only the buffer
        CBaseCANBufFSE* pouBuffer = m_ConMgrArr[i]->pouGetBuf();
        m_pIDIL_CAN->DILC_ManageMsgBuf(MSGBUF_CLEAR,
                                       m_ConMgrArr[i]->m_dwClientID, pouBuffer);
      }
