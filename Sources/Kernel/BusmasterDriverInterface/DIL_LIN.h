...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_DIL_LIN_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_DIL_LIN_H_
...
class CDIL_LIN : public CBaseDIL_LIN {
 public:
  ...
  /* member variable to hold the pointer of currently selected controller
   * interface */
   ...
   * existing license will be returned. If List is nullptr, only number is
   * returned.
   */
   ...
   * intended one isn't allowed with the present license, this returns
   * NO_LICENSE. A list for DALs (or bDriverID) is shown below: DRIVER_STUB :
   * DAL for Stub DRIVER_ES520                : DAL for ES520 DRIVER_TZMCYCLONE
   * : DAL for TZMCyclone DRIVER_LIN_USB              : DAL for LIN usb
  ...
  HRESULT DILL_RegisterClient(BOOL bRegister, DWORD& ClientID,
                              char* pacClientName);
   ...
   */
  HRESULT DILL_ManageMsgBuf(BYTE bAction, DWORD ClientID,
   ...
   * it
   */
  HRESULT DILL_GetTimeModeMapping(SYSTEMTIME& CurrSysTime, UINT64& TimeStamp,
   ...
   * user's choice.
   */
  HRESULT DILL_ListHwInterfaces(INTERFACE_HW_LIST& sSelHwInterface,
                                INT& nCount);

  /**
   * Call this function to select hardware interfaces.
   */
  HRESULT DILL_SelectHwInterfaces(const INTERFACE_HW_LIST& sSelHwInterface,
                                  INT nCount);
   ...
   * release the memory.
   */
  HRESULT DILL_SetConfigData(IBMNetWorkGetService* ouConfig);
   ...
   */
  HRESULT DILL_GetLastErrorString(std::string& acErrorStr);
   ...
   */
  HRESULT DILL_GetControllerParams(LONG& lParam, UINT nChannel,
                                   ECONTR_PARAM eContrParam);

  HRESULT DILL_GetConfiguration(SCONTROLLER_DETAILS_LIN[], INT& nSize);
   ...
   */
  HRESULT DILL_GetErrorCount(SERROR_CNT& sErrorCnt, UINT nChannel,
                             ECONTR_PARAM eContrParam);
  ...
  // Schedule table
  HRESULT DILL_RegisterLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                        CSheduleTable ouTable, int& nHandle);
  HRESULT DIIL_DeRegisterLinScheduleTabel(DWORD& dwClientId, int& nChannel,
                                          int& nTableHandle);
  HRESULT DILL_StartLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                     int& nTableHandle);
  HRESULT DILL_UpdateLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                      int& nHandle, CSheduleTable& ouTable);

  HRESULT DIIL_EnableLinScheuleCommand(DWORD& dwClientId, int& nChannel,
                                       int nTableHandle, int nIndex,
                                       bool bEnable);

  // Individual Header commands.
  HRESULT DIIL_RegisterLinHeader(DWORD& dwClientId, int& nChannel, int nId,
                                 int nCycleTimer);
 ...
 private:
  ...
  HRESULT(*m_pfGetTimeModeMapping)
  (SYSTEMTIME& CurrSysTime, UINT64& TimeStamp, LARGE_INTEGER& QueryTickCount);
  HRESULT(*m_pfListHwInterfaces)
  (INTERFACE_HW_LIST& asSelHwInterface, INT& nCount);
  HRESULT(*m_pfSelectHwInterface)
  (const INTERFACE_HW_LIST& asSelHwInterface, INT nCount);
  HRESULT (*m_pfDeselectHwInterfaces)(void);
  HRESULT(*m_pfDisplayConfigDlg)
  (SCONTROLLER_DETAILS_LIN* InitData, int& Length);
  ...
  HRESULT(*m_pfGetControllerParams)
  (LONG& lParam, UINT nChannel, ECONTR_PARAM eContrParam);
  HRESULT(*m_pfGetErrorCount)
  (SERROR_CNT& sErrorCnt, UINT nChannel, ECONTR_PARAM eContrParam);
};

#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_DIL_LIN_H_