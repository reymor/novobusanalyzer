...
#define VALIDATE_LIN_POINTER(Ptr) \
  if (Ptr == nullptr) {           \
    return S_FALSE;               \
  }
...
int CDIL_LIN::ExitInstance() { return 0; }
/* ROUTER CODE STARTS */
 ...
 * layers supported or available. If 'bAvailable' is true, this returns number
 * of the DILs implemented; else the list of the DILs supported by the existing
...
HRESULT CDIL_LIN::DILL_SelectDriver(DWORD dwDriverID, HWND hWndOwner) {
  ...
  if (DILL_GetSelectedDriver() == dwDriverID) {
    ...
    if (m_hDll == nullptr) {
      ...
      pfGetIDILLIN_Controller = (GETIDIL_LIN_CONTROLLER)GetProcAddress(
          m_hDll, "GetIDIL_LIN_Controller");
      pfGetIDILLIN_Controller((void**)&m_pBaseDILLIN_Controller);
      if (m_pBaseDILLIN_Controller) {
        // set the application parameters.
        m_pBaseDILLIN_Controller->LIN_SetAppParams(hWndOwner);
 ...
 */
HRESULT CDIL_LIN::DILL_ManageMsgBuf(BYTE bAction, DWORD ClientID,
                                    CBaseLINBufFSE* pBufObj) {
 ...
 */
HRESULT CDIL_LIN::DILL_RegisterClient(BOOL bRegister, DWORD& ClientID,
                                      char* pacClientName) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_RegisterClient(bRegister, ClientID,
                                                      pacClientName);
}
 ...
 */
DWORD CDIL_LIN::DILL_GetSelectedDriver(void) { return m_dwDriverID; }
 ...
 * \req       RS_23_10 - Getter for the time mode mapping (usually the 64-bit
 * time stamp by the driver)
 *
 * Call this function to get a system time and the time stamp associated with it
 */
HRESULT CDIL_LIN::DILL_GetTimeModeMapping(SYSTEMTIME& CurrSysTime,
                                          UINT64& TimeStamp,
                                          LARGE_INTEGER& QueryTickCount) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_GetTimeModeMapping(
      CurrSysTime, TimeStamp, QueryTickCount);
}
 ...
 * user's choice.
 */
HRESULT CDIL_LIN::DILL_ListHwInterfaces(INTERFACE_HW_LIST& sSelHwInterface,
                                        INT& nCount) {
  ...
  HRESULT hr =
      m_pBaseDILLIN_Controller->LIN_ListHwInterfaces(sSelHwInterface, nCount);
 ...
 * \req       RS_23_12 - Selection of a controller from the hardware interface
 * list
 *
 * Call this function to select hardware interfaces.
 */
HRESULT CDIL_LIN::DILL_SelectHwInterfaces(
    const INTERFACE_HW_LIST& sSelHwInterface, INT nCount) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_SelectHwInterface(sSelHwInterface,
                                                         nCount);
}
 ...
 * \req       RS_23_15 - Setting of the configuration data for the present
 * controller
 *
 * To set the configuration data for the currently selected DIL. Caller must
 * release the memory.
 */
HRESULT CDIL_LIN::DILL_SetConfigData(IBMNetWorkGetService* ouConfig) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
...
HRESULT CDIL_LIN::DILL_DisableSlaveRespData(DWORD dwClientID,
                                            STLIN_MSG& sMessage) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_DisableSlaveRespData(dwClientID,
                                                            sMessage);
}
 ...
 * \req       RS_23_21 - In case of any error, a function returns the associated
 ...
 */
HRESULT CDIL_LIN::DILL_GetCntrlStatus(const HANDLE& hEvent,
                                      UINT& unCntrlStatus) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
 ...
 */
HRESULT CDIL_LIN::DILL_GetControllerParams(LONG& lParam, UINT nChannel,
                                           ECONTR_PARAM eContrParam) {
  if (m_pBaseDILLIN_Controller) {
    return m_pBaseDILLIN_Controller->LIN_GetControllerParams(lParam, nChannel,
                                                             eContrParam);
  } else {
    return S_FALSE;
  }
...
HRESULT CDIL_LIN::DILL_GetConfiguration(
    SCONTROLLER_DETAILS_LIN psControllerDeConfig[], INT& nSize) {
  if (m_pBaseDILLIN_Controller) {
    return m_pBaseDILLIN_Controller->LIN_GetConfiguration(psControllerDeConfig,
                                                          nSize);
  } else {
    return S_FALSE;
  }
 ...
 */
HRESULT CDIL_LIN::DILL_SetControllerParams(int nValue,
                                           ECONTR_PARAM eContrparam) {
  if (m_pBaseDILLIN_Controller) {
    return m_pBaseDILLIN_Controller->LIN_SetControllerParams(nValue,
                                                             eContrparam);
  } else {
    return S_FALSE;
  }
 ...
 */
HRESULT CDIL_LIN::DILL_GetErrorCount(SERROR_CNT& sErrorCnt, UINT nChannel,
                                     ECONTR_PARAM eContrParam) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_GetErrorCount(sErrorCnt, nChannel,
                                                     eContrParam);
}

// Schedule table
HRESULT CDIL_LIN::DILL_RegisterLinScheduleTable(DWORD& dwClientId,
                                                int& nChannel,
                                                CSheduleTable ouTable,
                                                int& nHandle) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_RegisterLinScheduleTable(
      dwClientId, nChannel, ouTable, nHandle);
}

HRESULT CDIL_LIN::DIIL_DeRegisterLinScheduleTabel(DWORD& dwClientId,
                                                  int& nChannel,
                                                  int& nTableHandle) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_DeRegisterLinScheduleTabel(
      dwClientId, nChannel, nTableHandle);
}

HRESULT CDIL_LIN::DILL_StartLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                             int& nTableHandle) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_StartLinScheduleTable(
      dwClientId, nChannel, nTableHandle);
}

HRESULT CDIL_LIN::DILL_UpdateLinScheduleTable(DWORD& dwClientId, int& nChannel,
                                              int& nHandle,
                                              CSheduleTable& ouTable) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_UpdateLinScheduleTable(
      dwClientId, nChannel, nHandle, ouTable);
}

HRESULT CDIL_LIN::DIIL_EnableLinScheuleCommand(DWORD& dwClientId, int& nChannel,
                                               int nTableHandle, int nIndex,
                                               bool bEnable) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_EnableLinScheuleCommand(
      dwClientId, nChannel, nTableHandle, nIndex, bEnable);
}

// Individual Header commands.
HRESULT CDIL_LIN::DIIL_RegisterLinHeader(DWORD& dwClientId, int& nChannel,
                                         int nId, int nCycleTimer) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_RegisterLinHeader(dwClientId, nChannel,
                                                         nId, nCycleTimer);
}

HRESULT CDIL_LIN::DIIL_DeRegisterLinHeader(DWORD& dwClientId, int& nChannel,
                                           int nId) {
  VALIDATE_LIN_POINTER(m_pBaseDILLIN_Controller);
  return m_pBaseDILLIN_Controller->LIN_DeRegisterLinHeader(dwClientId, nChannel,
                                                           nId);
}