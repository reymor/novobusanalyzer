...
#ifndef KERNEL_BUSMASTERDRIVERINTERFACE_DIL_CAN_H_
#define KERNEL_BUSMASTERDRIVERINTERFACE_DIL_CAN_H_
...
class CDIL_CAN : public CBaseDIL_CAN {
 public:
  ...
  HRESULT DILC_RegisterClient(BOOL bRegister, DWORD& ClientID,
                              char* pacClientName);
   ...
   */
  HRESULT DILC_ManageMsgBuf(BYTE bAction, DWORD ClientID,
   ...
   * it
   */
  HRESULT DILC_GetTimeModeMapping(SYSTEMTIME& CurrSysTime, UINT64& TimeStamp,
   ...
   */
  HRESULT DILC_GetControllerParams(LONG& lParam, UINT nChannel,
                                   ECONTR_PARAM eContrParam);

  virtual HRESULT DILC_SetControllerParams(int nValue,
                                           ECONTR_PARAM eContrparam);
  /**
   * Call to Get Error Counts
   */
  HRESULT DILC_GetErrorCount(SERROR_CNT& sErrorCnt, UINT nChannel,
                             ECONTR_PARAM eContrParam);

  HRESULT DILC_SetHardwareChannel(
      PSCONTROLLER_DETAILS m_asControllerDetails, DWORD dwDriverId,
      bool bHardwareListed = true,
      unsigned int unChannelCount = defCHANNEL_CAN_MAX);
...
#endif  // KERNEL_BUSMASTERDRIVERINTERFACE_DIL_CAN_H_