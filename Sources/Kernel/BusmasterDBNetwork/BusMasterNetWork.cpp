...
ERRORCODE DBChangeManger::ManageClient(IDbChangeListner* newListner,
                                       bool bAdd) {
...
ERRORCODE DBChangeManger::UpdateChange(bool isAdded,
                                       IDbChangeListner::DBChangeInfo dbInfo) {
...
ERRORCODE BMNetwork::GetChannelSettings(ETYPE_BUS eouProtocol,
                                        int nChannelIndex,
                                        ChannelSettings* ouChannelSettings) {
  return m_ouProtocolConfig[eouProtocol].GetChannelSettings(nChannelIndex,
                                                            ouChannelSettings);
}

ERRORCODE BMNetwork::SetChannelSettings(ETYPE_BUS eouProtocol,
                                        int nChannelIndex,
                                        ChannelSettings* ouChannelSettings) {
  return m_ouProtocolConfig[eouProtocol].SetChannelSettings(nChannelIndex,
                                                            ouChannelSettings);
}

ERRORCODE BMNetwork::GetChannelCount(ETYPE_BUS eouProtocol,
                                     int& nChannelsConfigured) {
  return m_ouProtocolConfig[eouProtocol].GetChannelCount(nChannelsConfigured);
}

ERRORCODE BMNetwork::GetDBServiceCount(ETYPE_BUS eouProtocol, int nChannelIndex,
                                       int& nCount) {
  return m_ouProtocolConfig[eouProtocol].GetDBCount(nChannelIndex, nCount);
}

ERRORCODE BMNetwork::GetDBService(ETYPE_BUS eouProtocol, int nChannelIndex,
                                  int nDBIndex, ICluster** ouCluster) {
  *ouCluster = nullptr;
  return m_ouProtocolConfig[eouProtocol].GetDBService(nChannelIndex, nDBIndex,
                                                      ouCluster);
}

ERRORCODE BMNetwork::GetFrameList(ETYPE_BUS eouProtocol, int nChannelIndex,
                                  std::list<IFrame*>& frames) {
  std::list<ICluster*> ouClusterList;
  m_ouProtocolConfig[eouProtocol].GetDBServiceList(nChannelIndex,
                                                   ouClusterList);
...
ERRORCODE BMNetwork::GetDBServiceList(ETYPE_BUS eouProtocol, int nChannelIndex,
                                      std::list<ICluster*>& ouClusterList) {
  return m_ouProtocolConfig[eouProtocol].GetDBServiceList(nChannelIndex,
                                                          ouClusterList);
}

ERRORCODE BMNetwork::SetDBService(ETYPE_BUS eouProtocol, int nChannelIndex,
                                  int /*nDBIndex*/, ICluster* ouCluster) {
  return m_ouProtocolConfig[eouProtocol].SetDBService(nChannelIndex, ouCluster);
}

ERRORCODE BMNetwork::AddDBService(ETYPE_BUS eouProtocol, int nChannelIndex,
                                  ICluster* ouCluster) {
  ERRORCODE retVal =
      m_ouProtocolConfig[eouProtocol].AddDBService(nChannelIndex, ouCluster);
...
ERRORCODE BMNetwork::LoadDb(ETYPE_BUS forCluster, int channelIndex,
...
ERRORCODE BMNetwork::DeleteDBService(ETYPE_BUS eouProtocol, int nChannelIndex,
                                     std::string dbPath) {
  std::list<ICluster*> clusterList;
...
ERRORCODE BMNetwork::GetSimulatedEcuList(ETYPE_BUS eouProtocol,
  ...
  return m_ouProtocolConfig[eouProtocol].GetSimulatedEcuList(nChannelIndex,
                                                             ouEcuList);
}

ERRORCODE BMNetwork::GetFrame(ETYPE_BUS eouProtocol, int nChannelIndex,
                              unsigned int unId, void* pProtocolData,
                              IFrame** pouFrame) {
  return m_ouProtocolConfig[eouProtocol].GetFrame(nChannelIndex, unId,
                                                  pProtocolData, pouFrame);
}

ERRORCODE BMNetwork::GetElementByName(ETYPE_BUS eouProtocol, int nChannelIndex,
                                      eClusterElementType oueElementType,
                                      std::string strName,
                                      IElement** pouElement) {
  return m_ouProtocolConfig[eouProtocol].GetElementByName(
      nChannelIndex, oueElementType, strName, pouElement);
}
...
ERRORCODE BMNetwork::SetSimulatedEcuList(ETYPE_BUS eouProtocol,
                                         int nChannelIndex,
                                         std::list<IEcu*>& ouEcuList) {
  return m_ouProtocolConfig[eouProtocol].SetSimulatedEcuList(nChannelIndex,
                                                             ouEcuList);
}
...
ERRORCODE BMNetwork::ManageClientForDbChanges(IDbChangeListner* newListner,
                                              bool bAdd) {
...
ERRORCODE BMNetwork::ParseDbFile(std::string strFileName, ETYPE_BUS clusterType,
  ...
  ERRORCODE ecCode =
      mDbManagerAcessor.mParseDbFile(strFileName, clusterType, ouClusterResult);

  if (EC_FAILURE != ecCode) {
    for (std::list<ClusterResult>::iterator itr = ouClusterResult.begin();
         itr != ouClusterResult.end(); itr++) {
...
BMProtocolConfig::BMProtocolConfig() { m_nChannelConfigured = 0; }
...
ERRORCODE BMProtocolConfig::GetChannelSettings(int nChannelIndex,
...
ERRORCODE BMProtocolConfig::GetDBService(int nChannelIndex, int nDbIndex,
                                         ICluster** ouCluster) {
  *ouCluster = nullptr;
  if (nChannelIndex < m_nChannelConfigured) {
...
ERRORCODE BMProtocolConfig::GetDBServiceList(
    int nChannelIndex, std::list<ICluster*>& ouClusterList) {
  if (nChannelIndex < m_nChannelConfigured) {
...
ERRORCODE BMProtocolConfig::GetFrame(int nChannelIndex, unsigned int unId,
                                     void* ouFrameProps, IFrame** pouFrame) {
  if (nChannelIndex < m_nChannelConfigured) {
    return m_ouChannelConfig[nChannelIndex].GetFrame(unId, ouFrameProps,
                                                     pouFrame);
  }
  return EC_FAILURE;
}

ERRORCODE BMProtocolConfig::GetElementByName(int nChannelIndex,
                                             eClusterElementType oueElementType,
                                             std::string strName,
                                             IElement** pElement) {
  if (nChannelIndex < m_nChannelConfigured) {
    return m_ouChannelConfig[nChannelIndex].GetElementByName(oueElementType,
                                                             strName, pElement);
  }
  return EC_FAILURE;
}

ERRORCODE BMProtocolConfig::SetChannelSettings(
    int nChannelIndex, ChannelSettings* ouChannelSettings) {
  if (nChannelIndex < m_nChannelConfigured) {
    return m_ouChannelConfig[nChannelIndex].SetChannelSettings(
        ouChannelSettings);
  }
  return EC_FAILURE;
}

ERRORCODE BMProtocolConfig::SetDBService(int nChannelIndex,
                                         ICluster* ouCluster) {
  if (nChannelIndex < m_nChannelConfigured) {
...
ERRORCODE BMProtocolConfig::AddDBService(int nChannelIndex,
                                         ICluster* ouCluster) {
  if (nChannelIndex < m_nChannelConfigured) {
...
ERRORCODE BMProtocolConfig::LoadDb(int channelIndex, std::string dbFilePath) {
  // Comment this line once channelwise nework is added for CAN
  channelIndex = 0;
  return EC_FAILURE;
}
...
ERRORCODE BMProtocolConfig::GetSimulatedEcuList(int nChannelIndex,
                                                std::list<IEcu*>& ouEcuList) {
  return m_ouChannelConfig[nChannelIndex].GetSimulatedEcuList(ouEcuList);
  ;
}

ERRORCODE BMProtocolConfig::SetSimulatedEcuList(int nChannelIndex,
                                                std::list<IEcu*>& ouEcuList) {
  return m_ouChannelConfig[nChannelIndex].SetSimulatedEcuList(ouEcuList);
}

ERRORCODE BMChannelConfig::GetChannelSettings(
    ChannelSettings* ouChannelSettings) {
...
ERRORCODE BMChannelConfig::GetFrame(unsigned int unId, void* ouFrameProps,
                                    IFrame** ouFrame) {
  *ouFrame = nullptr;
...
ERRORCODE BMChannelConfig::GetElementByName(eClusterElementType oueElementType,
                                            std::string strName,
                                            IElement** pElement) {
  *pElement = nullptr;
  for (auto itr : m_ouDbList) {
...
ERRORCODE BMChannelConfig::SetChannelSettings(
    ChannelSettings* ouChannelSettings) {
  if (nullptr != ouChannelSettings) {
