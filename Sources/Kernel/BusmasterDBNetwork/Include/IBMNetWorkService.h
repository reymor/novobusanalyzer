#ifndef KERNEL_BUSMASTERDBNETWORK_IBMNETWORKSERVICE_H_
#define KERNEL_BUSMASTERDBNETWORK_IBMNETWORKSERVICE_H_
...
class IBMNetWorkService : public IBMNetWorkGetService {
 public:
  ...
  virtual ERRORCODE SetChannelSettings(ETYPE_BUS eouProtocol, int nChannelIndex,
                                       ChannelSettings*) = 0;
  virtual ERRORCODE SetDBService(ETYPE_BUS eouProtocol, int nChannelIndex,
  ...
  virtual ERRORCODE ParseDbFile(std::string strFileName, ETYPE_BUS clusterType,
                                std::list<ICluster*>&) = 0;
  virtual ERRORCODE SetSimulatedEcuList(ETYPE_BUS, int nChannelIndex,
                                        std::list<IEcu*>&) = 0;
  virtual ERRORCODE ReSetNetwork(ETYPE_BUS eouProtocol) = 0;
  virtual bool isDbManagerAvailable() = 0;
};

#endif  // KERNEL_BUSMASTERDBNETWORK_IBMNETWORKSERVICE_H_