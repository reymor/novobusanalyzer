#ifndef KERNEL_BUSMASTERDBNETWORK_IBMNETWORKGETSERVICE_H_
#define KERNEL_BUSMASTERDBNETWORK_IBMNETWORKGETSERVICE_H_
...
enum eFlexRayKeySlotMode { eKeySlotManual, eKeySlotOff, eKeySlotNone };
...
class FlexRayChannelSettings {
 public:
  // 2 Key Slot Configuration
  FlexRayKeySlot m_omKeySlotConfig[2];
  std::list<std::string> m_SelectedEcuList;
...
class CScheduleCommands {
 public:
  ...
  int m_nId;
  bool m_bEnabled;
  CScheduleCommands() { m_bEnabled = false; }
};
...
class CSheduleTable {
 public:
  ...
  CSheduleTable(const CSheduleTable& ouRefObj) {
    ...
    m_listCommands.clear();
    m_listCommands.insert(m_listCommands.begin(),
                          ouRefObj.m_listCommands.begin(),
                          ouRefObj.m_listCommands.end());
  }
...
class LINChannelSettings {
 public:
  ...
  LINChannelSettings() { vInitialiseConfig(); }
  ...
  LINChannelSettings(const LINChannelSettings& ouRefObj) {
    ...
    ouLinParams.clear();
    ouLinParams.insert(ouLinParams.begin(), ouRefObj.ouLinParams.begin(),
                       ouRefObj.ouLinParams.end());
  }
...
class IBMNetWorkGetService {
 public:
  virtual ERRORCODE GetChannelSettings(ETYPE_BUS eouProtocol, int nChannelIndex,
                                       ChannelSettings*) = 0;
  virtual ERRORCODE GetDBServiceList(ETYPE_BUS eouProtocol, int nChannelIndex,
                                     std::list<ICluster*>&) = 0;
  virtual ERRORCODE GetDBService(ETYPE_BUS eouProtocol, int nChannelIndex,
                                 int nDBIndex, ICluster**) = 0;
  // pProtocolData extra unique info base cycle, channelab etc
  virtual ERRORCODE GetFrame(ETYPE_BUS eouProtocol, int nChannelIndex,
                             unsigned int unId, void* pProtocolData,
                             IFrame**) = 0;
  virtual ERRORCODE GetChannelCount(ETYPE_BUS eouProtocol,
                                    int& nChannelsConfigured) = 0;
  virtual ERRORCODE GetDBServiceCount(ETYPE_BUS eouProtocol, int nChannelIndex,
                                      int& nCount) = 0;
  virtual ERRORCODE GetFrameList(ETYPE_BUS eouProtocol, int nChannelIndex,
                                 std::list<IFrame*>&) = 0;
  virtual ERRORCODE GetSimulatedEcuList(ETYPE_BUS, int nChannelIndex,
                                        std::list<IEcu*>&) = 0;
  virtual ERRORCODE GetElementByName(ETYPE_BUS eouProtocol, int nChannelIndex,
                                     eClusterElementType oueElementType,
                                     std::string strName,
                                     IElement** pouElement) = 0;
  virtual ERRORCODE ManageClientForDbChanges(IDbChangeListner*, bool bAdd) = 0;
};

#endif  // KERNEL_BUSMASTERDBNETWORK_IBMNETWORKGETSERVICE_H_