...
class INotifyClusterChange {
 public:
  virtual ERRORCODE OnClusterChange(eAction eActionType, UID_ELEMENT nId,
                                    eClusterElementType& ouElementType,
                                    void* pAcionData, void* pUserData) = 0;
};
...
class ICluster {
 public:
  ...
  virtual ERRORCODE CreateElement(eClusterElementType eType,
                                  IElement** pElement) = 0;
  virtual ERRORCODE DeleteElement(eClusterElementType eType,
                                  UID_ELEMENT& nUniqueId) = 0;

  virtual ERRORCODE GetProperties(ePropertyType ouePropertyType,
                                  void* pProperties) = 0;
  virtual ERRORCODE SetProperties(ePropertyType ouePropertyType,
                                  void* pProperties) = 0;

  virtual ERRORCODE GetElement(eClusterElementType eType, UID_ELEMENT nId,
                               IElement** pElement) = 0;
  virtual ERRORCODE GetElementList(
      eClusterElementType eType,
      std::map<UID_ELEMENT, IElement*>& pElement) = 0;
  virtual ERRORCODE GetElementByName(eClusterElementType eType,
                                     std::string ouElementName,
                                     IElement** pElement) = 0;
  virtual ERRORCODE GetElementListByName(
      eClusterElementType eType, std::string& strEcuName,
      std::list<IElement*>& pElementList) = 0;
  ...
  virtual ERRORCODE RegisterForChangeNotification(
      INotifyClusterChange* ouNotification,
      UID_ELEMENT uidOnElement = INVALID_UID_ELEMENT) = 0;
  virtual ERRORCODE NotifyClusterChange(eAction oueAction, UID_ELEMENT& m_nUnId,
...
class IElement {
 ...
 public:
  ...
  virtual ERRORCODE SetName(std::string& strName) {
    // if ( m_strName != strName )
    {
      ...
      if (nullptr != m_pBaseCluster) {
        m_pBaseCluster->NotifyClusterChange(eNameChange, this->m_nUnId,
                                            m_oueElementType, &ouData);
      }
      return EC_SUCCESS;
    }
    return EC_SUCCESS;
  ...
  virtual UID_ELEMENT GetUniqueId() { return m_nUnId; }

  virtual eClusterElementType GetElementType() { return m_oueElementType; }
  ...
  virtual ERRORCODE RegisterForChangeNotification(
      INotifyClusterChange* ouNotification) = 0;
};
...
class IEcu : public IElement {
 ...
 public:
  ...
  virtual ERRORCODE GetFrame(UID_ELEMENT& nId, void* pProtocolSpecParam,
                             IFrame** ouFrame) = 0;

  virtual ERRORCODE GetSignalList(eDIR eDir,
                                  std::map<ISignal*, ISignal*>& ouSignals) = 0;
...
class IFrame : public IElement {
 ...
 public:
  ...
  virtual ERRORCODE GetSignalList(
      std::map<ISignal*, SignalInstanse>& mapSignals) = 0;
  virtual unsigned int GetSignalCount() = 0;
  virtual ERRORCODE GetPduList(std::map<IPdu*, PduInstanse>& mapPdus) = 0;
  virtual ERRORCODE GetUpdatedPdus(unsigned char*,
                                   std::map<IPdu*, PduInstanse>&) = 0;

  virtual ERRORCODE InterpretSignals(
      const unsigned char*, int nSize,
      std::list<InterpreteSignals>& ouSignalInfoList, bool bIsHex,
      bool formatHex = false) = 0;
  virtual ERRORCODE InterpretSignals(
      const unsigned char*, int nSize,
      std::vector<SignalValue>& ouSignalInfoList) = 0;
};
...
class IPdu : public IElement {
 ...
 public:
  ...
  virtual ERRORCODE InterpretSignals(
      int nPduStartBit, const unsigned char*, int nSize,
      std::list<InterpreteSignals>& ouSignalInfoList, bool bIsHex,
      bool formatHex = false) = 0;
  ...
  virtual ERRORCODE GetMappedFrames(
      std::map<IFrame*, PduInstanse>& lstFrames) = 0;
};
...
class ISignal : public IElement {
 ...
 public:
  ...
  virtual ERRORCODE GetMinMaxValue(unsigned __int64& unMinValue,
                                   unsigned __int64& unMaxValue) = 0;
  virtual ERRORCODE MapNode(eDIR eDirection, UID_ELEMENT&) = 0;
  virtual ERRORCODE UnMapNode(eDIR eDirection, UID_ELEMENT&) = 0;
  ...
  virtual ERRORCODE GetRawValue(int nStartBit, int nSignalLength,
                                int nByteLength, bool bIntel,
                                const unsigned char* pchData,
                                unsigned __int64& unRawValue) = 0;
  virtual ERRORCODE GetEnggValueFromRaw(unsigned __int64 dwRawValue,
                                        double& dEnggValue) = 0;
  virtual ERRORCODE GetRawValueFromEng(double dEnggValue,
                                       unsigned __int64& dwRawValue) = 0;
};
...
class ICoding : public IElement {
 ...
 public:
  ...
  virtual ERRORCODE GetSignalList(
      std::map<UID_ELEMENT, ISignal*>& lstSignals) = 0;
  virtual ERRORCODE MapSignal(UID_ELEMENT) = 0;
  virtual ERRORCODE UnMapSignal(UID_ELEMENT) = 0;

  virtual ERRORCODE GetEngValue(eSignalDataType eDataType, int nSiglength,
                                unsigned __int64 unRawValue,
                                double& dEngValue) = 0;
  virtual ERRORCODE GetRawValue(double dEngValue,
                                unsigned __int64& unRawValue) = 0;
};
...
class CLINSheduleTable : public IScheduleTable, public INotifyClusterChange {
 ...
 public:
  ...
  ERRORCODE OnClusterChange(eAction eActionType, UID_ELEMENT nId,
                            eClusterElementType& ouElementType,
...
class INodeConfiguration : public IElement {
 ...
 public:
  virtual ERRORCODE GetCompositeNodes(
      std::list<CompositeNode>& ouCompositeNodeList) = 0;
  virtual ERRORCODE SetCompositeNodes(
      std::list<CompositeNode>& ouCompositeNodeList) = 0;
};
...
extern "C" __declspec(dllexport) ERRORCODE
    ParseDBFile(std::string strFileName, ETYPE_BUS clusterType,
                std::list<ClusterResult>& ouClusterResultList);
extern "C" __declspec(dllexport) ERRORCODE FreeCluster(ICluster* pCluster);
extern "C" __declspec(dllexport) ERRORCODE
    CreateLDFCluster(ICluster** pouLdfCluster);
