...
Functionality   :   Interface function. Sets the start pos to the 'nIndex'th
entry. Member of       :   CMsgBufVVSE Friend of       :   - Authors         :
Pradeep Kadoor Date Created    :   10/11/2009 Modifications   :   -
************************************************************************************/
...
Modifications   :   -
************************************************************************************/
HRESULT CMsgBufVVSE::ReadEntry(int& nType, BYTE* pbyMsg, int& nSize, int nEntry,
...
into circular function. Member of       :   CMsgBufVVSE Friend of       :   -
Authors         :   Pradeep Kadoor
Date Created    :   22/06/2009
Modifications   :   -
************************************************************************************/
...
int CMsgBufVVSE::GetMsgCount(void) const { return m_nMsgCount; }

HANDLE CMsgBufVVSE::hGetNotifyingEvent(void) const { return m_hNotifyingEvent; }
...
HRESULT CMsgBufVVSE::AdvanceToNextMsg() {
  ...
  if (m_nMsgCount == 0) {
    ...
    // Helper function to advance the read index to the next msg.
...
int CMsgBufVVSE::nAdvanceReadIndex(int& nReadIndex) {
  ...
  // Get current msg header TYPE, MSG LENGTH
...
int CMsgBufVVSE::nWriteBuffer(INT nType, BYTE* pbyMsg, INT nSize) {
  int nResult = CALL_SUCCESS;
  static BYTE abyHeader[HEADER_LEN] = {0};
  // Helper function to construct header from TYPE, MSG LENGTH
...
Modifications   :   -
************************************************************************************/
HRESULT CMsgBufVVSE::ReadBuffer(INT& nType, BYTE* pbyMsg, INT& nSize,
                                INT& nIndex) {
  ...
  if (nMsgLen > nSize) {
    ...
    if ((nIndex + HEADER_LEN + nMsgLen) <= m_nBufferSize) {
      ...
      memcpy(pbyMsg, (m_pbyMsgBuffer + nIndex + HEADER_LEN),
             (m_nBufferSize - nIndex - HEADER_LEN));
      memcpy((pbyMsg + (m_nBufferSize - nIndex - HEADER_LEN)), m_pbyMsgBuffer,
             (nMsgLen - (m_nBufferSize - nIndex - HEADER_LEN)));
      nIndex = nMsgLen - (m_nBufferSize - nIndex - HEADER_LEN);
    } else {
      memcpy(pbyMsg, (m_pbyMsgBuffer + (nIndex + HEADER_LEN - m_nBufferSize)),
             nMsgLen);
...
int CMsgBufVVSE::nGetCurrMsgHeader(int nIndex, BYTE* pbyHeader) {
  if ((nIndex + HEADER_LEN) <= m_nBufferSize) {
    ...
    memcpy(pbyHeader + (m_nBufferSize - nIndex), m_pbyMsgBuffer,
           (nIndex + HEADER_LEN) - m_nBufferSize);
  }
  return CALL_SUCCESS;
}
