...
#ifndef KERNEL_UTILITIES_MSGBUFCANVFSE_H_
#define KERNEL_UTILITIES_MSGBUFCANVFSE_H_

//@TODO: Delete this way of find the header
...
class CMsgBufCANVFSE {
 protected:
  ...
  int m_nMsgSize; /* At the beginning we need to store size of a message entry.
 ...
 public:
  ...
  SMSGBUFFER* nGetBuffer() { return m_asMsgBuffer; }
...
******************************************************************************/
template <typename SMSGBUFFER>
void CMsgBufCANVFSE<SMSGBUFFER>::vClearMessageBuffer(void) {
  // Lock the buffer
...
template <typename SMSGBUFFER>
int CMsgBufCANVFSE<SMSGBUFFER>::ReadFromBuffer(SMSGBUFFER* psMsg,
                                               __int64 nSlotId) {
...
int CMsgBufCANVFSE<SMSGBUFFER>::WriteIntoBuffer(const SMSGBUFFER* psMsg,
  ...
  if (m_nMsgCount == SIZE_APP_CAN_BUFFER) {
...
******************************************************************************/
template <typename SMSGBUFFER>
int CMsgBufCANVFSE<SMSGBUFFER>::GetBufferLength(void) const {
  return m_nMsgCount;
}
...
******************************************************************************/
template <typename SMSGBUFFER>
HANDLE CMsgBufCANVFSE<SMSGBUFFER>::hGetNotifyingEvent(void) const {
...
******************************************************************************/
template <typename SMSGBUFFER>
void CMsgBufCANVFSE<SMSGBUFFER>::vDoSortBuffer(int /*nField*/,
                                               bool /*bAscending*/) {}
/******************************************************************************
  Function Name    :  vDoSortIndexMapArray
  Input(s)         :
  Output           :  -
  Functionality    :  Reorders the Index Map Array according to the order
specified. Member of        :  CMsgBufCANVFSE Friend of        :  - Author(s) :
Arunkumar K Date Created     :  28-06-2010 Modification date: Modification By  :
******************************************************************************/
template <typename SMSGBUFFER>
void CMsgBufCANVFSE<SMSGBUFFER>::vDoSortIndexMapArray() {
...
__int64 CMsgBufCANVFSE<SMSGBUFFER>::GetSlotID(STCANDATA& pDatCAN) {
  ...
  int nMsgID =
      MAKE_RX_TX_MESSAGE(sMsg.m_unMsgID, IS_RX_MESSAGE(pDatCAN.m_ucDataType));
  ...
  __int64 n64MapIndex = MAKE_CHANNEL_SPECIFIC_MESSAGE(nMsgID, sMsg.m_ucChannel);
  return n64MapIndex;
}
...
// clang-format on
template <typename SMSGBUFFER>
void CMsgBufCANVFSE<SMSGBUFFER>::nGetMapIndexAtID(int nIndex,
                                                  __int64& nMapIndex) {
...
#endif  // KERNEL_UTILITIES_MSGBUFCANVFSE_H_