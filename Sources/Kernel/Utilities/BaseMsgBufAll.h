...
#ifndef KERNEL_UTILITIES_BASEMSGBUFALL_H_
#define KERNEL_UTILITIES_BASEMSGBUFALL_H_
...
template <typename SMSGBUFFER>
CBaseMsgBufFSE<SMSGBUFFER>::CBaseMsgBufFSE() {
  ;
}
...
/* This is the interface class of a circular queue where each entry is of
variable size. VVSE stands for 'variant variable sized entry'. It is a special
type of VSE where user can read n'th element from the buffer. */
class CBaseMsgBufVVSE {
 public:
  // Standard constructor
  CBaseMsgBufVVSE(){};
  // Destructor
  virtual ~CBaseMsgBufVVSE(){};
  ...
  virtual HRESULT ReadEntry(int &nType, BYTE *pbyMsg, int &nSize, int nEntry,
                            BOOL bSetNextIndexStartPos) = 0;
};

#endif  // KERNEL_UTILITIES_BASEMSGBUFALL_H_