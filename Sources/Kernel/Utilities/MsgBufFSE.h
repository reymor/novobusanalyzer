...
#ifndef KERNEL_UTILITIES_MSGBUFFSE_H_
#define KERNEL_UTILITIES_MSGBUFFSE_H_
 ...
 * of fixed size. Implemented as a template class so as to cater to any data
 * type. Here SMSGBUFFER is the data type in operation.
 */
...
template <typename SMSGBUFFER>
void CMsgBufFSE<SMSGBUFFER>::vClearMessageBuffer(void) {
...
template <typename SMSGBUFFER>
HRESULT CMsgBufFSE<SMSGBUFFER>::ReadFromBuffer(SMSGBUFFER* psMsg) {
...
template <typename SMSGBUFFER>
HRESULT CMsgBufFSE<SMSGBUFFER>::WriteIntoBuffer(SMSGBUFFER* psMsg) {
  int nResult = CALL_SUCCESS;
...
template <typename SMSGBUFFER>
int CMsgBufFSE<SMSGBUFFER>::GetMsgCount(void) const {
  return m_nMsgCount;
}

template <typename SMSGBUFFER>
HANDLE CMsgBufFSE<SMSGBUFFER>::hGetNotifyingEvent(void) const {
  return m_hNotifyingEvent;
}

template <typename SMSGBUFFER>
HRESULT CMsgBufFSE<SMSGBUFFER>::ReadFromBuffer(SMSGBUFFER* /*psMsgBuffer*/,
                                               __int64 /*nSlotId*/) {
...
template <typename SMSGBUFFER>
HRESULT CMsgBufFSE<SMSGBUFFER>::WriteIntoBuffer(
    const SMSGBUFFER* /*psMsgBuffer*/, __int64 /*nSlotId*/, int& /*nIndex*/) {
  return ERR_NOT_SUPPORTED;
}
...
#endif  // KERNEL_UTILITIES_MSGBUFFSE_H_