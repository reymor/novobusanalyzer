...
{
    CExecuteManager::ouGetExecuteManager(J1939, &CGlobalObj::ouGetObj(J1939)).vManageOnDataConfHandlerJ1939(dwClient, unPGN, bySrc,
            byDest, bSuccess);
}
static void sg_vAddressClaimEventFnJ1939(DWORD dwClient, BYTE byAddress)
{
    CExecuteManager::ouGetExecuteManager(J1939, &CGlobalObj::ouGetObj(J1939)).vManageOnAddressClaimHandlerJ1939(dwClient, byAddress);
}
...
BOOL CGlobalObj::bOpenFunctioneditorfile(CString omStrNewCFileName, bool bIsOldFormat)
{
    ...
    if (pDoc != nullptr)
    {
        ...
        {
            bFileFound = pDoc->bCreateNewDocument(omStrNewCFileName, this);
        }
        if (bFileFound == TRUE)
        {
            //// Now open the selected file
            pDoc->OnOpenDocument(omStrNewCFileName, this, bIsOldFormat);
...
CString CGlobalObj::omGetBusSpecMsgStructName(ETYPE_BUS eBus)
{
    CString omName;
    switch (eBus)
    {
        ...
        case LIN:
        {
            omName = "STLIN_MSG"; // Protected Identifier
        }
        break;
    }
    return omName;
    ...
    switch (m_eBus)
    {
        ...
        case J1939:
        {
            hResult = CGlobalObj::GetIJ1939DIL()->DILIJ_RegisterClient(bRegister,
                      pNodeInfo->m_omStrNodeName.GetBuffer(0),
