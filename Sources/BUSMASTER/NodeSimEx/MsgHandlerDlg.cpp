...
void CMsgHandlerDlg::OnCbtnMsgHandlerApply()
{
    ...
    if (nullptr != pDoc)
    {
        ...
        {
            CString omStrBusSpecHndlrName = CGlobalObj::omGetBusSpecMsgHndlrName(sBusSpecInfo.m_eBus);

            CString omFunc = omStrBusSpecHndlrName;
            ...
            if (bIsMsgSpecificHandler == TRUE && (m_eBus == J1939 || m_eBus == CAN || m_eBus == LIN))
            {
            ...
            CString strFunctionName = omSelectedText;
            omSelectedText.Format(  omFormatString,
                                    omStrBusSpecHndlrName,
                                    omSelectedText,   // Fun name
                                    omStrParamtype ); // Parameter type

            pDoc->m_omSourceCodeTextList.AddTail( omSelectedText );

            if ( bIsMsgSpecificHandler == TRUE )
            {
                pDoc->bAddFunctionWrappers(omSelectedText, omMsgHandlerType);
            }
            else
            {
...
void CMsgHandlerDlg::OnRbtnMsgName()
{
    ...
    m_omListMsgName.SetCurSel(-1);
    vUpdateMessageNameListBox();
}
void CMsgHandlerDlg::vUpdateMessageNameListBox()
{
    CString strChnlNum;
    ...
    if(nullptr != pCluster)
    {
        pCluster->GetFrameList(lstMsgNames);
    }
    CFunctionEditorDoc* pDoc = CGlobalObj::ouGetObj(m_eBus).podGetFunctionEditorDoc();
    ...
    m_omListMsgName.ResetContent();
    std::string omFrameName;
    std::list<std::string> strNameList;
    vGetUniqueFrameNames(strNameList);
    for(auto itrLstMsg = strNameList.begin(); itrLstMsg != strNameList.end(); itrLstMsg++)
    {
        bAddMessageNameInListBox(pomStrArray, itrLstMsg->c_str());
    }
}

std::string CMsgHandlerDlg::GetUniqueFrameName(IFrame* pouFrame)
{
    if (pouFrame == nullptr)
    {
        return "";
...
void CMsgHandlerDlg::vGetUniqueFrameNames(std::list<std::string>& strNameList)
{
    std::list<IFrame*> lstMsgNames;
    CString strChnlNum;
    ((CComboBox*)GetDlgItem(IDC_CMB_CHANNEL))->GetWindowTextA(strChnlNum);
...
for ( auto itr : lstMsgNames )
    {
        ...
        strNameList.push_back(GetUniqueFrameName(itr));
    }
}
...
BOOL CMsgHandlerDlg::OnInitDialog()
{
    ...
    INT unChnlConfig;
    CGlobalObj::ouGetObj(m_eBus).m_ouClusterConfig->GetChannelCount(m_eBus, unChnlConfig);

    if(unChnlConfig == 0)
    {
        //This has to be removed on multichannel support
        unChnlConfig = 1;
    }
...
void CMsgHandlerDlg::vUpdateMsg()
{
    vUpdateMessageNameListBox();
}