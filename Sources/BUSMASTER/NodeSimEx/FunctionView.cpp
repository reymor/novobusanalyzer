...
CGlobalObj* CFunctionView::m_pGlobalObj = nullptr;
...
/*                    and m_nStartingLine                                    */
/*****************************************************************************/
CFunctionView::CFunctionView(CGlobalObj* pGlobalObj)
{
    m_omStrFnName      = "";
...
BEGIN_MESSAGE_MAP(CFunctionView, CRichEditView)
    ...
    ON_COMMAND(IDM_CONTEXT_INSERTPDU, OnInsertPdu)
...
bool CFunctionView::bDisableBusSpecificMenu()
{
    return true;
}
...
void CFunctionView::OnContextMenu(CWnd* pWnd, CPoint point)
{
    ...
    if ( m_sStartPos != nullptr ) // Display only if user has selected
    {
        ...
        CMDIFrameWnd* pMainFrame = (CMDIFrameWnd*)CWnd::FromHandle(m_pGlobalObj->sm_hWndMDIParentFrame);
        ...
        if(pMainFrame != nullptr )
        {
            ...
            if(podParentFrame == podActiveChildWnd)
            {
                ...
                if (omCtxMenu.LoadMenu(IDM_FNVIEW_CTXMENU))
                {
                    ...
                    //Disable insert Pdu.
                    if (bDisableBusSpecificMenu())
                    {
                        pomCtxMenu->EnableMenuItem(
                            IDM_CONTEXT_INSERTPDU, nDisable );
                    }
                    // Disable insert Message and Signal if there is no active
                    //database
                    if( m_pGlobalObj->m_odMsgNameMsgCodeListDb.GetCount() == 0)
                    {
                        int nChannelsConfigured;
                        m_pGlobalObj->m_ouClusterConfig->GetChannelCount( m_eBus, nChannelsConfigured );
                        if( nChannelsConfigured <= 0)
                        {
                            pomCtxMenu->EnableMenuItem(
...
{
    CFunctionEditorDoc* pDoc = (CFunctionEditorDoc*)CView::GetDocument();
    CExploreMsgSg od_Dlg(pDoc, m_pGlobalObj->m_odMsgNameMsgCodeListDb,
                         TRUE, FNVIEW, SEL_SIGNAL, m_pGlobalObj);

    od_Dlg.vSetFrameList(m_pGlobalObj->m_ouClusterConfig, m_eBus );
...
void CFunctionView::OnInsertMessage()
{
    ...
    CFunctionEditorDoc* pDoc = m_pGlobalObj->podGetFunctionEditorDoc();//(CFunctionEditorDoc*)CView::GetDocument();
    CExploreMsgSg od_Dlg(pDoc, m_pGlobalObj->m_odMsgNameMsgCodeListDb,
                         TRUE, FNVIEW, eType, m_pGlobalObj);
    od_Dlg.vSetFrameList(m_pGlobalObj->m_ouClusterConfig, m_eBus );
...
{
    //AFX_MANAGE_STATE(AfxGetStaticModuleState());
    CSelectFunctions od_Dlg(m_eBus, m_pGlobalObj);
...
/**************************************************************************************
    ...
    Input(s)         :  void
    Output           :  void
    Functionality    :  Clear Rich edit control Set Readonly
    Member of        :  CFunctionView
    Author(s)        :  Robin G.K.
    Date Created     :  13.10.2015
    Requirement ID   :
***************************************************************************************/
void CFunctionView::vClear()
{
    // Get ref to rich edit control
    CRichEditCtrl& omRichEdit=GetRichEditCtrl();
    // Set view to empty
...
void CFunctionView::OnInsertPdu()
{
