...
BOOL CEditFrameWnd::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
    ...
    if ( m_bIsSplitterCreated == TRUE )
    {
        ...
        CFnsTreeView::m_pGlobalObj = &CGlobalObj::ouGetObj(m_eBus);
        ...
        if ( TRUE == m_bIsSplitterCreated )
        {
            ...
            if ( TRUE == m_bIsSplitterCreated )
            {
                ...
                if ( TRUE == m_bIsSplitterCreated )
                {
                    ...
                    CFunctionView::m_pGlobalObj = &CGlobalObj::ouGetObj(m_eBus);
...
/**************************************************************************************
    ...
    Output           :  void
    Functionality    :  Called by the framework when set focus on the frame.
                        It calls Reload modified file of CFunctionEditorDoc,
                        if document is open
    Member of        :  CEditFrameWnd
    Author(s)        :  Robin G.K.
...
{
    CFunctionEditorDoc* pDoc = (CFunctionEditorDoc*)GetActiveDocument();
    if ( pDoc != nullptr )
    {
        pDoc->ReloadModifiedFile();
