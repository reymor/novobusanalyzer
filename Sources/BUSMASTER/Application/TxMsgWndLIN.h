...
typedef struct tagMsgToBeSent {
    UINT32 m_unPGN;
    ELIN_MSG_TYPE m_eMsgType;
    UINT m_unChannel;
    BYTE m_bySrc;
    BYTE m_byDest;
    UINT m_unDLC;
    BYTE m_byPriority;
    BYTE m_abyData[MAX_DATA_LEN_LIN];
    UINT m_unTimerVal;
} SMSGTOBESENT;

// CTxMsgWndJ1939 dialog

class CTxMsgWndLIN : public CDialog {
    DECLARE_DYNAMIC(CTxMsgWndLIN)

public:
    CTxMsgWndLIN(CWnd* pParent, SLINCLIENTPARAM& sClientParam);   // standard constructor
    virtual ~CTxMsgWndLIN();
...
public:
    ...
    void vSetLINClientParam(SLINCLIENTPARAM& sClientParam);
    ...
    ELIN_MSG_TYPE eGetCurrMsgType(void);
...
private:
    ...
    SLINCLIENTPARAM m_sClientParams;
    BOOL m_bNM;
