...
CSigGrphConfigDlg::CSigGrphConfigDlg( IBMNetWorkGetService* bmNetwork, UINT nHardware, CWnd* pParent /*=nullptr*/ )
    : CDialog(CSigGrphConfigDlg::IDD, pParent)
    , m_nSymbolType(0)
    , m_nLineType(0)
{
    mBmNetwork = bmNetwork;
...
BOOL CSigGrphConfigDlg::OnInitDialog()
{
    ...
    CRect wndRect;
    m_omSignalList.GetWindowRect(&wndRect);
...
{
    CElementSelection omConfig( mBmNetwork, m_nHardware );
...
void CSigGrphConfigDlg::vPopulateElementList()
{
    ...
    IBMNetWorkGetService* pomDatabase = mBmNetwork;
    CGraphElement odTemp;
    if( podList != nullptr && pomDatabase != nullptr )
    {
        UINT unMsgID = 0;
        ...
        for( int nIndex = 0; nIndex < nItemCount; nIndex++ )
        {
            ...
            else
            {
                ...
                frame = nullptr;
                pomDatabase->GetFrame(CAN, channel, unMsgID, nullptr, &frame );
                if ( nullptr != frame )
                {
                    frame->GetName( omStrName );

                    // Add Element Catogory
                    m_omSignalList.InsertItem( nIndex, omStrName.c_str() );
                    // Add Element Name
                    m_omSignalList.SetItemText( nIndex, 1,
                                                odTemp.m_omStrElementName );
                    // Set Color of the entry
