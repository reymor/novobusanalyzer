...
void CMDIChildBase::OnShowWindow(BOOL bShow, UINT nStatus)
{
    ...
    if (bShow == TRUE) //This check needs to be removed when TxWindow is reused for remaing protocols
    {
                ...
                sizeof(WINDOWPLACEMENT)) != 0 )
    {
        ...
        switch (m_eBusType)
        {
            case CAN:
                ...
                {
                    pITxFormView->SetTxData(TX_WINDOW_PLACEMENT, &m_sWindowPlacement);
                    ((CCANTxFormView*)pITxFormView)->ReleaseInstance();
                }
                break;
            case LIN:
                ...
                {
                    pITxFormView->SetTxData(TX_WINDOW_PLACEMENT, &m_sWindowPlacement);
                    ((CLINTxFormView*)pITxFormView)->ReleaseInstance();
                }
                break;
            default:
                break;
