...
#ifndef BUSMASTER_UDS_PROTOCOL_UDSMAINWND_H_
#define BUSMASTER_UDS_PROTOCOL_UDSMAINWND_H_
...
class CUDSMainWnd : public CDialog {
 ...
 public:
  CUDSMainWnd(UINT SA, UINT TA, UDS_INTERFACE fInterface, UINT CanId,
              CWnd* pParent = NULL);
  virtual ~CUDSMainWnd();

  /**
  This variable contains all the remaining bytes that has to be sent in a long
  request
  */
  CString DatatoSend;
  ...
  /** This variable contains  the value of the CAN ID or Base Address that the
   * user inserts into the Settings Wnd  */
  UINT CanID;
  ...
  /**
   * This function is called to set in the UDS_Main_Window the value of the
   * CanID editor Box It takes into account the Interface that has been chosen
   * and the values os SA and TA
   */
  void setValue();

  /**
   * This function is called by framework when user wants to Transmit a  message
   * it Filters the message so only the msg with a proper SA,TA and CANId can be
   * sent
   */
  void CUDSMainWnd::OnBnClickedSendUD();

  /**
   * This function is called by the framework when the user has pressed the
   * TesterPresent's check box to start the timer to send the tester present
   * message or to stops its sending
   */
  void CUDSMainWnd::OnBnClickedTesterPresent();

  /**
   * This function in called by the framework when the user modifies the
   * Databytes section in the Main Window. It's used to format the data inserted
   * and to count the bytes and put the result in the DLC editor box.
   */
  void CUDSMainWnd::OnEnChangeData();
  /**
   * This function in called by the framework when the user modifies the SA
   * section in the Main Window. It's used to set the the CAN identifier with
   * the Source Address added
   */
  ...
  /**
    \brief  Sends the first frame (FF) of a long request
    * Prepares the structure of the FF of a long request and it sends it.
    \param omByteStr contains the bytes that are sent in the FF of request
  ...
  */
  ...
  \brief It prepares the message to be sent
    * Called by OnBnClickedSendUDS when the user needs to transmit a message
    * It evaluates the message and divide it according to its lenght
    \param  omByteStr bytes that has to be sent in the FF of the long request
    \param  abByteArr[] is the array where the data should be put to be sent
    \param  psTxCanMsgUds is the general structure of the message
  ...
  */
  ...
  /** This function is called to send the flow control because it has been
   *received a long response  */
  ...
  * This function is called from the Mainfrm when the user has changed the
  channel selection
  */
 ...
 private:
  ...
  /**
  \brief This function is used to calulate the difference in time between two
  specified moments
...
#endif  // BUSMASTER_UDS_PROTOCOL_UDSMAINWND_H_