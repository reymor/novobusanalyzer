...
NEGATIVE_RESP_STRUCT NegativeResponseStructUDS[SIZE_NEG_RESP_STRUCT] = {
    ...
    {0x8F,
     "Neg Response - Brake Switch(es)NotClosed (Brake Pedal not pressed or not "
     "applied)",
     RGB(255, 00, 0)},
    ...
    {10, "     Positive Response",
     RGB(0, 100, 0)},  // It has to be always declared here
...
NEGATIVE_RESP_STRUCT NegativeResponseStructKWP[32] = {
    ...
    {0x13, "Neg Response - Incorrect Message Length Or Invalid Format",
     RGB(255, 00, 0)},
    {0x21, "Neg Response - Busy Repeat Request", RGB(255, 00, 0)},
    {0x22, "Neg Response - Conditions Not Correct Or Request Sequence Error",
     RGB(255, 00, 0)},
    {0x23, "Neg Response - Routine Not Complete", RGB(255, 00, 0)},
    {0x31, "Neg Response - Request Out Of Range", RGB(255, 00, 0)},
    {0x33, "Neg Response - Security Access Denied- Security Access Requested  ",
     RGB(255, 00, 0)},
    {0x35, "Neg Response - Invalid Key", RGB(255, 00, 0)},
    {0x36, "Neg Response - Exceed Number Of Attempts", RGB(255, 00, 0)},
    {0x37, "Neg Response - Required Time Delay Not Expired", RGB(255, 00, 0)},
    ...
    {0x42, "Neg Response - Can not download to specified address",
     RGB(255, 00, 0)},
    {0x43, "Neg Response - Can not download number of bytes requested",
     RGB(255, 00, 0)},
    {0x50, "Neg Response - Upload not accepted", RGB(255, 00, 0)},
    {0x51, "Neg Response - Improper upload type", RGB(255, 00, 0)},
    {0x52, "Neg Response - Can not upload from specified address",
     RGB(255, 00, 0)},
    {0x53, "Neg Response - Can not upload number of bytes requested",
     RGB(255, 00, 0)},
    ...
    {0x75, "Neg Response - Illegal Byte Count In Block Transfer",
     RGB(255, 00, 0)},
    {0x76, "Neg Response - Illegal Block Transfer Type", RGB(255, 00, 0)},
    {0x77, "Neg Response - Block Transfer Data Checksum Error",
     RGB(255, 00, 0)},
    {0x78, "Neg Response - Request Correctly Received-Response Pending",
     RGB(255, 00, 0)},
    {0x79, "Neg Response - Incorrect ByteCount During Block Transfer",
     RGB(255, 00, 0)},
    {0x80, "Neg Response - Service Not Supported In Active Diagnostic Mode",
     RGB(255, 00, 0)},
    {10, "     Positive Response",
     RGB(0, 100, 0)},  // It has to be always declared here
    {4, "Negative Response", RGB(255, 00, 0)},
...
CString CUDS_NegRespMng::evaluateResp(unsigned char arrayMsg[], int Byte2Init) {
  // Byte2Init indicates which is the first byte to read from the message  to
  // evaluate if it's a possitive or a negative Response Sometimes this value is
  // Initialbyte but in te case of a long response is Initialbyte+1

  unsigned char prueba = arrayMsg[Byte2Init + 2];
  int TempValue = strtol(CurrentService, NULL, 16);
  if (arrayMsg[Byte2Init + 1] == 0x7F) {
    /*CurrentService*/
    if (arrayMsg[Byte2Init + 2] == TempValue) {
      ...
      if (arrayMsg[Byte2Init + 3] == 0x78) {
        ...
        // Start the timer again and disable SEND button
        UdsProtocolPtr2->StartTimer_Disable_Send();
        BytesShown_Line = NUM_BYTES_SHOWN_RESP - 2;
        UdsProtocolPtr2->numberOfBytes = -2;
      }
      // arrayMsg[Byte2Init+3] will indicate wich message'll be shown in the
      // RespDescription section
    ...
    // The Positive resp message will always be at the end of the list
    return NegativeResponseStructUDS[SIZE_NEG_RESP_STRUCT - 2].m_omTypeResp;
