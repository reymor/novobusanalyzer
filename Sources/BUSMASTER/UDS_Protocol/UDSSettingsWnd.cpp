 ...
 * \brief     implementation file for UDSSettingsWnd class
...
void vPopulateDiagnosticStandardComboBox(STANDARD_COMB sg_asInterface[],
                                         int nSize, CComboBox& omComboBox) {
  omComboBox.ResetContent();
  for (int i = 0, nIncr = 0; i < nSize; i++) {
...
void vPopulateFCLength(CComboBox& omComboBox) {
  ...
  // We just have two options for the length of Flowcontrol
...
/**********************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :   Called when OK button is press and saves the new value
...
void CUDSSettingsWnd::OnEnChangeBSize() {
  ...
  // in the case that the BSize Box is empty when OK botton has been pressed
  if (!Temp_BSize.GetLength()) {
    Temp_BSize = "0";
    UpdateData(false);
  }
}
...
/**********************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :   Called when press OK button and it saves the new value
                    of the FCLength
 Member of      :   CUDSSettingsWnd
 Author(s)      :   Sanchez Marin Maria Alejandra
**********************************************************************************************************/
...
void CUDSSettingsWnd::OnSaveSTmin() {
  ...
  // in the case that the STMin Box is empty when OK botton has been pressed
  if (!Temp_SSTMin.GetLength()) {
    Temp_SSTMin = "0";
    UpdateData(false);
  }
...
/**********************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :   Called when press OK and get the value of the parameter
...
/**********************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :   Called when OK is press to get the value of the parameter
                    S3_Client inserted in the SettingsWnd.
 Member of      :   CUDSSettingsWnd
...
void CUDSSettingsWnd::vInitializeUDSSettingsfFields() {
  ...
  vPopulateInterfaceComboBox(sg_asInterface, SIZE_INTERFACE_COMB,
                             m_omInterface);
  // Changed to modify only the Diagnostic Standard box
  vPopulateDiagnosticStandardComboBox(sg_asDiagStandard, NUM_STAND_APPLIED,
                                      m_omStdDiag);
...
/*******************************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :   Called by the framework when the user selects
...
void CUDSSettingsWnd::OnCbnSelchangeComboInterface() {
  ...
  UdsProtocolPtr->fInterface =
      sg_asSupportedInterface[m_nInterfaceIndex]
          .m_eType;  // Lo coloco ac� para modificar cosas en el MainWnd
...
void CUDSSettingsWnd::OnCbnSelchangeStandardDiag() {
  ...
  if (sg_asSupportedDiagStandard[m_omStdDiag.GetCurSel()].Diagnostic_eType ==
      STANDARD_KWP2000) {
...
/*******************************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :  Called by the framework when is press the OK button in the
...
void CUDSSettingsWnd::OnBnApplyPressed() {
  ...
  UdsProtocolPtr->fDiagnostics =
      sg_asSupportedDiagStandard[m_nDiagnosticsIndex].Diagnostic_eType;
  UdsProtocolPtr->fInterface =
      sg_asSupportedInterface[m_nInterfaceIndex].m_eType;
  switch (UdsProtocolPtr->fInterface) {
    case INTERFACE_NORMAL_11: {
      ...
      if (StringReqCanID.GetLength() == 3 && TempReq_CanID >= 0x001 &&
          TempReq_CanID <= 0x7FF) {
        ReqCanID = strtoul(StringReqCanID.Right(3), NULL,
                           16);  /// 3 in loc de 2 casian
        UdsProtocolPtr->SourceAddress = ReqCanID;
        UdsProtocolPtr->MsgID = strtoul(StringReqCanID.Right(3), NULL, 16);
      } else {
        // Isn't a valid CAN ID
        UdsProtocolPtr->SourceAddress = 0x00;
        UdsProtocolPtr->TargetAddress = 00;
      }

      if (StringRespCanID.GetLength() == 3 && TempResp_CanID >= 0x001 &&
          TempResp_CanID <= 0x7FF) {
        /// 3 in loc de 2 casian
        ReqCanID = strtoul(StringRespCanID.Right(3), NULL, 16);
        UdsProtocolPtr->TargetAddress = ReqCanID;
        // the StringRespCanID is updated only if the CanID is correct
        respID = strtoul(StringRespCanID, NULL, 16);

        // casian //
        if (UdsProtocolPtr->SourceAddress == UdsProtocolPtr->TargetAddress) {
          UdsProtocolPtr->MsgID = strtoul(StringReqCanID.Right(3), NULL, 16);

          if (strtoul(StringReqCanID.Right(3), NULL, 16) ==
              strtoul(StringRespCanID.Right(3), NULL, 16)) {
    ...
    case INTERFACE_EXTENDED_11: {
      ...
      if (StringReqBaseAddress.GetLength() == 3 &&
          TempReq_BaseAddress >= 0x100 && TempResp_BaseAddress <= 0x700) {
      ...
      if (StringRespBaseAddress.GetLength() == 3 &&
          TempResp_BaseAddress >= 0x100 && TempReq_BaseAddress <= 0x700) {
    ...
    case INTERFACE_NORMAL_ISO_29: {
      ...
      if (StringReqCanID.GetLength() == 8 &&
          TempReq_CanID <= MAX_VALUE_29BIT_ID) {
      ...
      if (StringRespCanID.GetLength() == 8 &&
          TempResp_CanID <= MAX_VALUE_29BIT_ID) {
        // the StringRespCanID is updated only if the CanID is correct
        respID = strtoul(StringRespCanID, NULL, 16);
      } else {
        // Isn't a valid CAN ID
    ...
    case INTERFACE_NORMAL_J1939_29: {
      ...
      if (StringReqCanID.GetLength() == 8 &&
          TempReq_CanID <= MAX_VALUE_29BIT_ID) {
        ReqCanID = strtoul(StringReqCanID, NULL, 16);
        UdsProtocolPtr->TargetAddress =
            (ReqCanID & MASK_TA_ID_29Bits_J1939) >> 8;
        UdsProtocolPtr->MsgID = strtoul(StringReqCanID.Right(8), NULL, 16);
  ...
  // To verify the length of the simple messages
...
LRESULT CUDSSettingsWnd::OnCommandHelp(WPARAM wParam, LPARAM lParam) {
  ...
  strChmFilePath =
      PathCombine(cChmFilePath, cCurrWkDirPath,
                  "BUSMASTER.chm ::/topics/Diagnostics_Settings.html");
...
/*******************************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :  Sets the settings Window to default vals according to the std
 Member of      :  CUDSSettingsWnd
...
void CUDSSettingsWnd::OnBnCancelPressed() {
  ...
  OnCbnSelchangeStandardDiag();
  UdsProtocolPtr->fInterface =
      sg_asSupportedInterface[Prev_InterfaceIndex].m_eType;
