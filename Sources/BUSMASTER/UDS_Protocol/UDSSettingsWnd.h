...
#ifndef BUSMASTER_UDS_PROTOCOL_UDSSETTINGSWND_H_
#define BUSMASTER_UDS_PROTOCOL_UDSSETTINGSWND_H_
...
/** number of data bytes that can be sent in the current message */
...
class CUDSSettingsWnd : public CDialog {
 ...
 public:
  CUDSSettingsWnd(CWnd* pParent, CUDS_Protocol* pEngine);
  ...
  /**
   * This function is called by the framework when the user selects an option in
   * the interface combo box.
   */
  ...
  /**
  \brief It saves all the settings settled previously in the settings window and
  also closes the Wnd
  * This function is called by the framework when the user press the OK button
  in the
  ...
  * This function is called by the framework when the user press the Apply
  button in the
  * Settings Window to save all the changes made  previously.
  */
  void CUDSSettingsWnd::OnBnApplyPressed();

  /** It sets in the settings Window the default values according to the
   * standard */
  ...
  */
  ...
  /**  This function is called when the user press the OK to get the value of
   * the parameter STmin inserted in the SettingsWnd.
   */
  void CUDSSettingsWnd::OnSaveSTmin();

  /**
  \brief It takes the value of the Flow Control Length combo box
  * This function is called when the user press the OK button and it saves the
  new value
  * of  FCLength
  */
  ...
  */
  ...
  /**  This function is called when the user press the OK to get the value of
   * the parameter P2_TimeOut inserted in the SettingsWnd.
   */
  void CUDSSettingsWnd::OnEnChangeP2_TimeOut();

  /**  This function is called when the user press the OK to get the value of
   * the parameter P2_Extended inserted in the SettingsWnd.
   */
  void CUDSSettingsWnd::OnEnChangeP2_Extended();

  /**  This function is called when the user press the OK to get the value of
   * the parameter S3_Client inserted in the SettingsWnd.
   */
...
#endif  // BUSMASTER_UDS_PROTOCOL_UDSSETTINGSWND_H_