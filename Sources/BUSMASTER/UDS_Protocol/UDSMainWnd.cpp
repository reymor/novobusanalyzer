...
/** This variable is used to indicate that the system should wait for a flow
 * control */
...
CUDSMainWnd::CUDSMainWnd(UINT SA, UINT TA, UDS_INTERFACE FInterface, UINT CanId,
                         CWnd* pParent)
...
/**********************************************************************************************************
 Function Name  :   StartTimer
  Input(s)      :   -
  Output        :   -
 Description    :   Called to start the timer used to send the Tester Present
...
/**********************************************************************************************************
 ...
 Input(s)       :   omByteStr bytes that has to be sent in the FF of the long
request abByteArr[] is the array where the data should be put to be sent
                    psTxCanMsgUds is the general structure of the message
                    FInterface indicates the interface selected in the
SettingsWnd Output         :   - Description    :   This function sends the
first Message of a long request Member of      :   CUDSMainWnd

 Author(s)      :   Sanchez Marin Maria Alejandra
 Date Created   :   28.05.2013
**********************************************************************************************************/
int CUDSMainWnd::SendFirstFrame(CString omByteStr, unsigned char abByteArr[],
  ...
  int c_numberOfTaken =
      numberOfTaken;  // It contains the number of Data bytes that can be sent
                      // in the current message

  Length.Format("%.3x\n", TotalLength);  // Prepare the first 2 bytes of a first
                                         // frame of a long request
  Length = "1" + Length;
  ...
  int i = 7;
  omByteStrTemp =
      omByteStr.Left(c_numberOfTaken);  // I take the part of omByteStr that is
                                        // going to be sent in the first Frame

  // if something is being sent in this moment
  while (omByteStrTemp.GetLength()) {
    omTempByte = omByteStrTemp.Right(NO_OF_CHAR_IN_BYTE);
    // Fill the array to sent with the current data
    abByteArr[i--] = (BYTE)_tcstol(omTempByte, L'\0', 16);
    omByteStrTemp =
        omByteStrTemp.Left(omByteStrTemp.GetLength() - NO_OF_CHAR_IN_BYTE);
  }
  // The FirstFrame always has 8 bytes
  psTxCanMsgUds->m_psTxMsg->m_ucDataLen = 8;

  // Send the message
  SendSimpleDiagnosticMessage();
  // The bytes that were sent are deleted
  omByteStr = omByteStr.Right(((UINT)TotalLength * 2) - c_numberOfTaken);
  // DatatoSend will contain the rest of the bytes that hasn't been sent yet.
  DatatoSend = omByteStr.Right(((UINT)TotalLength * 2) - c_numberOfTaken);
  TotalLength = (((UINT)TotalLength * 2) - c_numberOfTaken) / 2;

  FirstFrame = FALSE;
  FWaitFlow = TRUE;  // No Wait for the Flow Control comming from the ECU
  // Initialize this -> because a long frame will be sent
  ConsecutiveFrame = 0x21;
  return 0;
}

/**********************************************************************************************************
 Function Name  :   SendContinuosFrames
  Input(s)      :   abByteArr[] is the array where the data should be put to be
sent psTxCanMsgUds is the general structure of the message FInterface indicates
the interface selected in the SettingsWnd Output         :   - Description    :
This function sends the continuos frames of a long request. It sends several
messages until the BSize parameter has been reached. Member of      :
CUDSMainWnd

 Author(s)      :   Sanchez Marin Maria Alejandra
 Date Created   :   28.05.2013
**********************************************************************************************************/
void CUDSMainWnd::SendContinuosFrames(unsigned char abByteArr[],
                                      mPSTXSELMSGDATA psTxCanMsgUds,
                                      UDS_INTERFACE FInterface) {
  CString Length;
  CString omTempByte;
  CString omByteStrTemp;
  // It will indicate how many multiples frames have been sent
  int numberofFrames = 0;
  // The consecutive Messages will contain one byte more that the FirstFrame
  int c_numberOfTaken = numberOfTaken + 2;
  // aux_Finterface it's used to indicate that i must be bigger if we're in
  // extended Addressing
  int i = aux_Finterface + c_numberOfTaken / 2;
  if (TotalLength * 2 < c_numberOfTaken) {
    // It only enters here once, at the end when the last message of the
    // multiple frames has to be sent when
    i = TotalLength + aux_Finterface;  // the number of frames that has to be
                                       // sent is less than c_numberOfTaken
  }
  // While there is remaining data that has to be sent
  while (DatatoSend.GetLength()) {
    // take the part of the msg that is going to be sent in the current Frame
    omByteStrTemp = DatatoSend.Left(c_numberOfTaken);

    // Wait if something is being sent in this moment
    while (omByteStrTemp.GetLength()) {
      omTempByte = omByteStrTemp.Right(NO_OF_CHAR_IN_BYTE);
      abByteArr[i--] =
          (BYTE)_tcstol(omTempByte, L'\0', 16);  // It fills the array
      omByteStrTemp =
          omByteStrTemp.Left(omByteStrTemp.GetLength() - NO_OF_CHAR_IN_BYTE);
    }
    // Consecutive Frames can always have 8 bytes
    psTxCanMsgUds->m_psTxMsg->m_ucDataLen = 8;
    // Put the initial Byte of the consecutive frames in a long request
    abByteArr[initialByte] = ConsecutiveFrame;

    SendSimpleDiagnosticMessage();  // Send the current Message
    // DatatoSend will contain the rest of the bytes that hasn't been sent yet.
    DatatoSend = DatatoSend.Right(((UINT)TotalLength * 2) - c_numberOfTaken);
    ...
    numberofFrames++;
    // It enters here when I've reached the quantity of Blocks settled by the
    // ECU in the flow Control
    if (numberofFrames == BSizE) {
      FWaitFlow = TRUE;  // Now it has to wait for the Flow control again
      numberofFrames = 0;
      c_dDiffTime = 0;
      return;  // Now it has to wait for another FlowControl
    } else {
      for (c_dDiffTime = 0, c_unPreviousTime = -1; c_dDiffTime <= STMin;
           CalculateDiffTime()) {
      }  // Wait for the STMin Time settled by the ECU in the flow Control
    }
    c_unPreviousTime = -1;  // ReStart the variables for the timmings
    c_dDiffTime = 0;
    // it must be a bigger number in the case of extended addressing->
    // aux_Finterface to control this.
    i = aux_Finterface + c_numberOfTaken / 2;
    // It only enters here once, at the end when the last message of the
    // multiple frames has to be sent when
    if (TotalLength * 2 < c_numberOfTaken) {
      // the number of frames that has to be sent is less than c_numberOfTaken
      i = TotalLength + aux_Finterface;
    }
  }
  // It only enters here when it has sent all the msg In the case that this
  // function cannot be completed there is a timer as default to activate the
  // SEND button
...
void CUDSMainWnd::SendSimpleDiagnosticMessage(void) {
  ...
  if (psTxCanMsgUds->m_psTxMsg->m_unMsgID != 0 &&
      (TargetAddress || SourceAddress) != 0) {
    int nReturn = g_pouDIL_CAN_Interface->DILC_SendMsg(
        g_dwClientID, psTxCanMsgUds->m_psTxMsg[0]);
  }
  StartTimer();
}
...
void CUDSMainWnd::SendSimpleDiagnosticMessagePanels(mPSTXSELMSGDATA pstx) {
  ...
  int nReturn =
      g_pouDIL_CAN_Interface->DILC_SendMsg(g_dwClientID, pstx->m_psTxMsg[0]);
}
...
UINT CUDSMainWnd::OnSendSelectedMsg(LPVOID pParam) {
  ...
  // Now that the msg was sent its structure can be modified
  FWait_SendingFrame = FALSE;
  return 0;
}
...
/**********************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :   Call when it has been received a long res to send the FC
 Member of      :   CUDSMainWnd
...
void CUDSMainWnd::PrepareFlowControl() {
  // BSize and SSTMin  depends of the value put in the settingsWnd.
  Counter_BSize = BSize;
  // The DLC of the FCmessage depends of the value put in the
  // settingsWnd.
  psTxCanMsgUds->m_psTxMsg->m_ucDataLen = SizeFC;
...
/**********************************************************************************************************
 Function Name  :   PrepareDiagnosticMessage
  Input(s)      :   omByteStr bytes sent in FF of the long request
                    abByteArr[] data should be put to be sent
                    psTxCanMsgUds  general structure of the message
                    ByteArrLen length of the message to send
 Output         :   -
 Description    :   Call by OnBnClickedSendUDS to transmit a message
                    Evaluate the message and divide it according to its lenght
 Member of      :   CUDSMainWnd
...
void CUDSMainWnd::PrepareDiagnosticMessage(CString omByteStr,
                                           mPSTXSELMSGDATA psTxCanMsgUds,
                                           unsigned char abByteArr[],
                                           UINT ByteArrLen) {
  ...
  TotalLength = Result;
  // I have to disable the sendButton everytime that I press SEND
  // If it's a long request I need it
  // If it's a long response I've to restart it  and receive all the bytes
  // If it's a simple response: the response could take more time or it could be
  // a 0x78 resp
  m_omSendButton.EnableWindow(FALSE);

  // Time to wait to enable the send button again
  m_nTimer = SetTimer(ID_TIMER_SEND_BUTTON, P2_Time, NULL);
  ...
  switch (fInterface) {
    case INTERFACE_NORMAL_11: {
      ...
      if (Result > 7) {
        ...
        while (omByteStr.GetLength()) {
          ...
          omByteStr =
              omByteStr.Left(omByteStr.GetLength() - NO_OF_CHAR_IN_BYTE);
        }
    ...
    case INTERFACE_EXTENDED_11: {
      ...
      // Long Request - The limit of a simple request for
      // extended is 6 because the byte that contains the TA.
      if (Result > 6) {
        m_omSendButton.EnableWindow(FALSE);
        // Start Timer to wait to enable by default the send button again
        m_nTimer = SetTimer(ID_TIMER_SEND_BUTTON, P2_Time, NULL);
        ...
        Result = Result + 2;
        CString omTempByte;
        // Take all the bytes that will be part of the simple message
        while (omByteStr.GetLength()) {
          omTempByte = omByteStr.Right(NO_OF_CHAR_IN_BYTE);
          abByteArr[i_counter--] = (BYTE)_tcstol(omTempByte, L'\0', 16);
          omByteStr =
...
/**********************************************************************************************************
 ...
 Input(s)       :   -
 Output         :   -
 Description    :   Call by framework when user wants to Transmit a  message
                    Filter the message only with a proper SA,TA and CANId
 Member of      :   CUDSMainWnd
...
void CUDSMainWnd::OnBnClickedSendUD() {
  KillTimer(ID_TIMER_TP);  // Added to kill the timer everyTime I've pressed the
                           // SEND button
  FSending = TRUE;  // This flag is used to know if a message has been sent from
                    // the UDSMainWnd
  Bytes_to_Show = ("\r\n   1 -> ");
  BytesShown_Line = 1;
  m_abDatas = " ";
  ...
  if (psTxCanMsgUds != NULL) {
    ...
    if (psTxCanMsgUds->m_psTxMsg != NULL) {
      ...
      psTxCanMsgUds->m_psTxMsg->m_ucChannel =
          (UCHAR)m_omComboChannelUDS.GetCurSel() + 1;
      Current_Channel = psTxCanMsgUds->m_psTxMsg->m_ucChannel;
      psTxCanMsgUds->m_psTxMsg->m_unMsgID = (int)m_omCanID.lGetValue();
      if (psTxCanMsgUds->m_psTxMsg->m_unMsgID != 0 &&
          (TargetAddress || SourceAddress) != 0 &&
          m_omMsgDataEdit.GetLength() != 0) {
        // This funtion evaluate if the message is a long or a short request and
        // prepare the message
        PrepareDiagnosticMessage(m_omMsgDataEdit, psTxCanMsgUds,
                                 psTxCanMsgUds->m_psTxMsg->m_ucData, 8);
...
Input(s)       : nIDEvent
Output         : -
Functionality  : Call during Timer events to send TP enable the SEND button
Member of      : CUDSMainWnd
Author(s)      : Sanchez Marin Maria Alejandra
Date Created   : 28.05.2013
Modifications  :
...
void CUDSMainWnd::OnTimer(UINT_PTR nIDEvent) {
  if (nIDEvent == ID_TIMER_TP &&
      psTxCanMsgUds->m_psTxMsg != NULL)  // Prepare the message
  {
    if (fInterface == INTERFACE_NORMAL_ISO_29 ||
        fInterface == INTERFACE_NORMAL_J1939_29) {
    ...
    memset(psTxCanMsgUds->m_psTxMsg->m_ucData, 0, 8);
    if (fInterface == INTERFACE_EXTENDED_11) {
      psTxCanMsgUds->m_psTxMsg->m_ucData[initialByte - 1] = TargetAddress;
    }
    // The tester Present will always have 2 bytes 0x3E y 0x00
    psTxCanMsgUds->m_psTxMsg->m_ucData[initialByte] = 0x02;
    psTxCanMsgUds->m_psTxMsg->m_ucData[initialByte + 1] = 0x3E;
    // how many bytes has the flow Control?
    if (fMsgSize) {
      psTxCanMsgUds->m_psTxMsg->m_ucDataLen = 8;
    } else {
    ...
    if (FCRespReq) {
      // if NoResponse Required is activated
      psTxCanMsgUds->m_psTxMsg->m_ucData[initialByte + 2] = 0x80;
    }
    SendSimpleDiagnosticMessage();
  }
...
Input(s)       : CUDSMainWnd
Output         : -
Functionality  : Called by the framework when the user has pressed the TP's
                 check box to start the timer to send the tester present message
                 or to stops its sending
Member of      : CUDSMainWnd
Author(s)      : Sanchez Marin Maria Alejandra
...
void CUDSMainWnd::OnBnClickedTesterPresent() {
  if (m_omCheckTP.GetCheck()) {
    ...
    if (psTxCanMsgUds != NULL) {
      // to verify if it has been created correctly
      psTxCanMsgUds->m_unCount = 1;
      psTxCanMsgUds->m_psTxMsg = new STCAN_MSG[1];
    }
  } else {
...
void CUDSMainWnd::OnEnChangeTA() {
  ...
  if (fInterface == INTERFACE_NORMAL_ISO_29) {
    MsgID =
        (CanID & NEG_MASK_TA_ID_29Bits) + (TargetAddress & MASK_TA_ID_29Bits);
    CanID = MsgID;
...
BOOL CUDSMainWnd::OnInitDialog() {
  ...
  g_pouDIL_CAN_Interface->DILC_RegisterClient(TRUE, g_dwClientID,
                                              _("CAN_MONITOR"));
...
void CUDSMainWnd::vInitializeUDSfFields() {
  ...
  // To evaluate the status of the Textbox  SA y TA
  switch (fInterface) {
    case INTERFACE_NORMAL_11: {
...
/** This function is called from the Mainframe when the user has changed the
 * channel selection */
...
int CUDSMainWnd::nCalculateCurrTime(BOOL bFromDIL) {
  ...
  int nResult = (CurrSysTimes.wHour * 3600000 + CurrSysTimes.wMinute * 60000 +
                 CurrSysTimes.wSecond) *
                    1000 +
                CurrSysTimes.wMilliseconds * 1;
...
Input(s)       : CUDSMainWnd
Output         : -
Functionality  : Calle to evaluate if the sent message waits for
...
CString CUDSMainWnd::initialEval(CString Data2Send) {
  ...
  if (CService == 0x10 || CService == 0x11 || CService == 0x28 ||
      CService == 0x31 || CService == 0x3E || CService == 0x85 ||
      CService == 0xA0) {
    CString SendingData = Data2Send;
    // added in adition to previos function not working
    SendingData.Remove(' ');
  ...
  Font_Color = RGB(184, 134, 11);
  // If it's the case of  No Positive Response Required return the default value
...
LRESULT CUDSMainWnd::OnCommandHelp(WPARAM wParam, LPARAM lParam) {
  ...
  strChmFilePath =
      PathCombine(cChmFilePath, cCurrWkDirPath,
                  "BUSMASTER.chm ::/topics/Diagnostics_Main_Window.html");
