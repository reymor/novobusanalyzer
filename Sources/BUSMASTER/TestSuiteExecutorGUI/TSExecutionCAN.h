...
public:
    ...
    BOOL bVerifyCanMessage( CVerify_MessageData& ouVerifyData, std::list<InterpreteSignals>&, CMessageResult& ouMsgResult );
...
private:
    ...
    BOOL bMakeCanMessage(IFrame*& pMsg, CSend_MessageData& ouSendData, UCHAR aucData[8], STCAN_MSG& stCanMsg);

    void vSetSignalValue( SignalInstanse& ouSignalInstance, ISignal* ouSigStrct, UCHAR aucData[8], UINT64 u64SignVal );
