...
#include "Utility/FlexListCtrl.h"           // For Flexible List Control
...
public:
    ...
    CFilterConfigDlg( SFILTERAPPLIED_CAN* psSrcList,
                      IBMNetWorkGetService* pMsgDBDetails, UINT nHardware,
...
private:
    ...
    IBMNetWorkGetService* m_pClusterConfig;
    UINT m_nHardware;
    ETYPE_BUS m_eCurrBUS;
    std::map<UID_ELEMENT , std::string> m_ouEcuNameToEcuID;// Ecu can be searched only using ECU id and user needs to select ECU name hence this map.
public:
...
private:
    ...
    void vAddCANFilter();
    void vAddLINFilter();
    HRESULT hGetFrameFromId(UINT unSlotId, ETYPE_BUS eBus, int nChannel, void* pouProtocolId, IFrame** ouFrameStrct);



    EDIRECTION eGetDirection(int nIndex);
public:
