...
int SignalGroupDlg::nValidate() {
  ...
  if (m_ouExistingNameMap.find(strName) != m_ouExistingNameMap.end()) {
    QMessageBox::critical(this, "Error", "Duplicate SignalGroup Name",
                          QMessageBox::Ok);
    return 1;
  }
  ...
  if (unSize <= 0) {
    QMessageBox::critical(this, "Error", "Size Should be greater than 0",
                          QMessageBox::Ok);
    return 1;
  }
    ...
    if (ui.tableSignals->item(i, 0)->checkState() == Qt::Checked) {
      ...
      if (pEdit->text().toInt() > unSize - 1) {
        QMessageBox::critical(
            this, "Error", "Signal Offset should not be more than group size",
            QMessageBox::Ok);
        pEdit->setFocus();
        return 1;
      }
...
void SignalGroupDlg::vFillExistingNameList() {
  eClusterElementType ouClusterElement[2] = {eSignalElement,
                                             eSignalGroupElement};
  std::string strName;
