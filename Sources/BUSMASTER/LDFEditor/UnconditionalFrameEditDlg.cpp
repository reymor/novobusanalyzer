...
UnconditionalFrameEditDlg::UnconditionalFrameEditDlg(IFrame** pouFrame,
  ...
  connect(ui.btnMapSignal, SIGNAL(clicked(bool)), this,
          SLOT(onMapSignal(bool)));
  ...
  connect(ui.comboPublisher, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(onChangePublisher(QString)));
  connect(ui.comboFrameLength, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(onChangeFrameLength(QString)));

  QIcon ouWindowIcon;
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/Frame.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/Frame.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/Frame.png",
                       QSize(16, 16));
...
void UnconditionalFrameEditDlg::SetUpUi() {
  ...
  if (m_eUIMode == eNew) {
    ...
    auto itr = std::find(ouDyanamicFramelist.begin(), ouDyanamicFramelist.end(),
                         ouFrameProps.m_nMsgId);
  ...
  for (auto itrECU : lstRxECUs) {
    ...
    if (maSubscribers.find(strECUName) == maSubscribers.end()) {
      ...
      QListWidgetItem* item = new QListWidgetItem(
          QString::fromStdString(strECUName), ui.listSubscribers);
...
void UnconditionalFrameEditDlg::vAddSignalToTree(
    ISignal* pSignal, SignalInstanse ouSignalInstance) {
  ...
  ouColumns.push_back(GetString(ouSignalProps.m_unSignalSize, 10));
...
void UnconditionalFrameEditDlg::onMapSignal(bool) {
  UID_ELEMENT uidECU =
      ui.comboPublisher->itemData(ui.comboPublisher->currentIndex())
          .value<UID_ELEMENT>();
  ...
  for (unsigned int unIndex = 0; unIndex < ui.tableSignals->rowCount();
       unIndex++) {
    ouSignalInstanse.m_nStartBit =
        GetUnsignedInt(ui.tableSignals->item(unIndex, 1)->text(), 10);
    mapSignalInstance[ui.tableSignals->item(unIndex, 0)
                          ->data(Qt::UserRole)
                          .value<UID_ELEMENT>()] = ouSignalInstanse;
  }
...
void UnconditionalFrameEditDlg::vUpdateFrameSignalList(
    std::map<UID_ELEMENT, SignalInstanse>& mapSelectedSignals) {
  ...
  for (auto itrSignal : mapSelectedSignals) {
    ...
    if (nullptr != pSignal) {
      ...
      mapTempSignals[((ISignal*)pSignal)->GetUniqueId()] =
          itrSignal.second.m_nStartBit;
...
void UnconditionalFrameEditDlg::onSelectionCancel() { reject(); }
...
void UnconditionalFrameEditDlg::vUpdateNewFrameDetails() {
  ...
  // 2. Set Frame Props
  // Always Dec
  unsigned int nLength = GetUnsignedInt(ui.comboFrameLength->currentText(), 10);
  ...
  m_pouLDFCluster->GetEcu(ui.comboPublisher->currentText().toStdString(),
                          &pouEcu);
  if (pouEcu != nullptr) {
  ...
  for (unsigned int unIndex = 0; unIndex < ui.tableSignals->rowCount();
       unIndex++) {
    uidSignal = ui.tableSignals->item(unIndex, 0)
                    ->data(Qt::UserRole)
                    .value<UID_ELEMENT>();
    m_pouLDFCluster->GetElement(eSignalElement, uidSignal,
                                (IElement**)&pSignal);
...
void UnconditionalFrameEditDlg::vUpdateEditFrameDetails() {
  ...
  ouFrameProps.m_unMsgSize =
      GetUnsignedInt(ui.comboFrameLength->currentText(), 10);
  ouFrameProps.m_nMsgId = GetUnsignedInt(ui.comboFrameId->currentText());
  ouFrameProps.m_eLinFrameType = eLinUnconditionalFrame;
...
void UnconditionalFrameEditDlg::vUpdateDynamicFrameDetails() {
  if (ui.chkDynFrame->isEnabled() == true) {
    ...
    m_pouLDFCluster->GetProperties(eLdfDyanmicFrameList, &ouDyanamicFramelist);
    if (m_eUIMode == eEdit) {
      auto itr = std::find(ouDyanamicFramelist.begin(),
                           ouDyanamicFramelist.end(), m_unFrameId);
      if (ouDyanamicFramelist.end() != itr) {
        ...
        if (ui.chkDynFrame->isChecked() == false && m_bIsDynFrame == true) {
          ...
          m_pouLDFCluster->SetProperties(eLdfDyanmicFrameList,
                                         &ouDyanamicFramelist);
        }
      }
    }
...
ERRORCODE UnconditionalFrameEditDlg::nValidateValues() {
  ...
  if (m_mapFrameNames.find(omStrName) != m_mapFrameNames.end() &&
      omStrName != omstrCurrFrame) {
...
void UnconditionalFrameEditDlg::onChangeFrameLength(QString omCurrFrameLen) {
  ...
  for (int nIndex = 0; nIndex < nCount; nIndex++) {
    nStartBit = GetUnsignedInt(ui.tableSignals->item(nIndex, 1)->text(),
                               10);  // Always dec
    nSignalLength = GetUnsignedInt(ui.tableSignals->item(nIndex, 3)->text());
    nEndBit = nStartBit + nSignalLength;

    if (nEndBit > (nCurrLen * 8)) {
      lstSignals.push_back(
          ui.tableSignals->item(nIndex, 0)->text().toStdString());
  ...
  if (strSignals.empty() == false) {
    ...
    omStringStrm << "Signal(s) [" << strSignals.c_str()
                 << "] exceeding frame length will be unmapped.";
    ...
    if (nResult == QMessageBox::Ok) {
      ...
      while (nSigIndex < nSigCount) {
        if (lstSignals.end() !=
            std::find(
                lstSignals.begin(), lstSignals.end(),
                ui.tableSignals->item(nSigIndex, 0)->text().toStdString())) {
      ...
      ui.comboFrameLength->setCurrentText(
          QString::fromStdString(m_omFrameLength));
    }
  }
