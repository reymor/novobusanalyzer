...
SlaveDlg::SlaveDlg(ICluster* pouCluster, IEcu** pouEcu, bool bNew,
  ...
  connect(ui.comboProtocol, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onProtocolSelectionChanged(int)));
  connect(ui.buttonFaultSignals, SIGNAL(clicked()), this,
          SLOT(onEditFaultSignals()));
  ...
  QIcon ouWindowIcon;
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/Ecu.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/Ecu.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/Ecu.png",
                       QSize(16, 16));
...
void SlaveDlg::onSelectionOk() {
  if (0 == nValidateValue()) {
    ...
    ouEcuProps.mSlaveProps.m_fProtocolVersion =
        ui.comboProtocol->currentText().toDouble();
    ouEcuProps.mSlaveProps.m_nConfiguredNAD =
        GetUnsignedInt(ui.editConfigNAD->text());
    ouEcuProps.mSlaveProps.m_nFunctionId =
        GetUnsignedInt(ui.editFunctionId->text());
    ouEcuProps.mSlaveProps.m_nSupplierId =
        GetUnsignedInt(ui.editSupplierID->text());
    ouEcuProps.mSlaveProps.m_nInitialNAD =
        GetUnsignedInt(ui.editInitialNAD->text());
    ...
    if (ui.comboRespErr->currentText().toStdString().empty() == false) {
      ouEcuProps.mSlaveProps.m_nRespErrSignal =
          ui.comboRespErr->currentData().value<UID_ELEMENT>();
    } else {
...
void SlaveDlg::onSelecetionCancel() { reject(); }
  ...
  if (false == m_bEcuInfoRead) {
    ...
    for (auto itrEcu : ouUidElementMap) {
      ...
      if (ouecuProps.m_eEcuType == eSlave) {
        m_MapunConfigId[ouecuProps.mSlaveProps.m_nConfiguredNAD] =
            ouecuProps.mSlaveProps.m_nConfiguredNAD;
      }
    }
    m_bEcuInfoRead = true;
    ...
    if (true == m_bModeEdit) {
      ...
      auto itrId =
          m_MapunConfigId.find(ouecuProps.mSlaveProps.m_nConfiguredNAD);
