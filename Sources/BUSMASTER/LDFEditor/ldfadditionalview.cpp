  ...
  for (int i = 0; i < nRows; i++) {
    ...
    if (nullptr != pItem) {
      ConfigFrameDetails ouConfigFrame =
          pItem->data(Qt::UserRole).value<ConfigFrameDetails>();
...
void LDFAdditionalView::onTreeItemClicked(QTreeWidgetItem* pouTreeItem,
                                          int nId) {
  if (m_pHelpLabel != nullptr) {
    m_pHelpLabel->setText(" ");
  }
  TreeItemID ouTreeItemID =
      pouTreeItem->data(0, Qt::UserRole).value<TreeItemID>();
  if (ouTreeItemID.m_eTreeElementType == eLdfItem &&
      ouTreeItemID.m_uidElement != 0) {
    ICluster* pouCluster = m_pouLDFDatabaseManager->GetLDFCluster();
    IElement* pouElement;
    VALIDATE_POINTER(pouCluster);
    pouCluster->GetElement(
        pouCluster->GetElementType(ouTreeItemID.m_uidElement),
        ouTreeItemID.m_uidElement, &pouElement);
...
int LDFAdditionalView::DisplayScedulingTableAdditionalProps(
    IScheduleTable* pouTable, int nItemIndex) {
...
int LDFAdditionalView::DisplayScheduleTableFrameProps(
  ...
  switch (ouTableItem.m_eDiagType) {
    ...
    case eLIN_SLAVE_FRAME_ID: {
      IElement* pouElement;
      m_pouLdfCluster->GetElement(eFrameElement, ouTableItem.m_nFrameId,
                                  &pouElement);
...
int LDFAdditionalView::DisplaySignalGroupAdditionalProps(
    ISignalGroup* pouLdfElement) {
...
int LDFAdditionalView::DisplayDiagProps(CLINSheduleTableItem& ouTableItem) {
  ...
  strVal.asprintf("{0x%X, 0x%X, 0x%X, 0x%X, 0x%X, 0x%X, 0x%X, 0x%X}",
                 ...
                 ouTableItem.m_chDataBytes[6], ouTableItem.m_chDataBytes[7]);
...
int LDFAdditionalView::DisplayAssignFrameIdProps(
    CLINSheduleTableItem& ouTableItem) {
  return 0;
}
...
int LDFAdditionalView::DisplaySignalAdditionalProps(ISignal* pouLdfElement) {
  ...
  if (nullptr != m_pHelpLabel) {
    ...
    m_pHelpLabel->setText(
        tr("Codings of signal <b>%1</b>").arg(strName.c_str()));
  }
  ...
  if (nullptr != ouCoding) {
    ...
    int nRow = 0;
    setRowCount(ouCompuMethods.m_ouPhysicalValueList.size() +
                ouCompuMethods.m_ouLogicalValueList.size());
...
int LDFAdditionalView::DisplayMasterAdditionalProps(IEcu* pouEcu) {
  ...
  if (m_pHelpLabel != nullptr) {
    ...
    m_pHelpLabel->setText(tr("Frames Associated with the Master Ecu <b>%1</b>")
                              .arg(strName.c_str()));
  }
...
int LDFAdditionalView::DisplayConfigFrames(
    IEcu* pouEcu, std::list<ConfigFrameDetails>& ouFrameUidList) {
  int nRow = 0;
  QList<QVariant> ouValueList;
  std::string strTemp;
  unsigned int unId = -1;
  ...
  strColumnList << "Frame Name"
                << "Frame Id"
                << "Length (Byte(s))";

  if ((ouEcuProperties.m_eEcuType == eSlave) &&
      (bAreEqual(ouEcuProperties.mSlaveProps.m_fProtocolVersion,
                 defVersion_2_0) == true)) {
  ...
  for (auto itr : ouFrameUidList) {
    ouValueList.clear();
    m_pouLdfCluster->GetElement(eFrameElement, itr.m_uidFrame,
                                (IElement**)&pouFrame);
    if (nullptr != pouFrame) {
      ...
      if (ouFrameProps.m_eLinFrameType == eLinDiagnosticFrame) {
        ...
        ouValueList.push_back(
            QVariant(GetString(ouFrameProps.m_unMsgSize, 10)));
        InsertRow(nRow, ouValueList);
        QVariant qVar;
        qVar.setValue(itr);
        ...
        if (bAreEqual(ouEcuProperties.mSlaveProps.m_fProtocolVersion,
                      defVersion_2_0) == true) {
          LineEditWidget* pLineWidget = new LineEditWidget(nRow, this);
          this->setCellWidget(nRow, 3, pLineWidget);
          if (LDFDatabaseManager::GetDatabaseManager()->bIsDisplayHexOn() ==
              false) {
          ...
          connect(pLineWidget, SIGNAL(textChanged(const QString&)), this,
                  SLOT(onConfigFrameEditChange(const QString&)));
        }
        QVariant qVar;
        qVar.setValue(itr);
        item(nRow, 0)->setData(Qt::UserRole, qVar);
...
void LDFAdditionalView::onConfigFrameEditChange(const QString& strText) {
  if (currentRow() >= 0 && m_pCurrentEcu != nullptr) {
    ...
    if (itrFrame != ouEcuProps.mSlaveProps.m_nConfiurableFrameIdList.end()) {
      ...
      // No clients need to know about this changes.
      LDFDatabaseManager::GetDatabaseManager()
          ->GetLDFCluster()
...
int LDFAdditionalView::DisplaySlaveAdditionalProps(IEcu* pouEcu) {
  ...
  if (m_pHelpLabel != nullptr) {
    std::string strName;
    pouEcu->GetName(strName);
    if (ouEcuProperties.mSlaveProps.m_fProtocolVersion == defVersion_1_3) {
      m_pHelpLabel->setText(
          tr("Frames Associated with the Ecu <b>%1</b>").arg(strName.c_str()));
      verticalHeader()->setSectionsMovable(false);
    } else {
      m_pHelpLabel->setText(
          tr("Configurable Frame List of Ecu <b>%1</b><br>Drag & Drop to "
             "change the order</br>")
              .arg(strName.c_str()));
...
int LDFAdditionalView::DisplayUnConditionalFrameProps(IFrame* pouFrame) {
  ...
  std::string strSigName, strPublishers, strSubscribers, strEncoding, strUnit,
      strCoding;
  unsigned int unId;
  ...
  if (nullptr != m_pHelpLabel) {
    ...
    m_pHelpLabel->setText(tr("Signals mapped for Unconditional Frame <b>%1</b>")
                              .arg(strName.c_str()));
  }
  ...
  for (auto itr : mapSignals) {
    ...
    // In dec always
    ouValueList.push_back(
        QVariant(GetString(ouSignalProps.m_unSignalSize, 10)));
...
int LDFAdditionalView::DisplayEventTrigProps(IFrame* pouFrame) {
  ...
  for (auto itr :
       ouFrameProps.m_ouLINEventTrigFrameProps.m_pouUnconditionalFrame) {
    IFrame* pouFrame = nullptr;
    m_pouLdfCluster->GetElement(eFrameElement, UID_ELEMENT(itr.first),
                                (IElement**)&pouFrame);
    if (nullptr != pouFrame) {
  ...
  std::string strFrameName, strPublishers, strSubscribers, strEncoding, strUnit,
      strCoding;
  unsigned int unId;
  CompuMethodProps ouCompuProps;
  ouCompuProps.m_eType = eInvalidProtocol;
  ...
  if (nullptr != m_pHelpLabel) {
    ...
    pouFrame->GetName(strName);
    m_pHelpLabel->setText(
        tr("Unconditional Frames associated to Eventtriggered Frame <b>%1</b>")
            .arg(strName.c_str()));
  }
...
int LDFAdditionalView::DisplaySporadicProps(IFrame* pouFrame) {
  ...
  for (auto itr :
       ouFrameProps.m_ouLINSporadicFrameProps.m_pouUnconditionalFrame) {
    IFrame* pouFrame = nullptr;
    m_pouLdfCluster->GetElement(eFrameElement, UID_ELEMENT(itr.first),
                                (IElement**)&pouFrame);
    if (nullptr != pouFrame) {
  ...
  if (nullptr != m_pHelpLabel) {
    std::string strName;
    pouFrame->GetName(strName);
    m_pHelpLabel->setText(tr("Frames associated to Sporadic Frame <b>%1</b>")
                              .arg(strName.c_str()));
  }
...
int LDFAdditionalView::DisplayDiagProps(IFrame* pouFrame) {
  ...
  if (nullptr != m_pHelpLabel) {
    ...
    pouFrame->GetName(strName);
    m_pHelpLabel->setText(
        tr("Diagnostic signals associated to Dignostic Frame <b>%1</b>")
            .arg(strName.c_str()));
  }
...
ERRORCODE LDFAdditionalView::nGetEncodingnUnit(LINCompuMethods ouCompuProps,
