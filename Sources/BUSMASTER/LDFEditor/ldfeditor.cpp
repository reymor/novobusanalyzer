...
LDFEditor::LDFEditor(std::string omstrLdfPath, QWidget* parent)
  ...
  connect(ui.m_pouLDFElementView, SIGNAL(itemSelectionChanged()), this,
          SLOT(OnitemElementViewSelected()));
  connect(ui.m_pouLDFNetworkView, SIGNAL(itemSelectionChanged()), this,
          SLOT(OnitemNetworkViewSelected()));

  connect(LDFDatabaseManager::GetDatabaseManager(), SIGNAL(DocumentModified()),
          this, SLOT(OnDocumentModified()));
...
void LDFEditor::OnDocumentModified() {
  bool bModified =
      LDFDatabaseManager::GetDatabaseManager()->bIsDocumentModified();
...
void LDFEditor::OnitemNetworkViewSelected() {
  if (ui.m_pouLDFNetworkView->selectedItems().size() > 0) {
    ...
    QTreeWidgetItem* selecedItem =
        ui.m_pouLDFNetworkView->selectedItems().at(0);

    if (nullptr != selecedItem) {
      TreeItemID ouTreeItemId =
          selecedItem->data(0, Qt::UserRole).value<TreeItemID>();
...
void LDFEditor::OnitemElementViewSelected() {
  if (ui.m_pouLDFElementView->selectedItems().size() > 0) {
    QTreeWidgetItem* selecedItem =
        ui.m_pouLDFElementView->selectedItems().at(0);
...
int LDFEditor::ConnectMenuActions(void) {
  QIcon newIcon;
  newIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/new.png",
                  QSize(16, 16));
  newIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/new.png",
                  QSize(22, 22));
  newIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/new.png",
                  QSize(32, 32));
  ui.actionNew->setIcon(newIcon);

  QIcon openIcon;
  openIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/open.png",
                   QSize(16, 16));
  openIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/open.png",
                   QSize(22, 22));
  openIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/open.png",
                   QSize(32, 32));
  ui.actionOpen->setIcon(openIcon);

  QIcon saveIcon;
  saveIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/save.png",
                   QSize(16, 16));
  saveIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/save.png",
                   QSize(22, 22));
  saveIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/save.png",
                   QSize(32, 32));
  ui.actionSave->setIcon(saveIcon);
  ui.actionSave->setDisabled(true);

  QIcon saveIconAs;
  saveIconAs.addFile(":/LDFEditorIcons/Resources/icons/16x16/saveas.png",
                     QSize(16, 16));
  saveIconAs.addFile(":/LDFEditorIcons/Resources/icons/22x22/saveas.png",
                     QSize(22, 22));
  saveIconAs.addFile(":/LDFEditorIcons/Resources/icons/32x32/saveas.png",
                     QSize(32, 32));
  ui.actionSave_As->setIcon(saveIconAs);
  ui.actionSave_As->setDisabled(true);

  QIcon hexIcon;
  hexIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/hex.png",
                  QSize(16, 16));
  hexIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/hex.png",
                  QSize(22, 22));
  hexIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/hex.png",
                  QSize(32, 32));
  ...
  QIcon recentIcon;
  recentIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/recent.png",
                     QSize(16, 16));
  recentIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/recent.png",
                     QSize(22, 22));
  recentIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/recent.png",
                     QSize(32, 32));
  ...
  connect(ui.actionSave_As, SIGNAL(triggered()), this,
          SLOT(onActionFileSaveAs()));
  connect(ui.action_About, SIGNAL(triggered()), this, SLOT(onActionAbout()));
  connect(ui.actionNewLIN_1_3, SIGNAL(triggered()), this,
          SLOT(onActionLIN1_3()));
  connect(ui.actionNewLIN_2_0, SIGNAL(triggered()), this,
          SLOT(onActionLIN2_0()));
  connect(ui.actionNewLIN_2_1, SIGNAL(triggered()), this,
          SLOT(onActionLIN2_1()));
  connect(ui.actionHex, SIGNAL(triggered()), this, SLOT(onActionHex()));
  connect(ui.actionPreview_LDF_File, SIGNAL(triggered()), this,
          SLOT(onActionPreviewFile()));
...
int LDFEditor::UpdatedRecentFileList() {
  ...
  int nMaxRecentFiles =
      LDFDatabaseManager::GetDatabaseManager()->GetMaxRecentFileCount();
  LDFDatabaseManager::GetDatabaseManager()->GetRecentFileList(
      strRecentFileList);

  int numRecentFiles = min((int)strRecentFileList.size(), (int)nMaxRecentFiles);

  int i = 0;
  for (auto itr = strRecentFileList.begin(); itr != strRecentFileList.end();
       itr++) {
...
int LDFEditor::onActionFileOpen(void) {
  ...
  QString strLDFFilePath = QFileDialog::getOpenFileName(
      this, tr("Open LDF File..."), "", tr("LDF Files (*.ldf)"));
...
int LDFEditor::OpenLDFFile(QString strLDFFilePath) {
  if ("" != strLDFFilePath) {
    ...
    if (file.exists() == false) {
      QMessageBox::critical(this, "Error", "File does not exists",
                            QMessageBox::Ok);
      return -1;
    }
    ERRORCODE ecResult =
        LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->LoadFromFile(
            strLDFFilePath.toStdString(), ouErrors, ouWarnings);

    m_pWarningTable->setRowCount(0);
    if (ecResult == EC_SUCCESS) {
      ICluster* pLdfCluster =
          LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster();
      if (nullptr != pLdfCluster) {
        ...
        if (!(bAreEqual(ouLINSettings.m_dProtocolVers, defVersion_1_3) ||
          ...
          ouParsingResults.m_strErrorDesc =
              "Invalid Protocol Version (Supported Versions 1.3, 2.0, 2.1)";
...
int LDFEditor::OnFileLoad(QString strLDFFilePath) {
  LDFDatabaseManager::GetDatabaseManager()->SetLDFFilePath(
      strLDFFilePath.toStdString());
  setTitle();
  m_strCurrentLDFFilePath = strLDFFilePath;
  if (strLDFFilePath.size() > 0) {
    // Empty file created for New Menu
    UpdatedRecentFileList();
    ui.actionSave->setDisabled(true);
    ui.actionSave_As->setDisabled(false);
    LDFDatabaseManager::GetDatabaseManager()->setDocumentModified(false);
...
void LDFEditor::setTitle() {
  ...
  strTitle +=
      LDFDatabaseManager::GetDatabaseManager()->GetLDFFilePath().c_str();
  if (LDFDatabaseManager::GetDatabaseManager()->bIsDocumentModified() == true) {
    strTitle += "*";
  }
...
int LDFEditor::onActionFileSave(void) {
  if (ValidateForSaving() == 0 && m_strCurrentLDFFilePath.length() > 0) {
    LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->SaveToFile(
        m_strCurrentLDFFilePath.toStdString());
    LDFDatabaseManager::GetDatabaseManager()->setDocumentModified(false);
    ui.actionSave_As->setDisabled(false);
    ui.actionSave->setDisabled(true);
    return 0;
...
int LDFEditor::ValidateForSaving() {
  ...
  if (ouErrors.size() > 0) {
    ...
    QMessageBox::StandardButton ret = QMessageBox::warning(
        this, "Warning",
        "Warnings found while saving the file.Do you Want to Continue to save?",
...
int LDFEditor::onActionFileSaveAs(void) {
  if (ValidateForSaving() == 0) {
    QString strLDFFilePath = QFileDialog::getSaveFileName(
        this, tr("Save LDF File"), "", tr("LDF Files (*.ldf)"));
    LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->SaveToFile(
        strLDFFilePath.toStdString());
    return 0;
  }
...
int LDFEditor::onActionLIN1_3(void) {
  ...
  CloseCurrentLDFFile();
  m_strCurrentLDFFilePath = QFileDialog::getSaveFileName(
      this, tr("New LDF File 1.3"), "", tr("LDF Files (*.ldf)"));
...
int LDFEditor::onActionLIN2_0(void) {
  ...
  CloseCurrentLDFFile();
  m_strCurrentLDFFilePath = QFileDialog::getSaveFileName(
      this, tr("New LDF File 2.0"), "", tr("LDF Files (*.ldf)"));
...
int LDFEditor::onActionLIN2_1(void) {
  ...
  CloseCurrentLDFFile();
  m_strCurrentLDFFilePath = QFileDialog::getSaveFileName(
      this, tr("New LDF File 2.1"), "", tr("LDF Files (*.ldf)"));
...
int LDFEditor::onActionHex(void) {
  LDFDatabaseManager::GetDatabaseManager()->SetDisplayHex(
      ui.actionHex->isChecked());
  return 0;
}
...
void LDFEditor::AddWarningsOfNewFile(double dVers) {
  ...
  ouRow.push_back("Warning");
  ouRow.push_back(
      "Valid LDF File should Contain Atleast one Unconditional Frame");
  ouRow.push_back("");
  m_pWarningTable->InsertRow(nRow, ouRow);
  nRow++;
...
int LDFEditor::nValidateForCluster(list<ParsingResults>& ouErrors,
                                   list<ParsingResults>& ouWarnings) {
  // 1. Ecu Validations
  std::map<UID_ELEMENT, IElement*> pElement;

  LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->GetElementList(
      eEcuElement, pElement);
  ...
  if (false == bSlaveFound) {
    ...
    ouPasringResult.m_ouErrorType = eError;
    ouPasringResult.m_strActionTaken = "";
    ouPasringResult.m_strErrorDesc =
        "Atleast One Slave should be Defined in LDF File";
    ouPasringResult.m_unErrorCode = 1;
    ouPasringResult.m_unLineNum = 0;
    ouErrors.push_back(ouPasringResult);
  }
  ...
  if (false == SignalFound) {
    ...
    ouPasringResult.m_strActionTaken = "";
    ouPasringResult.m_strErrorDesc =
        "Valid LDF File should Contain Atleast one signal";
    ouPasringResult.m_unErrorCode = 1;
    ouPasringResult.m_unLineNum = 0;
    ouWarnings.push_back(ouPasringResult);
  }
  ...
  if (false == TableFound) {
    ...
    ouPasringResult.m_strActionTaken = "";
    ouPasringResult.m_strErrorDesc =
        "Valid LDF File should Contain Atleast one Schedule Table";
    ouPasringResult.m_unErrorCode = 1;
    ouPasringResult.m_unLineNum = 0;
    ouWarnings.push_back(ouPasringResult);
  }
