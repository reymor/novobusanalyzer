...
MapSignalsDlg::MapSignalsDlg(UID_ELEMENT uidEcu, int nFrameLen,
                             std::map<UID_ELEMENT, SignalInstanse> mapSignals,
                             QWidget* parent) {
  IElement* pElement = nullptr;
  m_pouEcu = nullptr;
  m_nFrameLen = nFrameLen;
  if (EC_SUCCESS ==
      LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->GetElement(
          eEcuElement, uidEcu, &pElement)) {
  ...
  QIcon ouWindowIcon;
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/Signals.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/Signals.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/Signals.png",
                       QSize(16, 16));
    ...
    int nRowCount = 0;
    {
      ...
      for (auto itrSignal : mapSignals) {
        ...
        if (mapTempSignals.find(uidSignal) == mapTempSignals.end()) {
          ...
          ouColumns.push_back(
              QVariant(GetString(ouSigProps.m_unSignalSize, 10)));

          qVariant.setValue(itrSignal.first->GetUniqueId());

          ui.tableSignals->InsertRow(nRowCount, ouColumns);

          ui.tableSignals->item(nRowCount, defCOL_SIGNAL_NAME)
              ->setData(Qt::UserRole, qVariant);

          ui.tableSignals->item(nRowCount, defCOL_SIGNAL_NAME)
              ->setCheckState(Qt::Unchecked);
          auto itrMap =
              m_mapSignalUIDSigInst.find(itrSignal.first->GetUniqueId());
          ...
          ui.tableSignals->setCellWidget(nRowCount++, defCOL_START_BIT,
                                         pRawEdit);
          ouColumns.clear();
...
void MapSignalsDlg::onSelectionCancel() { reject(); }
...
ERRORCODE MapSignalsDlg::vCollectSelectedSignals(std::string& ouErrorString) {
  ...
  ouSigInstance.m_nStartBit = INVALID_DATA;
  ouSigInstance.m_nUpdateBitPos = INVALID_DATA;
  UID_ELEMENT uidSig;
  ...
  for (unsigned int unIndex = 0; unIndex < ui.tableSignals->rowCount();
       unIndex++) {
    pStartBit =
        (QLineEdit*)ui.tableSignals->cellWidget(unIndex, defCOL_START_BIT);

    if (nullptr != pStartBit) {
      if (ui.tableSignals->item(unIndex, defCOL_SIGNAL_NAME)->checkState() ==
          Qt::Checked) {
        qVariant = ui.tableSignals->item(unIndex, defCOL_SIGNAL_NAME)
                       ->data(Qt::UserRole);
        ...
        if (EC_FAILURE == vValidateSignalStartBit(uidSig, nFrameLengthInBits,
                                                  pStartBit->text(),
                                                  ouErrorString)) {
          return EC_FAILURE;
        }
  ...
  if (EC_FAILURE ==
      nValidateSignalOverlapping(nFrameLengthInBits, ouErrorString)) {
    return EC_FAILURE;
  }
...
void MapSignalsDlg::vGetSelectedSignals(
    std::map<UID_ELEMENT, SignalInstanse>& mapSignals) {
  mapSignals = m_mapSignalUIDSigInst;
}

ERRORCODE MapSignalsDlg::vValidateSignalStartBit(UID_ELEMENT uidSignal,
                                                 int nFrameLengthInBits,
                                                 QString strSigSB,
                                                 std::string& ouErrorString) {
  IElement* pSignal = nullptr;

  LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->GetElement(
      eSignalElement, uidSignal, &pSignal);
  ...
  if (nullptr != pSignal) {
    ...
    if (nStartBit < 0 || nStartBit > defMAX_SIGNAL_STARTBIT) {
      ...
      omStringStrm << defERROR << omSignalName.c_str()
                   << defSIGNAL_STARTBIT_INVALID << nStartBit;
    ...
    if (nStartBit >= nFrameLengthInBits) {
      std::ostringstream omStringStrm;
      omStringStrm << defERROR << omSignalName.c_str() << defERR_SIGNAL_STARTBIT
                   << nStartBit << defERR_FRAME_LENGTH
                   << (nFrameLengthInBits / 8) << defBYTES;
      ouErrorString = omStringStrm.str();
      return EC_FAILURE;
    }
    ...
    if (nEndBit >= nFrameLengthInBits) {
      std::ostringstream omStringStrm;
      omStringStrm << defERROR << omSignalName.c_str() << defERR_SIGNAL_OFFSET
                   << nStartBit << defHIPHEN << nEndBit << defERR_FRAME_LENGTH
                   << (nFrameLengthInBits / 8) << defBYTES;
      ouErrorString = omStringStrm.str();
      return EC_FAILURE;
    }
...
ERRORCODE MapSignalsDlg::nValidateSignalOverlapping(
    int nFrameLength, std::string& ouErrorString) {
  ...
  for (int nIndex = 0; nIndex < nCount; nIndex++) {
    ...
    if (ui.tableSignals->item(nIndex, defCOL_SIGNAL_NAME)->checkState() ==
        Qt::Checked) {
      uidSignal = ui.tableSignals->item(nIndex, defCOL_SIGNAL_NAME)
                      ->data(Qt::UserRole)
                      .value<UID_ELEMENT>();
      ...
      int nLength = GetUnsignedInt(
          ui.tableSignals->item(nIndex, defCOL_LENGTH)->text(), 10);

      // Signal End bit
      nSignalEndBit = nStartBit + (nLength - 1);

      for (int nStartIndex = nStartBit; nStartIndex <= nSignalEndBit;
           nStartIndex++) {
        if (nArrSignalBit[nStartIndex] == 1) {
          omStringStrm << defSIGNAL << "<B>" << omSigName.toStdString()
                       << "</B>" << defSIGNAL_OVERLAPPING;
          ouErrorString = omStringStrm.str();
          return EC_FAILURE;
        }
