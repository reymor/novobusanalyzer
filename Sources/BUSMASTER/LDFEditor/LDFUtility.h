#ifndef BUSMASTER_LDFEDITOR_LDFUTILITY_H_
#define BUSMASTER_LDFEDITOR_LDFUTILITY_H_
...
#define VALIDATE_AND_RETURN(x) \
  if (nullptr == x) return EC_FAILURE;
#define VALIDATE_POINTER(x) \
  if (nullptr == x) return;
...
enum eTreeElementType { eTreeHeader, eLdfItem };
...
#endif  // BUSMASTER_LDFEDITOR_LDFUTILITY_H_