...
CodingDlg::CodingDlg(ICluster* pouCluster, ICoding** pouCoding, bool bNew,
  ...
  connect(ui.buttonAddPhysical, SIGNAL(clicked()), this,
          SLOT(AddPhysicalRow()));
  connect(ui.buttonDeleteLogical, SIGNAL(clicked()), this,
          SLOT(DeleteLogicalRow()));
  connect(ui.buttonDeletePhysical, SIGNAL(clicked()), this,
          SLOT(DeletePhysicalRow()));
  connect(ui.tableLogicalValues,
          SIGNAL(ui.tableLogicalValues->itemSelectionChanged()), this,
          SLOT(ItemSelectionChangeLogical()));
  connect(ui.buttonOption, SIGNAL(accepted()), this, SLOT(onSelectionOk()));
  connect(ui.buttonOption, SIGNAL(rejected()), this,
          SLOT(onSelecetionCancel()));

  QIcon ouWindowIcon;
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/16x16/Coding.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/22x22/Coding.png",
                       QSize(16, 16));
  ouWindowIcon.addFile(":/LDFEditorIcons/Resources/icons/32x32/Coding.png",
                       QSize(16, 16));
...
void CodingDlg::AddPhysicalRow() {
  ...
  LineEditWidget* pFactorEdit =
      new LineEditWidget(nRowCount, ui.tablePhysicalValues);
  pFactorEdit->setValidator(new QDoubleValidator());
  pFactorEdit->setFrame(false);

  LineEditWidget* pOffset =
      new LineEditWidget(nRowCount, ui.tablePhysicalValues);
  ...
  ui.tablePhysicalValues->setCurrentItem(
      ui.tableLogicalValues->item(nRowCount, 0));
...
void CodingDlg::AddLogicalRow() {
  ...
  ui.tableLogicalValues->setCurrentItem(
      ui.tableLogicalValues->item(nRowCount, 0));
...
void CodingDlg::vPrepareUiForEdit() {
  ...
  if (nullptr != pCoding) {
    ...
    for (auto itr : ouCompuMethodsProps.m_ouPhysicalValueList) {
      ouColumns.clear();
      LineEditWidget* pFactorEdit =
          new LineEditWidget(nRow, ui.tablePhysicalValues);
      pFactorEdit->setValidator(new QDoubleValidator());
      pFactorEdit->setFrame(false);
...
void CodingDlg::onSelectionOk() {
  ...
  if (m_strCodingMap.find(strName) != m_strCodingMap.end()) {
    QMessageBox::critical(this, "Error", "Duplicate Coding Name",
                          QMessageBox::Ok);
  }
...
int CodingDlg::FillPhysicalValues(LINCompuMethods& ouCompuMethods) {
  ...
  for (int i = 0; i < nRowCount; i++) {
    LineEditWidget* pOffsetWidget =
        (LineEditWidget*)ui.tablePhysicalValues->cellWidget(i, 0);
    LineEditWidget* pFactorWidget =
        (LineEditWidget*)ui.tablePhysicalValues->cellWidget(i, 1);
    LineEditWidget* pMinWidget =
        (LineEditWidget*)ui.tablePhysicalValues->cellWidget(i, 2);
    LineEditWidget* pMaxWidget =
        (LineEditWidget*)ui.tablePhysicalValues->cellWidget(i, 3);
    LineEditWidget* pUnitWidget =
        (LineEditWidget*)ui.tablePhysicalValues->cellWidget(i, 4);
...
void CodingDlg::FillLogicalValues(LINCompuMethods& ouCompuMethods) {
  ...
  std::string strDesc;
  for (int i = 0; i < nRowCount; i++) {
    LineEditWidget* pWidget =
        (LineEditWidget*)ui.tableLogicalValues->cellWidget(i, 0);
    if (nullptr != pWidget) {
...
void CodingDlg::onSelecetionCancel() { reject(); }