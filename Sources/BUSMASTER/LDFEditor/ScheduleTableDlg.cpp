...
ScheduleTableDlg::ScheduleTableDlg(IScheduleTable** pouScheduleTable,
  ...
  connect(ui.tableFrames, SIGNAL(itemSelectionChanged()), this,
          SLOT(onFrameSelectionChange()));
  connect(ui.btnAdd, SIGNAL(clicked(bool)), this, SLOT(onButtonClickAdd(bool)));
  connect(ui.btnDelete, SIGNAL(clicked(bool)), this,
          SLOT(onButtonClickDelete(bool)));
  connect(ui.tableFrames, SIGNAL(onItemDragged(int, int, int)), this,
          SLOT(onScheduleItemDragged(int, int, int)));

  ui.tableFrames->verticalHeader()->setDefaultSectionSize(24);
  vPrepareValidations();
  vPrepareUI();
  QIcon ouWindowIcon;
  ouWindowIcon.addFile(
...
void ScheduleTableDlg::onScheduleItemDragged(int nLogical, int nOld, int nNew) {
  ...
  std::list<CLINSheduleTableItem> tempScheduleTableItem;
  int nRows = ui.tableFrames->rowCount();
  CLINSheduleTableItem ouTableItem;
  for (int unIndex = 0; unIndex < nRows; unIndex++) {
    ...
    ouTableItem = pItem->data(Qt::UserRole).value<CLINSheduleTableItem>();
...
void ScheduleTableDlg::vPrepareUIForEditMode() {
  ...
  pScheduleTable->GetProperties(ouScheduleTableProps);
  QVariant qVar;
  ui.tableFrames->setRowCount(
      ouScheduleTableProps.m_ouCLINSheduleTableItem.size());
  ...
  vUpdateScheduleTableItems(ouScheduleTableProps.m_ouCLINSheduleTableItem);
}

void ScheduleTableDlg::vUpdateScheduleTableItems(
    std::list<CLINSheduleTableItem>& lstScheduleTableItem) {
  IFrame* pouMsg = nullptr;
  ...
  for (auto itr : lstScheduleTableItem) {
    m_pouLDFCluster->GetElement(eFrameElement, itr.m_nFrameId,
                                (IElement**)&pouMsg);
    ...
    else if (itr.m_eDiagType >= eLIN_SID_ASSIGN_FRAME_ID &&
             itr.m_eDiagType <= eLIN_SID_READ_BY_IDENTIFIER) {
      strFrameType = "Master Request";
    ...
    connect(pLineEdit, SIGNAL(textChanged(const QString&)), this,
            SLOT(onDelayEditChange(const QString&)));

    connect(pComboBox, SIGNAL(onScheduleComboChange(int, int)), this,
            SLOT(onComboSelectionChange(int, int)));
    ouColumns.clear();
  }
}
...
void ScheduleTableDlg::onDelayEditChange(const QString& str) {
  ...
  if (nRow >= 0) {
    QVariant qVar;
    CLINSheduleTableItem ouSchedTableItem = ui.tableFrames->item(nRow, 0)
                                                ->data(Qt::UserRole)
                                                .value<CLINSheduleTableItem>();
    ouSchedTableItem.m_dDelay =
        ((LineEditWidget*)ui.tableFrames->cellWidget(nRow, 3))
            ->text()
            .toDouble();
...
ERRORCODE ScheduleTableDlg::vFillFramesCombo(int nRow,
                                             ScheduleComboWidget** pComboBox) {
  ...
  for (auto itrFrame : mapFrames) {
    ...
    if (ouFrameProps.m_eLinFrameType == eLinDiagnosticFrame) {
      if (ouFrameProps.m_ouLINDiagnosticFrameProps.m_eDiagType ==
    ...
    ecCode = EC_SUCCESS;
    pTempComboBox->addItem(QString::fromStdString(strFrameName),
                           QVariant(uidFrame));
  }
...
eScheduleCommand_Type ScheduleTableDlg::vGetFrameType(
    eLinFrameType ouFrameType, std::string omstrFrameName) {
  switch (ouFrameType) {
    case eLinUnconditionalFrame:
...
int ScheduleTableDlg::nValidateValues() {
  ...
  if (strName == "") {
    QMessageBox::critical(this, "Error", "Name Should Not Be Empty",
                          QMessageBox::Ok);
    return 1;
  }
  if (m_TableNameMap.find(strName) != m_TableNameMap.end()) {
    QMessageBox::critical(this, "Error", "Name Already Exists",
                          QMessageBox::Ok);
    return 1;
  }
  return 0;
...
void ScheduleTableDlg::OnButtonClickOk() {
  if (0 == nValidateValues()) {
    ...
    for (int i = 0; i < nRowCount; i++) {
      CLINSheduleTableItem ouTableItem = ui.tableFrames->item(i, 0)
                                             ->data(Qt::UserRole)
                                             .value<CLINSheduleTableItem>();
      LineEditWidget* pDelayEdit =
          (LineEditWidget*)ui.tableFrames->cellWidget(i, 3);
      if (nullptr != pDelayEdit) {
        ouTableItem.m_dDelay = pDelayEdit->text().toDouble();
      } else {
        ouTableItem.m_dDelay = defSCHEDULE_DELAY;
      }
      ouTableProps.m_ouCLINSheduleTableItem.push_back(ouTableItem);
    }

    if (m_ouMode == eEdit) {
      pouTable = *m_pouSchedTable;
    } else {
      m_pouLDFCluster->CreateElement(eScheduleTableElement,
                                     (IElement**)&pouTable);
...
void ScheduleTableDlg::onFrameSelectionChange() { vPopulateSelFrameDetails(); }

void ScheduleTableDlg::vPopulateSelFrameDetails() {
  int nSelectedRow = ui.tableFrames->currentRow();

  if (nSelectedRow >= 0) {
    ScheduleComboWidget* pComboWidget =
        (ScheduleComboWidget*)ui.tableFrames->cellWidget(nSelectedRow, 0);
...
void ScheduleTableDlg::vPopulateSelFrameDetails(int nIndex) {
  ...
  if (nSelectedRow >= 0) {
    ...
    UID_ELEMENT uidElement =
        pComboWidget->itemData(nIndex, Qt::UserRole).value<UID_ELEMENT>();

    CLINSheduleTableItem omSchedItem = ui.tableFrames->item(nSelectedRow, 0)
                                           ->data(Qt::UserRole)
                                           .value<CLINSheduleTableItem>();
    ...
    if (nullptr != pFrame) {
      ...
      if (ouFrameProps.m_eLinFrameType == eLinUnconditionalFrame ||
          ouFrameProps.m_eLinFrameType == eEventTriggered ||
          ouFrameProps.m_eLinFrameType == eSporadic) {
        omSchedItem.m_eDiagType = eLIN_NORMAL_FRAME_ID;
        ui.tableFrames->item(nSelectedRow, 2)
            ->setText(nGetFrameLength(ouFrameProps));
      } else {
...
void ScheduleTableDlg::vPopulatePropertyView(
    CLINSheduleTableItem& ouSchedTableItem) {
  ...
  switch (ouSchedTableItem.m_eDiagType) {
    ...
    case eLIN_NORMAL_FRAME_ID:
      m_pouLDFCluster->GetElement(eFrameElement, ouSchedTableItem.m_nFrameId,
                                  &pElement);
...
void ScheduleTableDlg::vPopulateAssignNADProperties(
    CLINSheduleTableItem& ouSchedTableItem) {
  ...
  if (EC_SUCCESS ==
      m_pouLDFCluster->GetElement(eEcuElement, unECUId, &pElement)) {
    pElement->GetName(strECUName);
    pComboBox->setCurrentText(QString::fromStdString(strECUName));
  }

  connect(pComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onComboAssignNADNodeChange(int)));
  ...
  omStrStream
      << "["
      << GetString(ouEcuProps.mSlaveProps.m_nInitialNAD).toStdString().c_str()
      << "," << GetString(nPCI).toStdString().c_str() << ","
      << GetString(nSID).toStdString().c_str() << ","
      << GetString(nLSBSupplierId).toStdString().c_str() << ","
      << GetString(nMSBSupplierId).toStdString().c_str() << ","
      << GetString(nLSBFunctionId).toStdString().c_str() << ","
      << GetString(nMSBFunctionId).toStdString().c_str() << ","
      << GetString(ouEcuProps.mSlaveProps.m_nConfiguredNAD)
             .toStdString()
             .c_str()
      << "]";
  ouColumns.clear();
...
void ScheduleTableDlg::onCondNADEditChange(const QString& strText) {
  if (ui.tableFrames->currentRow() >= 0) {
    QVariant qVar;
    CLINSheduleTableItem ouSchedTableItem =
        ui.tableFrames->item(ui.tableFrames->currentRow(), 0)
            ->data(Qt::UserRole)
            .value<CLINSheduleTableItem>();
    ouSchedTableItem.m_chDataBytes[0] =
        GetUnsignedInt(((LineEditWidget*)ui.tableProperties->cellWidget(4, 1))
                           ->text());  // nad
    ouSchedTableItem.m_chDataBytes[3] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(0, 1))->text());  // id
    ouSchedTableItem.m_chDataBytes[4] =
        GetUnsignedInt(((LineEditWidget*)ui.tableProperties->cellWidget(1, 1))
                           ->text());  // byte
    ouSchedTableItem.m_chDataBytes[5] =
        GetUnsignedInt(((LineEditWidget*)ui.tableProperties->cellWidget(2, 1))
                           ->text());  // mask
    ouSchedTableItem.m_chDataBytes[6] =
        GetUnsignedInt(((LineEditWidget*)ui.tableProperties->cellWidget(3, 1))
                           ->text());  // invert
    ouSchedTableItem.m_chDataBytes[7] =
        GetUnsignedInt(((LineEditWidget*)ui.tableProperties->cellWidget(5, 1))
                           ->text());  // new nad

    ouSchedTableItem.m_eDiagType = eLIN_SID_COND_CHANGE_NAD;
    qVar.setValue(ouSchedTableItem);
    ui.tableFrames->item(ui.tableFrames->currentRow(), 0)
        ->setData(Qt::UserRole, qVar);

    int nPCI = 0x06, nSID = 0xB3;
    std::ostringstream omStrStream;
    omStrStream << "["
                << GetString(ouSchedTableItem.m_chDataBytes[0]).toStdString()
                << "," << GetString(nPCI).toStdString().c_str() << ","
                << GetString(nSID).toStdString().c_str() << ","
                << GetString(ouSchedTableItem.m_chDataBytes[3]).toStdString()
                << ","
    ...
    ui.tableProperties->item(6, 1)->setText(
        QString::fromStdString(omStrStream.str().c_str()));
  }
}

void ScheduleTableDlg::vPopulateReadByIdentifier(
    CLINSheduleTableItem& ouSchedTableItem) {
  std::map<UID_ELEMENT, IElement*> mapEcus;
  std::string strName;
  ui.tableProperties->setRowCount(0);
  ...
  for (int nIndex = 0; nIndex < 6; nIndex++) {
    connect(pLineEdit[nIndex], SIGNAL(textChanged(const QString&)), this,
            SLOT(onReadByIdEditChange(const QString&)));
  }
}

void ScheduleTableDlg::vPopulateFreeFormat(
    CLINSheduleTableItem& ouSchedTableItem) {
  std::map<UID_ELEMENT, IElement*> mapEcus;
  std::string strName;
  ui.tableProperties->setRowCount(0);
  ...
  for (int nIndex = 0; nIndex < 8; nIndex++) {
    connect(pLineEdit[nIndex], SIGNAL(textChanged(const QString&)), this,
            SLOT(onFreeFormatEditChange(const QString&)));
  }
}
...
void ScheduleTableDlg::onReadByIdEditChange(const QString&) {
  if (ui.tableFrames->currentRow() >= 0) {
    ...
    ouSchedTableItem.m_chDataBytes[0] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(0, 1))->text());
    ouSchedTableItem.m_chDataBytes[1] = nPCI;
    ouSchedTableItem.m_chDataBytes[2] = 0xB2;
    ouSchedTableItem.m_chDataBytes[3] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(1, 1))->text());
    ouSchedTableItem.m_chDataBytes[4] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(2, 1))->text());
    ouSchedTableItem.m_chDataBytes[5] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(3, 1))->text());
    ouSchedTableItem.m_chDataBytes[6] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(4, 1))->text());
    ouSchedTableItem.m_chDataBytes[7] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(5, 1))->text());
...
void ScheduleTableDlg::onFreeFormatEditChange(const QString&) {
  if (ui.tableFrames->currentRow() >= 0) {
    ...
    ouSchedTableItem.m_chDataBytes[0] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(0, 1))->text());
    ouSchedTableItem.m_chDataBytes[1] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(1, 1))->text());
    ouSchedTableItem.m_chDataBytes[2] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(2, 1))->text());
    ouSchedTableItem.m_chDataBytes[3] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(3, 1))->text());
    ouSchedTableItem.m_chDataBytes[4] = GetUnsignedInt(
    ...
    ouSchedTableItem.m_chDataBytes[5] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(5, 1))->text());
    ouSchedTableItem.m_chDataBytes[6] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(6, 1))->text());
    ouSchedTableItem.m_chDataBytes[7] = GetUnsignedInt(
        ((LineEditWidget*)ui.tableProperties->cellWidget(7, 1))->text());
    ...
    ui.tableProperties->item(8, 1)->setText(
        QString::fromStdString(omStrStream.str().c_str()));
  }
}

void ScheduleTableDlg::vPopulateCondChangeNAD(
    CLINSheduleTableItem& ouSchedTableItem) {
  std::map<UID_ELEMENT, IElement*> mapEcus;
  std::string strName;
  ui.tableProperties->setRowCount(0);
  ...
  for (int nIndex = 0; nIndex < 6; nIndex++) {
    connect(pLineEdit[nIndex], SIGNAL(textChanged(const QString&)), this,
            SLOT(onCondNADEditChange(const QString&)));
  }
}

void ScheduleTableDlg::onComboAssignNADNodeChange(int nIndex) {
  UID_ELEMENT uidECU =
      ((ScheduleComboWidget*)ui.tableProperties->cellWidget(0, 1))
          ->currentData(Qt::UserRole)
          .value<UID_ELEMENT>();

  IElement* pElement = nullptr;
  if (EC_SUCCESS ==
      m_pouLDFCluster->GetElement(eEcuElement, uidECU, &pElement)) {
    LinEcuProps ouEcuProps;
    ouEcuProps.m_eEcuType = eEcuNone;
    ((IEcu*)pElement)->GetProperties(ouEcuProps);
    ui.tableProperties->item(1, 1)->setText(
        GetString(ouEcuProps.mSlaveProps.m_nInitialNAD));
    ui.tableProperties->item(2, 1)->setText(
        GetString(ouEcuProps.mSlaveProps.m_nSupplierId));

    ui.tableProperties->item(3, 1)->setText(
        GetString(ouEcuProps.mSlaveProps.m_nFunctionId));

    ui.tableProperties->item(4, 1)->setText(
        GetString(ouEcuProps.mSlaveProps.m_nConfiguredNAD));
    ...
    omStrStream
        ...
        << "," << GetString(nPCI).toStdString().c_str() << ","
        << GetString(nSID).toStdString().c_str() << ","
        << GetString(ouEcuProps.mSlaveProps.m_nSupplierId).toStdString().c_str()
        << "," << GetString(nMSBSupplierId).toStdString().c_str() << ","
    ...
    CLINSheduleTableItem ouSchedItem =
        ui.tableFrames->item(ui.tableFrames->currentRow(), 0)
            ->data(Qt::UserRole)
            .value<CLINSheduleTableItem>();
    ...
    ouSchedItem.m_dDelay = ui.tableFrames->item(ui.tableFrames->currentRow(), 3)
                               ->text()
                               .toDouble();
    QVariant qVar;
...
void ScheduleTableDlg::onComboAssignIdRangeNodeChange(int nIndex) {
  ...
  if (EC_SUCCESS ==
    ...
    for (; itr != ouEcuProps.mSlaveProps.m_nConfiurableFrameIdList.end();
         itr++) {
      if (nIndex >= 4) {
        break;
      }
      if (EC_SUCCESS == m_pouLDFCluster->GetElement(eFrameElement,
                                                    itr->m_uidFrame, &pFrame)) {
        unFrameId = 0;
    ...
    ((LineEditWidget*)ui.tableProperties->cellWidget(1, 1))
        ->setText((GetString(nPID[0], false).toStdString().c_str()));
    ((LineEditWidget*)ui.tableProperties->cellWidget(2, 1))
        ->setText(GetString(nPID[1], false).toStdString().c_str());

    ((LineEditWidget*)ui.tableProperties->cellWidget(3, 1))
        ->setText(GetString(nPID[2], false).toStdString().c_str());

    ((LineEditWidget*)ui.tableProperties->cellWidget(4, 1))
        ->setText(GetString(nPID[3], false).toStdString().c_str());
    ((LineEditWidget*)ui.tableProperties->cellWidget(5, 1))
        ->setText(GetString(nStartIndex, false).toStdString().c_str());
    ...
    omStrStream << "["
                ...
                << "," << GetString(nPCI).toStdString().c_str() << ","
                << GetString(nSID).toStdString().c_str() << ","
                << GetString(nStartIndex).toStdString().c_str() << ","
                << GetString(nPID[0]).toStdString().c_str() << ","
  ...
  if (EC_SUCCESS ==
    ...
    omStrStream << "["
                << GetString(ouEcuProps.mSlaveProps.m_nConfiguredNAD)
                       .toStdString()
                       .c_str()
                << "," << GetString(nVal).toStdString().c_str() << ","
...
void ScheduleTableDlg::onComboSaveConfigChange(int nIndex) {
  ...
  if (EC_SUCCESS ==
    ...
    omStrStream << "["
                ...
                << "," << GetString(nVal).toStdString().c_str() << ","
                << GetString(nVal1).toStdString().c_str() << ","
                << GetString(nVal2).toStdString().c_str() << ","
                << GetString(nVal2).toStdString().c_str() << ","
                << GetString(nVal2).toStdString().c_str() << ","
                << GetString(nVal2).toStdString().c_str() << ","
                << GetString(nVal2).toStdString().c_str() << "]";
...
void ScheduleTableDlg::vPopulateDataDump(
    CLINSheduleTableItem& ouSchedTableItem) {
  std::map<UID_ELEMENT, IElement*> mapEcus;
  std::string strName;
  ui.tableProperties->setRowCount(0);
  ...
  connect(pComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onComboDataDumpChange(int)));
  IEcu* pEcu = (IEcu*)pElement;
  ...
  for (int nIndex = 0; nIndex < 5; nIndex++) {
    connect(pLineEdit[nIndex], SIGNAL(textChanged(const QString&)), this,
            SLOT(onAssignDataDumpEditChange(const QString&)));
  }

  ouSchedTableItem.m_nNode = unECUId;
}

void ScheduleTableDlg::vPopulateSaveConfig(
    CLINSheduleTableItem& ouSchedTableItem) {
  std::map<UID_ELEMENT, IElement*> mapEcus;
  std::string strName;
  ui.tableProperties->setRowCount(0);
  ...
  connect(pComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onComboSaveConfigChange(int)));
  IEcu* pEcu = (IEcu*)pElement;
...
void ScheduleTableDlg::vPopulateAssignFrameIdRange(
    CLINSheduleTableItem& ouSchedTableItem) {
  std::map<UID_ELEMENT, IElement*> mapEcus;
  std::string strName;
  ui.tableProperties->setRowCount(0);
  ...
  ouColumns.push_back(
      QVariant(GetString(nPID[0], false).toStdString().c_str()));
  ui.tableProperties->InsertRow(1, ouColumns);
  ...
  for (int nIndex = 0; nIndex < 5; nIndex++) {
    connect(pLineEdit[nIndex], SIGNAL(textChanged(const QString&)), this,
            SLOT(onAssignFrameIdRangeEditChange(const QString&)));
  }
  connect(pComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onComboAssignIdRangeNodeChange(int)));
}
...
void ScheduleTableDlg::onAssignFrameIdRangeEditChange(const QString&) {
  if (ui.tableFrames->currentRow() >= 0) {
    ...
    if (EC_SUCCESS ==
      ...
      ouSchedItem.m_chDataBytes[3] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(5, 1))->text());
      ouSchedItem.m_chDataBytes[4] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(1, 1))->text());
      ouSchedItem.m_chDataBytes[5] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(2, 1))->text());
      ouSchedItem.m_chDataBytes[6] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(3, 1))->text());
      ouSchedItem.m_chDataBytes[7] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(4, 1))->text());
...
void ScheduleTableDlg::onAssignFrameIdEditChange(const QString&) {
  if (ui.tableFrames->currentRow() >= 0) {
    ...
    if (EC_SUCCESS ==
      ...
      ouSchedItem.m_chDataBytes[0] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(5, 1))->text());
      ouSchedItem.m_chDataBytes[1] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(1, 1))->text());
      ouSchedItem.m_chDataBytes[2] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(2, 1))->text());
      ouSchedItem.m_chDataBytes[3] = GetUnsignedInt(
          ((LineEditWidget*)ui.tableProperties->cellWidget(3, 1))->text());
      ouSchedItem.m_chDataBytes[4] = GetUnsignedInt(
...
void ScheduleTableDlg::onComboAssignFrameIdFrameChange(int nIndex) {
  UID_ELEMENT uidFrame = ((QComboBox*)ui.tableProperties->cellWidget(0, 1))
                             ->currentData(Qt::UserRole)
                             .value<UID_ELEMENT>();
  UID_ELEMENT uidECU = ((QComboBox*)ui.tableProperties->cellWidget(1, 1))
                           ->currentData(Qt::UserRole)
                           .value<UID_ELEMENT>();
  ...
  if (INVALID_UID_ELEMENT != uidFrame) {
    ...
    int nPid = 0xFF;
    if (EC_SUCCESS ==
        m_pouLDFCluster->GetElement(eFrameElement, uidFrame, &pFrame)) {
      unsigned int unFrameId;
      ((IFrame*)pFrame)->GetFrameId(unFrameId);
    ...
    if (EC_SUCCESS == m_pouLDFCluster->GetElement(eEcuElement, uidECU, &pECU)) {
      ...
      ouSchedItem.m_dDelay =
          ui.tableFrames->item(ui.tableFrames->currentRow(), 3)
              ->text()
              .toDouble();
      QVariant qVar;
      qVar.setValue(ouSchedItem);
...
void ScheduleTableDlg::onComboAssignFrameIdNodeChange(int nIndex) {
  ...
  int nFramePID = 0xFF;
  UID_ELEMENT uidECU =
      ((ScheduleComboWidget*)ui.tableProperties->cellWidget(1, 1))
          ->currentData(Qt::UserRole)
          .value<UID_ELEMENT>();
  ...
  if (EC_SUCCESS ==
    ...
    (ui.tableProperties->item(2, 1))
        ->setText(GetString(ouEcuProps.mSlaveProps.m_nSupplierId));
    (ui.tableProperties->item(3, 1))->setText(GetString(nFramePID));
...
ERRORCODE ScheduleTableDlg::vPopulateAssignFrameId(
    CLINSheduleTableItem& ouSchedTableItem) {
  std::map<UID_ELEMENT, IElement*> mapEcus;
  std::map<UID_ELEMENT, IElement*> mapFrames;
  ...
  for (auto itrFrame : mapFrames) {
    ...
    if (props.m_eLinFrameType == eLinDiagnosticFrame ||
        props.m_eLinFrameType == eSporadic) {
      continue;
    }
  ...
  if (pFrameComboBox->count() <= 0) {
    QMessageBox::critical(
        this, "Error",
        "Cannot create Assign Frame Id as there are no frames available",
        QMessageBox::Ok);
    ((QComboBox*)ui.tableFrames->cellWidget(nSelectedRow, 0))
        ->setCurrentText("MasterReq");
  ...
  if (ouSchedTableItem.m_chDataBytes[7] != 0xFF &&
      ouSchedTableItem.m_chDataBytes[7] != INVALID_UID_ELEMENT) {
  ...
  connect(pComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onComboAssignFrameIdNodeChange(int)));
  connect(pFrameComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onComboAssignFrameIdFrameChange(int)));
  IEcu* pEcu = (IEcu*)pElement;
