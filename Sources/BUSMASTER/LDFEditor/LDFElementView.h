#ifndef BUSMASTER_LDFEDITOR_LDFELEMENTVIEW_H_
#define BUSMASTER_LDFEDITOR_LDFELEMENTVIEW_H_
...
class LDFElementView : public QTreeWidget, public INotifyClusterChange {
 ...
 private:
  ...
  void vPopulateLinElement(QTreeWidgetItem* pTopTreeItem, eClusterElementType,
  ...
  void showContextMenu(QMenu& ouMenu, QTreeWidgetItem* item,
                       const QPoint& globalPos);

  void vHandleElementChange(eAction, UID_ELEMENT, void*);

  void vCreateFrameElement(IFrame*, QTreeWidgetItem*);
  void vCreateSporadicFrameElement(IFrame* pouFrame,
                                   QTreeWidgetItem* pParentItem);
  void vCreateScheduleTable(IScheduleTable* pouSchedTable,
                            QTreeWidgetItem* pouParentTree);
  void vCreateSignalGroup(ISignalGroup* pouSignalGroup,
                          QTreeWidgetItem* pouParentTree);

  void DisplayElementPopupMenu(const QPoint& point, QTreeWidgetItem* pTreeItem,
                               UID_ELEMENT);
  void DisplayHeaderPopupMenu(const QPoint& point, QTreeWidgetItem* pTreeItem,
                              UID_ELEMENT);
  ...
  // helpers
  int nGetDigFrameTreeWidgetItem(IFrame* pDigFrame,
                                 QTreeWidgetItem*& pouTreeWidget);
  void OnSlaveNodeDelete(UID_ELEMENT);
  void MapNodeToSignals(IFrame*, IEcu* pouEcu, eDIR);
};

#endif  // BUSMASTER_LDFEDITOR_LDFELEMENTVIEW_H_