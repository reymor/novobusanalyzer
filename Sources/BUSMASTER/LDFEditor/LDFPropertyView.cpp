...
int LDFPropertyView::SetAdditionalPropsView(
    LDFAdditionalView* pouLDFAdditionalView) {
  m_pouLDFAdditionalView = pouLDFAdditionalView;
  return 0;
}

void LDFPropertyView::onTreeItemClicked(QTreeWidgetItem* pouTreeItem, int nId) {
  TreeItemID uidElement =
      pouTreeItem->data(0, Qt::UserRole).value<TreeItemID>();
  if (uidElement.m_eTreeElementType == eLdfItem &&
      uidElement.m_uidElement != 0) {
    ICluster* pouCluster = m_pouLDFDatabaseManager->GetLDFCluster();
    IElement* pouElement;

    VALIDATE_POINTER(pouCluster);
    pouCluster->GetElement(pouCluster->GetElementType(uidElement.m_uidElement),
                           uidElement.m_uidElement, &pouElement);
...
void LDFPropertyView::OnitemSelectionChanged() {
  ...
  for (auto index : indexList) {
    ...
    if (m_pouLDFAdditionalView != nullptr && m_pouCurrentTable != nullptr) {
      m_pouLDFAdditionalView->DisplayScedulingTableAdditionalProps(
          m_pouCurrentTable, row);
    }
    break;
  }
}
...
int LDFPropertyView::DisplayNodeCompositionDetails(IElement* pouLdfElement) {
  ...
  for (auto itrNodeConfig : lstCompNodes) {
    ouRowList.clear();
    if (EC_SUCCESS == m_pouLdfCluster->GetElement(
                          eEcuElement, itrNodeConfig.m_uidNode, &pECU)) {
  ...
  for (auto itr : ouScheduleTableProps.m_ouCLINSheduleTableItem) {
    ...
    if (0 == hr) {
      if (itr.m_eDiagType == eLIN_NORMAL_FRAME_ID ||
          itr.m_eDiagType == eLIN_MASTER_FRAME_ID ||
...
int LDFPropertyView::DisplaySignalElementProps(IElement* pouLdfElement) {
  ...
  QString strTemp =
      (ouSignalProps.m_ouSignalType == eSignalNormal) ? "Normal" : "Diagnostic";
...
int LDFPropertyView::DisplaySlaveDetails(IEcu* pouLdfElement) {
  ...
  if (ouEcuProps.mSlaveProps.m_fProtocolVersion == defVersion_1_3) {
    ...
    ouRowList.clear();
             ...
             ouEcuProps.mSlaveProps.m_fProtocolVersion == defVersion_2_0) {
    ...
    if (ouEcuProps.mSlaveProps.m_nRespErrSignal != INVALID_UID_ELEMENT) {
      IElement* pSignal;
      m_pouLdfCluster->GetElement(
          eSignalElement, ouEcuProps.mSlaveProps.m_nRespErrSignal, &pSignal);
...
int LDFPropertyView::GetUnconditionalFrames(std::list<IFrame*>& lstFrames,
...
int LDFPropertyView::DisplayUnConditionalFrameDetails(IFrame* pouFrame) {
  ...
  ouValueList.push_back(
      QVariant(GetString(ouFrameProps.m_unMsgSize, 10)));  // Size always dec
  InsertRow(nRow++, ouValueList);
