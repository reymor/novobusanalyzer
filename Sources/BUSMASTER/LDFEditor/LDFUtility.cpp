...
std::string g_strEdiagType[eLIN_SID_ALL] = {
    " Unconditionl Frame",  "MasterReq",       "SlaveResp",
    "AssignFrameId",        "UnAssignFrameId", "AssignNAD",
    "ConditionalChangeNAD", "Data Dump",       "Save Configuration",
    "Assign Frame Range",   "Free Format",     "Read By Identifier"};

QLDFTableWidgetItem::QLDFTableWidgetItem(QString strName)
    : QTableWidgetItem(strName) {
...
QLDFViewerProcess::QLDFViewerProcess() { m_ouTempFile = nullptr; }
      ...
      ;
    {
      int n =
          LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->SaveToFile(
              m_ouTempFile->fileName().toStdString());
...
int CreateDefaultPropertySettings(double fLDFVersion) {
  ...
  LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->SetProperties(
      eLINClusterProperties, (void*)&ouLINSettings);
...
int CreateDefauleNodeItems(double fLDFVersion) {
  IElement *pMaster = nullptr, *pSlave = nullptr;
  std::string strName;
  ...
  LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->CreateElement(
      eEcuElement, &pMaster);
