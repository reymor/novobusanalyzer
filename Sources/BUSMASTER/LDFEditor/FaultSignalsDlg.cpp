...
FaultSignalsDlg::FaultSignalsDlg(IEcu* pouEcu, double fSlaveProtVer,
                                 std::list<UID_ELEMENT>& lstSignals,
                                 QWidget* parent)
    : QDialog(parent) {
  ui.setupUi(this);
  m_pEcu = pouEcu;
...
void FaultSignalsDlg::vPrepareUi() {
  ...
  for (auto itr : ouSignsalList) {
    ...
    if (ouSignalProps.m_ouSignalType == eSignalNormal) {
      ...
      ui.tableSignalList->item(nRow, 0)->setData(
          Qt::UserRole, QVariant(itr.second->GetUniqueId()));
      nRow++;
    }
    ouColumnList.clear();
  }
...
void FaultSignalsDlg::OnselectedOk() {
  ...
  for (int i = 0; i < nRows; i++) {
    if (Qt::Checked == ui.tableSignalList->item(i, 0)->checkState()) {
      m_lstSignals.push_back(ui.tableSignalList->item(i, 0)
                                 ->data(Qt::UserRole)
                                 .value<UID_ELEMENT>());
    }
  }
