...
void EventFrameEditDlg::vPrepareUiForEditMode() {
  ...
  if (nullptr != pouFrame) {
    ...
    IElement* pTable;
    m_pLdfCluster->GetElement(
        eScheduleTableElement,
        ouFrameProps.m_ouLINEventTrigFrameProps.m_nCollisionResolveTable,
        &pTable);
    if (pTable != nullptr) {
...
void EventFrameEditDlg::vFillScheduleTables() {
  ...
  for (auto itr : pElementMap) {
    ...
    ui.comboCollisionTable->addItem(strName.c_str(),
                                    QVariant(itr.second->GetUniqueId()));
  }
}

void EventFrameEditDlg::vAddFrameToTable(
    IFrame* pFrame, std::map<UID_ELEMENT, UID_ELEMENT> ouSelectedElements) {
  if (nullptr == pFrame) {
    return;
  }
  ...
  if (eLinUnconditionalFrame == frameProps.m_eLinFrameType &&
    ...
    if (ouSelectedElements.find(pFrame->GetUniqueId()) ==
        ouSelectedElements.end()) {
      ui.tableFrames->item(ui.tableFrames->rowCount() - 1, 0)
    ...
    ui.tableFrames->item(ui.tableFrames->rowCount() - 1, 0)
        ->setData(Qt::UserRole, QVariant(pFrame->GetUniqueId()));
  }
}
    ...
    if (nullptr != pFrame) {
      ...
      for (int i = 0; i < nRow; i++) {
        ...
        if (Qt::Checked == pItem->checkState()) {
          m_pLdfCluster->GetElement(eFrameElement, uid,
                                    (IElement**)&pouTempFrame);
          if (nullptr != pouTempFrame) {
            ouFrameProps.m_ouLINEventTrigFrameProps
                .m_pouUnconditionalFrame[uid] = uid;
          }
        }
      }

      // Collision Table;
      UID_ELEMENT uidTable =
          ui.comboCollisionTable->currentData().value<UID_ELEMENT>();
...
int EventFrameEditDlg::nValidateValues() {
  ...
  if (m_strFrameNames.find(ui.editName->text().toStdString()) !=
      m_strFrameNames.end()) {
  ...
  LDFDatabaseManager::GetDatabaseManager()->GetLDFCluster()->GetProperties(
      eLINClusterProperties, &ouLINSettings);

  if ((ouLINSettings.m_dProtocolVers != defVersion_1_3 &&
       ouLINSettings.m_dProtocolVers != defVersion_2_0)) {
    if (ui.comboCollisionTable->currentText().length() == 0) {
      QMessageBox::StandardButton choice;
      choice = QMessageBox::warning(
          this, "Warning",
          "Schedule Table is not Associated.<br>Do you Want to Continue?</br>",
      ...
      UID_ELEMENT uid =
          ui.comboCollisionTable->currentData().value<UID_ELEMENT>();
      m_pLdfCluster->GetElement(eScheduleTableElement, uid,
                                (IElement**)&pTable);
      // Else case may not required
      if (nullptr != pTable) {
        pTable->GetProperties(ouTableProps);
        for (auto itr : ouTableProps.m_ouCLINSheduleTableItem) {
      ...
      if (ouSelectedItems.size() > 0) {
        QMessageBox::StandardButton choice = QMessageBox::warning(
            this, "Warning",
            "Selected Schedule Table does not contain all the Frames Mapped.\
