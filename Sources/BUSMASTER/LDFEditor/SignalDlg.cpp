...
void SignalDlg::vFillElementList() {
  eClusterElementType ouClusterElement[defTotalElement] = {
      eEcuElement, eSignalElement, eCodingElement};
  std::string strName;
...
void SignalDlg::vPrepareUiForNewMode() {
  ...
  for (auto itr : m_ouElementMap[defEcuIndex]) {
    ui.comboPublisher->addItem(QString::fromStdString(itr.first),
                               QVariant(itr.second));
  ...
  for (auto itr : m_ouElementMap[defCodingIndex]) {
    ui.comboCoding->addItem(QString::fromStdString(itr.first),
                            QVariant(itr.second));
  }
...
void SignalDlg::onSelectionOk() {
  ...
  if (strErrors.length() > 0) {
    QMessageBox::critical(this, "Following Errors Occured:", strErrors,
                          QMessageBox::Ok);
    return;
  }
  ...
  if (m_bEditMode == true && nLength > m_nActualLength) {
    QMessageBox::StandardButton reply = QMessageBox::warning(
        this, "Warning",
        "Increasing signal length may overlap in some associted "
        "messages.<br>Do you want to continue</br>",
  ...
  if (m_strCurrentPublisher != "" &&
    ...
    QMessageBox::StandardButton reply =
        QMessageBox::information(this, "Change in Publisher", strTemp,
                                 QMessageBox::Yes | QMessageBox::No);
    if (reply == QMessageBox::No) {
      return;
    }
...
void SignalDlg::onSelecetionCancel() { reject(); }
...
void SignalDlg::vFillSignalDetails() {
  ...
  if (ui.comboCoding->currentIndex() >= 0) {
    ...
    UID_ELEMENT uidCoding =
        ui.comboCoding->currentData(Qt::UserRole).value<UID_ELEMENT>();
    pSignal->SetEncoding(uidCoding);

    if (m_unExistingCoidngUid != uidCoding) {
      // Unmap to existing coding
      m_pLdfCluster->GetElement(eCodingElement, m_unExistingCoidngUid,
                                (IElement**)&pTempCoding);
      if (nullptr != pTempCoding) {
        pTempCoding->UnMapSignal(pSignal->GetUniqueId());
      }

      // Map to new coding
      m_pLdfCluster->GetElement(eCodingElement, uidCoding,
                                (IElement**)&pTempCoding);
      if (nullptr != pTempCoding) {
  ...
  UID_ELEMENT uidPublisher =
      ui.comboPublisher->currentData().value<UID_ELEMENT>();
