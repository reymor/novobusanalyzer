...
void NodeConfigurationDlg::onSelectionOk() {
  ...
  if (EC_FAILURE == nValidateNodes(ouErrString)) {
    ...
    if (eNew == m_ouMode) {
      ...
      m_pLdfCluster->CreateElement(eLinNodeComposition,
                                   (IElement**)&pNodeConfig);
    ...
    for (int nIndex = 0; nIndex < ui.tableWidget->rowCount(); nIndex++) {
      UID_ELEMENT uidECU = ((QComboBox*)ui.tableWidget->cellWidget(nIndex, 0))
      ...
      LineEditWidget* pLineEdit =
          (LineEditWidget*)ui.tableWidget->cellWidget(nIndex, 1);
      if (nullptr != pLineEdit) {
        qStrList = pLineEdit->text().split(',');
        for (auto itrStr = qStrList.begin(); itrStr != qStrList.end();
             itrStr++) {
...
ERRORCODE NodeConfigurationDlg::nValidateNodes(std::string& ouErrString) {
  ...
  if (m_mapNodeNames.find(strNodeConfigName) != m_mapNodeNames.end()) {
    std::ostringstream strStream;
    strStream << "Error: Node Configuration name <b>" << strNodeConfigName
              << "</b> is not unique";
    ouErrString = strStream.str();
    return EC_FAILURE;
  }
    ...
    if (nullptr != pLineEdit) {
      ...
      for (auto itrStr = qStrList.begin(); itrStr != qStrList.end(); itrStr++) {
        ...
        if (strLogicalNode.empty() == true) {
          std::ostringstream strStream;
          strStream << "Error: Logical Node cannot be empty "
                    << "in Composite Node <b>" << strCompNode
                    << "</b> Row Number " << nIndex + 1;
          ouErrString = strStream.str();
          return EC_FAILURE;
        }

        if (m_mapCompositeNodes.find(itrStr->toStdString()) !=
            m_mapCompositeNodes.end()) {
          std::ostringstream strStream;
          strStream << "Error: Logical Node <b>" << itrStr->toStdString()
                    << "</b> not unique in Composite Node <b>" << strCompNode
                    << "</b> Row Number " << nIndex + 1;
          ouErrString = strStream.str();
          return EC_FAILURE;
        }

        if (mapLogicalNodes.find(itrStr->toStdString()) !=
            mapLogicalNodes.end()) {
          std::ostringstream strStream;
          strStream << "Error: Logical Node <b>" << itrStr->toStdString()
                    << "</b> not unique in Composite Node <b>" << strCompNode
        ...
        if (m_logicalNodes.find(itrStr->toStdString()) !=
            m_logicalNodes.end()) {
          std::ostringstream strStream;
          strStream << "Error: Logical Node <b>" << itrStr->toStdString()
                    << "</b> not unique in Composite Node <b>" << strCompNode
...
void NodeConfigurationDlg::vPrepareUIForEdit() {
  ...
  for (auto itrNodeComp : lstCompNodes) {
    ...
    omstrNodeName = "";
    if (EC_SUCCESS ==
        m_pLdfCluster->GetElement(eEcuElement, itrNodeComp.m_uidNode, &pECU)) {
...
void NodeConfigurationDlg::onSelecetionCancel() { reject(); }
